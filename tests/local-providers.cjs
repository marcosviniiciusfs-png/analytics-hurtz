...
global.fetch=async(input,options={})=>{
 ...
 if(url.hostname==='graph.facebook.com'){
  ...
  if(route==='me/adaccounts')return response({data:[{id:who==='a'?'act_111':'act_222',name:'Account '+who,account_status:1,business:{id:'900',name:'Business '+who,...(url.searchParams.get('fields')?.includes('profile_picture_uri')?{profile_picture_uri:'https://media.local.test/bm.png'}:{})}}]});
