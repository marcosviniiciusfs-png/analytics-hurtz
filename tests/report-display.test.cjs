...
const reportGroupingSource=()=>[functionLine('reportGroupName'),sourceLine('const reportGroupNameByProductChannel='),sourceLine('const reportConfiguredProduct='),sourceLine('const reportIsUnidentified='),sourceLine('reportGroupName=function'),functionLine('reportGroupsForRow'),'globalThis.reportProductRules=[];'].join('\n');
...
test('unifies RD Sculpt campaign variants into one product group',()=>{
 const context=vm.createContext({});
 vm.runInContext([sourceLine('const normalizedCampaignText='),sourceLine('const inferredProduct='),reportGroupingSource()].join('\n'),context);
...
test('unifies any niche using a saved product rule',()=>{
 const context=vm.createContext({});
 vm.runInContext([sourceLine('const normalizedCampaignText='),sourceLine('const inferredProduct='),reportGroupingSource()].join('\n'),context);
 context.reportProductRules=[{match:'New Medic',label:'New Medic'}];
 const groups=context.reportGroupsForRow({campaigns:[{campaign_name:'[WA] New Medic Pro | Público frio',spend:100,results:5},{campaign_name:'NEW MEDIC | Remarketing | Cópia',spend:80,results:8}]});
 assert.equal(groups.length,1);assert.equal(groups[0].name,'NEW MEDIC');assert.equal(groups[0].results,13);assert.equal(groups[0].spend,180);
});
...
test('only unidentified report campaigns use their full original names',()=>{
 ...
 vm.runInContext([sourceLine('const normalizedCampaignText='),functionLine('reportGroupName'),'const reportConfiguredProduct=inferredProduct;',sourceLine('const reportIsUnidentified='),sourceLine('reportGroupName=function'),functionLine('reportGroupsForRow')].join('\n'),context);
...
test('report includes zero-spend leads and does not infer channel from campaign name',()=>{
 ...
 vm.runInContext([functionLine('reportGroupName'),'const reportConfiguredProduct=inferredProduct;',functionLine('reportGroupsForRow')].join('\n'),context);
 const groups=context.reportGroupsForRow({campaigns:[
