...
async function exchange(token,{fetchImpl=fetch,config=configuration(),facebookId}={}){
 ...
 try{
  ...
  let expiresAt=0,dataExpiresAt=0;const seconds=Number(data.expires_in??data.expires);
  if(Number.isFinite(seconds)&&seconds>0)expiresAt=Date.now()+seconds*1000;
   ...
   expiresAt=Number(metadata.expires_at)*1000;dataExpiresAt=Number(metadata.data_access_expires_at||0)*1000;
  }
  if(!Number.isFinite(expiresAt)||expiresAt<=Date.now()+86400000||!Number.isFinite(dataExpiresAt)||(dataExpiresAt&&dataExpiresAt<=Date.now())){console.warn(JSON.stringify({event:'meta_token_exchange_refused',reason:'insufficient_lifetime'}));return null}
  return {token:data.access_token,expiresAt,dataExpiresAt,longLived:true};
