...
async function exchange(token,{fetchImpl=fetch,config=configuration()}={}){
 ...
 try{const response=await fetchImpl(new URL('https://graph.facebook.com/v25.0/oauth/access_token'),{method:'POST',body:new URLSearchParams({grant_type:'fb_exchange_token',client_id:APP_ID,client_secret:config.secret,fb_exchange_token:token}),signal:AbortSignal.timeout(20000)});const data=await response.json();const seconds=Number(data.expires_in??data.expires);if(!response.ok||data.error||typeof data.access_token!=='string'||data.access_token.length<20||!Number.isFinite(seconds)||seconds<=0){console.warn(JSON.stringify({event:'meta_token_exchange_refused',status:response.status,code:data.error?.code,subcode:data.error?.error_subcode}));return null}return {token:data.access_token,expiresAt:Date.now()+seconds*1000,longLived:true}}catch(error){console.warn(JSON.stringify({event:'meta_token_exchange_network_error',name:error?.name||'Error'}));return null}
}
