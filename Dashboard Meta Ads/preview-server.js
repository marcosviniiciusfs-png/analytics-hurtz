...
http.createServer((req,res)=>{
  ...
  if(requestUrl.pathname==='/api/creative-audit/download'&&req.method==='GET'){
    ...
    if(!session||!video?.download_url||(session.audit_enabled&&video.expected_type!=='any'&&!audit?.relevant))return jsonResponse(res,404,{error:'Vídeo não encontrado ou pesquisa expirada.'});const token=secretValue('APIFY_TOKEN','APIFY_TOKEN__FILE');
  ...
  if(requestUrl.pathname==='/api/creative-search'&&req.method==='POST')return readBody(req,async(error,payload)=>{
    ...
    try{
      ...
      const candidates=[...unique.values()],sessionId=crypto.randomUUID(),session={id:sessionId,candidates,results:new Map(),per_term:perTerm,audit_enabled:CREATIVE_VISUAL_AUDIT_ENABLED,created_at:Date.now(),expires_at:Date.now()+CREATIVE_AUDIT_TTL};creativeAuditSessions.set(sessionId,session);
      if(!CREATIVE_VISUAL_AUDIT_ENABLED){const counts=new Map(),limited=candidates.filter(video=>{const count=counts.get(video.search_term)||0;if(count>=perTerm)return false;counts.set(video.search_term,count+1);return true}).map(video=>({...video,download_url:video.download_url?`/api/creative-audit/download?session=${encodeURIComponent(sessionId)}&video=${encodeURIComponent(video.id)}`:''}));return jsonResponse(res,200,{id:sessionId,status:'complete',total:0,approved:limited,rejected:0,visual_audit_enabled:false,temporary:true})}
