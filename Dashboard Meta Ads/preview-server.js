...
const creativeVisualAuditEnabled=()=>false;
...
function handleAuthorizedRequest(req,res,requestUrl){
  ...
  if(requestUrl.pathname==='/api/creative-audit/agent/claim'&&req.method==='POST'){
    ...
    if(!job)return jsonResponse(res,200,{job:null});job.status='claimed';if(localRuntime)job.claimedBy=localRuntime.store.user().id;job.claimed_at=now;job.attempts=(job.attempts||0)+1;return jsonResponse(res,200,{job:{id:job.id,expected_type:job.expected_type,title:job.video.title,media_url:`/api/creative-audit/agent/media?id=${encodeURIComponent(job.id)}`}});
  }
  ...
  if(requestUrl.pathname==='/api/creative-audit/agent/result'&&req.method==='POST')return readBody(req,(error,payload)=>{
    if(localRuntime)localRuntime.agentSeen(Date.now());else creativeAgentLastSeen=Date.now();const job=creativeAuditJobs.find(item=>item.id===String(payload?.job_id||'')&&(!localRuntime||creativeAuditSessions.has(item.session_id)));if(error||!job||(localRuntime&&!localRuntime.canFinish(job)))return jsonResponse(res,404,{error:'Trabalho não encontrado ou expirado.'});const session=creativeAuditSessions.get(job.session_id);if(!session)return jsonResponse(res,410,{error:'Pesquisa expirada.'});
  ...
  if(requestUrl.pathname==='/api/creative-audit/agent/media'&&req.method==='GET'){
    if(localRuntime)localRuntime.agentSeen(Date.now());else creativeAgentLastSeen=Date.now();const job=creativeAuditJobs.find(item=>item.id===requestUrl.searchParams.get('id')&&(!localRuntime||creativeAuditSessions.has(item.session_id)));if(!job||(localRuntime&&!localRuntime.canFinish(job)))return jsonResponse(res,404,{error:'Vídeo temporário não encontrado.'});const token=localRuntime?localRuntime.mediaToken(creativeAuditSessions.get(job.session_id).localOwner):secretValue('APIFY_TOKEN','APIFY_TOKEN__FILE');
  ...
  if(requestUrl.pathname==='/api/creative-audit/download'&&req.method==='GET'){
    ...
    if(!session||!video||(session.audit_enabled&&creativeVisualAuditEnabled()&&video.expected_type!=='any'&&!audit?.relevant))return jsonResponse(res,404,{error:'Vídeo não encontrado ou pesquisa expirada.'});const token=secretValue('APIFY_TOKEN','APIFY_TOKEN__FILE');
  ...
  if(requestUrl.pathname==='/api/creative-search'&&req.method==='POST')return readBody(req,async(error,payload)=>{
    ...
    if(localRuntime&&creativeVisualAuditEnabled()&&Date.now()-localRuntime.agentSeen()>45000)return jsonResponse(res,409,{error:'O serviço de análise visual está offline. Tente novamente quando o computador de análise estiver conectado.'});
