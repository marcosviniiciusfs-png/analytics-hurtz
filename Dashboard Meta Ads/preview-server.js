...
async function auditCreativeVisuals(videos,token){
  const audited=new Map(),groups=['car','property'].map(type=>videos.filter(video=>video.expected_type===type&&video.download_url));
  await Promise.all(groups.flatMap(group=>{const jobs=[];for(let index=0;index<group.length;index+=12)jobs.push(group.slice(index,index+12));return jobs}).map(async batch=>{
    ...
    rows.forEach((row,rowIndex)=>{const analysis=parseCreativeVisualAnalysis(row.analysis),video=batch.find(item=>item.download_url===row.url)||batch[rowIndex];if(video)audited.set(video.id,{relevant:analysis.relevant===true,detected_type:analysis.detected_type===type?type:'other',reason:String(analysis.reason||'').slice(0,220)})});
  }));return audited;
}
...
http.createServer((req,res)=>{
  ...
  if(requestUrl.pathname==='/api/creative-search'&&req.method==='POST')return readBody(req,async(error,payload)=>{
    ...
    try{
      const actorInput={searchQueries:terms,resultsPerPage:Math.min(30,perTerm*2),searchSection:'/video',videoSearchSorting:sorting,videoSearchDateFilter:period,shouldDownloadVideos:true,shouldDownloadCovers:false,shouldDownloadSlideshowImages:false,downloadSubtitlesOptions:'NEVER_DOWNLOAD_SUBTITLES',commentsPerPost:0,topLevelCommentsPerPost:0,maxRepliesPerComment:0,maxFollowersPerProfile:0,maxFollowingPerProfile:0,scrapeRelatedSearchWords:false};
      ...
      const approved=candidates.filter(video=>{if(video.expected_type==='any')return true;const audit=visualResults.get(video.id);if(!audit?.relevant)return false;video.visual_verified=true;video.detected_type=audit.detected_type;video.visual_reason=audit.reason;return true}),counts=new Map();const limited=approved.filter(video=>{const count=counts.get(video.search_term)||0;if(count>=perTerm)return false;counts.set(video.search_term,count+1);return true});
      jsonResponse(res,200,{videos:limited,rejected:candidates.length-limited.length,visual_audited:needsAudit.length,requested:{terms,per_term:perTerm,content_type:contentType},temporary:true});
