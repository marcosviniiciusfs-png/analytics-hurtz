...
http.createServer((req,res)=>{
  ...
  if(requestUrl.pathname==='/api/creative-search-settings'){
    ...
    if(req.method==='POST')return readBody(req,(error,payload)=>{const name=String(payload?.name||'').trim().slice(0,80),terms=[...new Set((Array.isArray(payload?.terms)?payload.terms:[]).map(value=>String(value||'').trim()).filter(Boolean))].slice(0,10);if(error||!name||!terms.length)return jsonResponse(res,400,{error:'Informe um nome e pelo menos um termo.'});const settings=readJsonFile(creativeSearchSettingsFile,{presets:[],history:{}}),preset={id:crypto.randomUUID(),name,terms,created_at:new Date().toISOString()};settings.presets=[...(Array.isArray(settings.presets)?settings.presets:[]).filter(item=>item.name.toLowerCase()!==name.toLowerCase()),preset].slice(-50);settings.history=settings.history&&typeof settings.history==='object'?settings.history:{};try{writeJsonFile(creativeSearchSettingsFile,settings);return jsonResponse(res,201,preset)}catch{return jsonResponse(res,500,{error:'Não foi possível salvar os termos.'})}});
    if(req.method==='DELETE'){const id=requestUrl.searchParams.get('id'),settings=readJsonFile(creativeSearchSettingsFile,{presets:[],history:{}});settings.presets=(Array.isArray(settings.presets)?settings.presets:[]).filter(item=>item.id!==id);try{writeJsonFile(creativeSearchSettingsFile,settings);return jsonResponse(res,200,{ok:true})}catch{return jsonResponse(res,500,{error:'Não foi possível excluir os termos.'})}}
    return jsonResponse(res,405,{error:'Método não permitido'});
  }
