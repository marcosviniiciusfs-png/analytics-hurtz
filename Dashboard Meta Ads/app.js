...
initializeCreativeLibrary=function(){
  ...
  form.onsubmit=async event=>{event.preventDefault();const terms=[...new Set(document.querySelector('#creativeSearchTerms').value.split(/\r?\n|,/).map(value=>value.trim()).filter(Boolean))],limit=Math.min(30,Math.max(1,Number(document.querySelector('#creativeResultLimit').value)||10));if(!terms.length)return alert('Informe pelo menos um termo, usando uma linha para cada pesquisa.');if(terms.length*limit>150)return alert('O máximo por execução é 150 vídeos. Reduza os termos ou a quantidade por termo.');const button=form.querySelector('[type="submit"]');button.disabled=true;progress.hidden=false;document.querySelector('#creativeLibraryStatus').textContent=`Pesquisando ${terms.length} termo(s)...`;try{const response=await fetch('/api/creative-search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({terms,limit,sorting:document.querySelector('#creativeSearchSorting').value,period:document.querySelector('#creativeSearchPeriod').value})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'A pesquisa não foi concluída.');creativeVideos=payload.videos||[];renderCreativeLibrary();clear.hidden=false;downloadSelected.hidden=!creativeVideos.some(video=>video.download_url)}catch(error){document.querySelector('#creativeLibraryStatus').textContent=error.message;document.querySelector('#creativeLibrary').innerHTML=`<div class="creative-library-empty"><strong>Não foi possível concluir</strong><span>${escapeHtml(error.message)}</span></div>`}finally{progress.hidden=true;button.disabled=false}};
