...
function renderComments(){
 ...
 root.innerHTML=rows.length?rows.map(item=>`<button type="button" class="comments-ad-card comments-ad-open" data-comment-ad="${escapeHtml(commentAdKey(item))}" aria-haspopup="dialog"><span class="comments-ad-head">${item.ad.thumbnail_url?`<img class="comments-ad-thumb" src="${escapeHtml(item.ad.thumbnail_url)}" alt="" referrerpolicy="no-referrer">`:'<span class="comments-ad-thumb"></span>'}<span class="comments-ad-copy"><strong>${escapeHtml(item.ad.name)}</strong><span>${escapeHtml(item.account_name)} · ${escapeHtml(item.ad.campaign_name)}</span><small>${escapeHtml(commentDate(item.ad))}</small></span><span class="comments-ad-meta"><span class="comments-status ${item.ad.effective_status==='ACTIVE'?'active':'inactive'}">${item.ad.effective_status==='ACTIVE'?'Ativo':item.ad.effective_status==='PAUSED'?'Pausado':'Inativo'}</span><span class="comments-count">${item.comments.length} comentário(s)</span><span class="comment-platform">${item.platform==='instagram'?'Instagram':item.platform==='facebook'?'Facebook · Página':'Origem não informada'}</span><span class="comments-open-label">Ver comentários →</span></span></span></button>`).join(''):'<div class="comments-empty"><strong>Nenhum comentário encontrado</strong><span>Selecione uma página ou altere os filtros.</span></div>';
...
async function moderateComments(action,ids){
 ...
 try{const response=await fetch('/api/meta-comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,comment_ids:ids})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'A moderação não foi conclu?da.');
 const succeeded=new Set((payload.results||[]).filter(row=>row.ok===true&&ids.includes(row.id)).map(row=>row.id));
 commentsData=commentsData.map(item=>({...item,comments:item.comments.filter(comment=>action!=='delete'||!succeeded.has(comment.id)).map(comment=>succeeded.has(comment.id)&&action!=='delete'?{...comment,is_hidden:action==='hide'}:comment)}));
