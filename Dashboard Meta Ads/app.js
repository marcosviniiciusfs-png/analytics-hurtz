...
function loadOptionalScript(name,url){if(window[name])return Promise.resolve(window[name]);if(optionalScripts.has(name))return optionalScripts.get(name);const promise=new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=url;script.async=true;script.onload=()=>{if(window[name])resolve(window[name]);else{optionalScripts.delete(name);reject(new Error('NÃ£o foi possÃ­vel iniciar '+name+'. Tente novamente.'))}};script.onerror=()=>{optionalScripts.delete(name);reject(new Error('NÃ£o foi possÃ­vel carregar '+name+'. Tente novamente.'))};document.head.append(script)});optionalScripts.set(name,promise);return promise}

let personalIdentity=null;
const originalStorage=window.localStorage;
let savedSession=originalStorage.getItem(MONITOR_SESSION_KEY);
if(window.HURTZ_LOCAL&&!savedSession&&!originalStorage.getItem('hurtz-local-signed-out')){const response=await browserFetch('/api/local/session',{method:'POST',headers:{'X-Local-Client':'1'}});if(response.ok){savedSession=(await response.json()).token;originalStorage.setItem(MONITOR_SESSION_KEY,savedSession)}}
if(savedSession){
...
function waitForMonitorLogin(message='Sua sessÃ£o expirou. Entre novamente para continuar.'){
...
function completeMonitorLogin(token){
  originalStorage.setItem(MONITOR_SESSION_KEY,token);originalStorage.removeItem('hurtz-local-signed-out');
  location.reload();
}
async function monitorApiFetch(input,options={}){
  ...
  if(response.status===401){userStorage.removeItem(MONITOR_SESSION_KEY);showAlertLogin('A nova sessÃ£o nÃ£o foi aceita. Entre novamente.');}
  return response;
}
const metadataRequests=new Map(),metadataCache=new Map();let metadataRevision=0;
const metadataRoutes=new Set(['/api/meta-monitor-config','/api/account-profiles','/api/alert-plans','/api/local/settings','/api/meta-comment-history','/api/creative-videos']);
function cachedMetadataFetch(input,options={}){const method=(options.method||'GET').toUpperCase(),route=String(input).split('?')[0];if(method!=='GET'){metadataRevision++;metadataCache.clear();metadataRequests.clear();return monitorApiFetch(input,options)}if(!metadataRoutes.has(route)||options.signal)return monitorApiFetch(input,options);const key=String(input),hit=metadataCache.get(key);if(hit&&hit.expires>Date.now())return Promise.resolve(hit.response.clone());if(metadataRequests.has(key))return metadataRequests.get(key).then(r=>r.clone());const revision=metadataRevision,promise=monitorApiFetch(input,options).then(response=>{if(response.ok&&revision===metadataRevision)metadataCache.set(key,{response:response.clone(),expires:Date.now()+15000});return response}).finally(()=>{if(metadataRequests.get(key)===promise)metadataRequests.delete(key)});metadataRequests.set(key,promise);return promise.then(r=>r.clone())}
...
async function personalReportFetch(input,options={}){
  ...
  const execute=async()=>{
    if(ids.length<=4)return limitedReportRequest(input,options);
    const result={since:url.searchParams.get('from'),until:url.searchParams.get('to'),accounts:{}};
    for(let offset=0;offset<ids.length;offset+=4){
      if(options.signal?.aborted)throw new DOMException('Consulta cancelada','AbortError');
      const batch=ids.slice(offset,offset+4);url.searchParams.set('accounts',batch.join(','));
      const response=await limitedReportRequest(url.pathname+url.search,options);
      if(!response.ok)return response;
      const payload=await response.json();Object.assign(result.accounts,payload.accounts||{});
      if(url.pathname==='/api/meta-spend'){storeAuditPayload(`${result.since}|${result.until}`,payload);renderSummary();renderAccounts(document.querySelector('#searchInput').value)}
    }
...
const brlExact=v=>v==null?'â€”':brl(v);
...
const fmtDateTime=d=>`${fmtDate(d)} Ã s ${d.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}`;
...
function loadPlans(){
  accounts.forEach(a=>{
    ...
    if(a.plan.weekStartDay==null)a.plan.weekStartDay=1;
    if(!a.plan.depositTime)a.plan.depositTime='00:00';
    a.trackingStart=a.plan.depositDate;
    a.trackedSpend=a.cycleSpend;
    // O servidor Ã© a fonte autoritativa. O cache local nÃ£o pode preencher a visÃ£o geral com valores antigos ou nÃ£o auditados.
  });
}
async function savePlans(){
  ...
  const payload=await response.json().catch(()=>null);
  if(!response.ok||!payload?.ok)throw new Error(payload?.error||'A VPS nÃ£o confirmou o salvamento dos limites.');
  return payload;
}
function metrics(a){
...
const statusLabel={active:'SaudÃ¡vel',attention:'AtÃ© 3 dias',empty:'Saldo esgotado'};
...
let globalPeriod={from:null,to:null,label:'Este mÃªs'};
...
function metricGoalState(metric,campaign,days,goals){
  ...
  if(metric==='investment'){
    ...
    if(!configured)return {tone:'neutral',reason:`Neutro: investimento mÃ©dio de ${brl(value)}/dia; configure mÃ­nimo, ideal e mÃ¡ximo.`};
    if(goals.max!==''&&value>Number(goals.max))return {tone:'danger',reason:`Vermelho: ${brl(value)}/dia ultrapassa o mÃ¡ximo de ${brl(goals.max)}.`};
    if(goals.min!==''&&value<Number(goals.min))return {tone:'warning',reason:`Amarelo: ${brl(value)}/dia estÃ¡ abaixo do mÃ­nimo de ${brl(goals.min)}.`};
    if(goals.ideal!==''&&value>Number(goals.ideal))return {tone:'warning',reason:`Amarelo: ${brl(value)}/dia passou do ideal de ${brl(goals.ideal)}, mas segue abaixo do mÃ¡ximo.`};
    return {tone:'good',reason:`Verde: ${brl(value)}/dia estÃ¡ dentro da faixa planejada.`};
  }
  if(metric==='results'){
    if(campaign.results==null)return {tone:'warning',reason:'Amarelo: a Meta nÃ£o retornou resultado auditÃ¡vel para esta campanha.'};
    const value=campaign.results/days;
    if(value===0)return {tone:'danger',reason:'Vermelho: a campanha gastou e teve zero resultados no perÃ­odo.'};
    if(!configured)return {tone:'neutral',reason:`Neutro: mÃ©dia de ${num(value)} resultados/dia; configure mÃ­nimo e ideal.`};
    if(goals.min!==''&&value<Number(goals.min))return {tone:'danger',reason:`Vermelho: ${num(value)} resultados/dia estÃ¡ abaixo do mÃ­nimo de ${num(goals.min)}.`};
    if(goals.ideal!==''&&value<Number(goals.ideal))return {tone:'warning',reason:`Amarelo: ${num(value)} resultados/dia estÃ¡ abaixo do ideal de ${num(goals.ideal)}.`};
    return {tone:'good',reason:`Verde: ${num(value)} resultados/dia atingiu a meta ideal.`};
  }
  if(campaign.costPerResult==null)return {tone:'danger',reason:'Vermelho: houve gasto, mas nÃ£o hÃ¡ resultado para calcular o custo por resultado.'};
  if(!configured)return {tone:'neutral',reason:`Neutro: custo de ${brl(campaign.costPerResult)}; configure o valor ideal e mÃ¡ximo.`};
  if(goals.max!==''&&campaign.costPerResult>Number(goals.max))return {tone:'danger',reason:`Vermelho: ${brl(campaign.costPerResult)} ultrapassa o mÃ¡ximo de ${brl(goals.max)}.`};
  if(goals.ideal!==''&&campaign.costPerResult>Number(goals.ideal))return {tone:'warning',reason:`Amarelo: ${brl(campaign.costPerResult)} estÃ¡ acima do ideal de ${brl(goals.ideal)}.`};
  return {tone:'good',reason:`Verde: ${brl(campaign.costPerResult)} estÃ¡ dentro do custo ideal.`};
}
...
async function findMetaAccounts(showProgress=true){
  ...
  if(showProgress)setButtonLoading(button,true,'Buscando...');status.textContent='Consultando as contas autorizadas pelo seu Facebook...';
  try{
    ...
    const previousIds=new Set(selectedAccountIds),allSelected=accounts.every(account=>previousIds.has(account.id));
    ...
    if(allSelected)selectedAccountIds=new Set(allowedIds);
    ...
    const connectionStatus=document.querySelector('#facebookConnectionStatus');if(connectionStatus)connectionStatus.textContent=accounts.length?`${accounts.length} contas autorizadas pelo seu Facebook. Lista atualizada.`:'O Facebook nÃ£o retornou contas de anÃºncio. Reconecte e confira as contas em Editar configuraÃ§Ãµes.';
    return payload;
...
function applyAccountSelection(){
  ...
  if(!checked.length){alert('Selecione pelo menos uma conta de anÃºncio.');return false}
...
function refreshPresetSelect(){
  ...
  select.innerHTML='<option value="">PredefiniÃ§Ãµes salvas</option>'+Object.keys(presets.items).map(name=>`<option value="${escapeHtml(name)}">${presets.defaultName===name?'âš‘ ':''}${escapeHtml(name)}</option>`).join('');
...
function openPresetManager(){
  ...
  if(!select.value){alert('Selecione uma predefiniÃ§Ã£o para editar.');return}
...
function updateSelectedPreset(){
  ...
  if(!oldName||!newName){alert('Selecione uma predefiniÃ§Ã£o e informe o nome.');return}
...
function deleteSelectedPreset(){
  ...
  if(!name){alert('Selecione uma predefiniÃ§Ã£o para apagar.');return}
  if(!confirm(`Apagar a predefiniÃ§Ã£o "${name}"?`))return;
...
function applyPreset(preset,applyPeriod=true){
  ...
  if(applyPeriod)applyGlobalPeriod(from,to,preset?.label||'PerÃ­odo salvo');
}
...
function applyGlobalPeriod(from,to,label,trigger=null){
  ...
  document.querySelector('#accountsPeriod').textContent=`PerÃ­odo: ${globalPeriod.from.toLocaleDateString('pt-BR')} atÃ© ${globalPeriod.to.toLocaleDateString('pt-BR')} â€¢ saldo conciliado por conta`;
...
function setGlobalRange(range,trigger=null){
  ...
  else if(range==='month'){start.setDate(1);label='Este mÃªs'}
  else{start.setDate(start.getDate()-(Number(range)-1));label=`Ãšltimos ${range} dias`}
...
async function loadAuditedPeriod(from,to,trigger=null,force=false){
  ...
  try{
    ...
    const payload=await response.json();
    if(!response.ok)throw new Error(payload.error||'Falha na coleta');
    storeAuditPayload(key,payload);
    renderSummary();
    renderAccounts(document.querySelector('#searchInput').value);
    if(selectedAccount)renderModal();
    const failed=Object.values(payload.accounts||{}).filter(row=>row.error).length;refresh.textContent=failed?`${failed} conta(s) com auditoria pendente`:'Auditoria concluÃ­da';if(failed){const status=document.querySelector('#facebookConnectionStatus');if(status)status.textContent=`Lista de contas carregada. ${failed} conta(s) nÃ£o puderam ser auditadas neste perÃ­odo; consulte os detalhes e tente atualizar.`}
  }catch(error){
    refresh.textContent='Falha na auditoria';const status=document.querySelector('#facebookConnectionStatus');if(status)status.textContent=error.message;
  }finally{
...
async function loadSelectedAccountAudit(){
  ...
  }catch(error){if(selectedAccount?.id===accountId){document.querySelector('#campaignCount').textContent='Falha na auditoria';document.querySelector('#campaignBody').innerHTML=`<tr><td colspan="6" class="audit-empty">NÃ£o foi possÃ­vel consultar esta conta agora. ${escapeHtml(error.message||'Tente novamente.')}</td></tr>`}}
}
...
function performanceForPeriod(a,from=globalPeriod.from,to=globalPeriod.to){
  ...
  if(live){
    if(!live.reconciled)return {complete:false,spend:null,leads:null,cpl:null,campaigns:null,source:live.error||'Conta e campanhas nÃ£o reconciliaram'};
    const resultsAudited=live.result_reconciled===true,campaigns=(live.campaigns||[]).map(campaign=>{const effective=campaign.effective_status||campaign.status||'UNKNOWN',labels={ACTIVE:'Ativa',PAUSED:'Pausada',ARCHIVED:'Arquivada',DELETED:'ExcluÃ­da',IN_PROCESS:'Em processamento',WITH_ISSUES:'Com problema',CAMPAIGN_PAUSED:'Pausada',ADSET_PAUSED:'Pausada'};return {id:campaign.campaign_id,name:campaign.campaign_name,objective:campaign.objective_label||'NÃ£o informado',metaObjective:campaign.objective,status:labels[effective]||'Status nÃ£o informado',effectiveStatus:effective,spend:Number(campaign.spend),results:campaign.results!=null?Number(campaign.results):null,costPerResult:campaign.cost_per_result!=null?Number(campaign.cost_per_result):null,resultType:campaign.result_type}});
    return {complete:true,resultsAudited,spend:Number(live.spend),accountSpend:Number(live.spend),campaignSum:Number(live.campaign_sum),leads:null,cpl:null,campaigns,daily:live.daily||[],activeCampaignCount:Number(live.active_campaign_count)||0,source:resultsAudited?`Conta ${brl(live.spend)} = campanhas ${brl(live.campaign_sum)}`:`Gasto reconciliado; resultados aguardando estabilizaÃ§Ã£o da Meta`};
  }
  ...
  while(cursor<=end){
    ...
    if(!row)return {complete:false,spend:null,leads:null,cpl:null,campaigns:null,source:'PerÃ­odo ainda nÃ£o auditado'};
  ...
  return {complete:true,spend,leads,cpl:leads?spend/leads:null,campaigns:[...campaigns.values()],source:rows.map(row=>row.source).join(' â€¢ ')};
}
...
function availableFundsForPeriod(account,performance,from=globalPeriod.from,to=globalPeriod.to){
  ...
  if(start>to)return {available:true,value:base,base,spent:0,tone:base>0?'good':'danger',reason:'Nenhum gasto ocorreu porque o ciclo configurado ainda nÃ£o comeÃ§ou.'};
  ...
  if(!live?.reconciled||daily.some(day=>!day.reconciled))return {available:false,value:null,base,spent:null,tone:'warning',reason:'Amarelo: a Meta ainda nÃ£o reconciliou todos os dias desde o inÃ­cio deste ciclo.'};
  const spent=daily.reduce((sum,day)=>sum+(Number(day.account_spend)||0),0),value=Math.max(0,base-spent),ratio=base>0?value/base:0,tone=value<=0?'danger':ratio<=.3?'warning':'good',source=credit?'limite semanal configurado':'verba lÃ­quida apÃ³s a taxa Meta';
...
function renderSummary(){
  ...
  summaryCards.innerHTML=`
    ...
    <article class="metric-card warning summary-filter-card ${activeSummaryFilter==='pending'?'selected':''}" data-summary-filter="pending" tabindex="0"><div class="metric-top">${metricLabel('Auditoria pendente','Contas sem reconciliaÃ§Ã£o completa entre gasto da conta, campanhas e dias do ciclo.')}<span class="metric-icon">â—·</span></div><div class="metric-value">${pending}</div><div class="metric-foot">Nenhum valor Ã© estimado</div></article>
    <article class="metric-card money summary-filter-card ${activeSummaryFilter==='exceeded'?'selected':''}" data-summary-filter="exceeded" tabindex="0"><div class="metric-top">${metricLabel('Acima do limite diÃ¡rio','Contas que ultrapassaram o limite diÃ¡rio em pelo menos um dos Ãºltimos trÃªs dias.')}<span class="metric-icon">â†—</span></div><div class="metric-value">${exceeded==null?'â€”':exceeded}</div><div class="metric-foot">Auditoria diÃ¡ria dos Ãºltimos 3 dias</div></article>`;
...
function forecastCell(a){
  const m=metrics(a);
  if(m.credit)return `<span class="forecast safe">CartÃ£o de crÃ©dito</span><small class="forecast-date">Verba semanal ${brl(a.plan.weeklyLimit)}</small>`;
  return `<span class="forecast ${m.status==='attention'?'warning':'safe'}">${a.plan.plannedDays} dias de duraÃ§Ã£o</span><small class="forecast-date">Termina ${fmtDate(m.plannedEnd)}</small>`;
}
...
function spendHeat(a,p){
  if(!p.complete)return {tone:'warning',reason:'Amarelo: auditoria da Meta pendente para o perÃ­odo selecionado.'};
  if(!(a.plan.dailyLimit>0))return {tone:'neutral',reason:'Neutro: configure um limite diÃ¡rio para avaliar o gasto.'};
  ...
  if(ratio>1)return {tone:'danger',reason:`Vermelho: maior gasto diÃ¡rio ${brl(maxDaily)}, acima do limite de ${brl(a.plan.dailyLimit)}.`};
  if(ratio>=.9)return {tone:'warning',reason:`Amarelo: maior gasto diÃ¡rio ${brl(maxDaily)}, entre 90% e 100% do limite de ${brl(a.plan.dailyLimit)}.`};
  return {tone:'good',reason:`Verde: maior gasto diÃ¡rio ${brl(maxDaily)}, abaixo de 90% do limite de ${brl(a.plan.dailyLimit)}.`};
}
function renderAccounts(filter=''){
  ...
  accountsBody.innerHTML=list.map(a=>{const m=metrics(a),p=performanceForPeriod(a),available=availableFundsForPeriod(a,p),status=metaAccountStatus(a),planExpired=!m.credit&&NOW>=m.plannedEnd;return `<tr>
    ...
    ${heatCell(`<span class="status ${status.css}">${status.label}</span>`,a.activeCampaignCount==null?'warning':a.activeCampaignCount>0?'good':'danger',a.activeCampaignCount==null?'Amarelo: aguardando a consulta de campanhas na Meta.':a.activeCampaignCount>0?`Verde: ${a.activeCampaignCount} campanhas estÃ£o ativas na Meta.`:'Vermelho: nenhuma campanha ativa foi encontrada.')}
    ${heatCell(`<div class="objective">${a.objectives.map(o=>`<span class="tag">${o}</span>`).join('')}</div>`,'neutral','Neutro: objetivo da campanha Ã© informativo e nÃ£o representa desempenho.')}
    ${m.credit?heatCell(`<strong>CartÃ£o de crÃ©dito</strong><small class="table-sub">Sem depÃ³sito ou saldo prÃ©-pago</small>`,'neutral','Conta configurada para acompanhar somente os gastos auditados.','number'):heatCell(`<strong>${brl(a.plan.deposit)}</strong><small class="table-sub">${fmtDateTime(m.start)} â€¢ ${a.plan.plannedDays} dias</small>`,a.plan.deposit<=0?'danger':planExpired?'warning':'good',a.plan.deposit<=0?'Vermelho: nenhum depÃ³sito foi configurado.':planExpired?`Laranja: o prazo de ${a.plan.plannedDays} dias deste depÃ³sito terminou em ${fmtDateTime(m.plannedEnd)}.`:`Verde: depÃ³sito vigente atÃ© ${fmtDateTime(m.plannedEnd)}.`,'number')}
    ${heatCell(available.available?`<strong>${brl(available.value)}</strong><small class="table-sub">${brl(available.base)} âˆ’ ${brl(available.spent)} Meta</small>`:`<strong>â€”</strong><small class="table-sub">Auditoria pendente</small>`,available.tone,available.reason,'number')}
    ${heatCell(available.available?`<strong>${brl(available.spent)}</strong><small class="table-sub">Usado desde o inÃ­cio do ciclo</small>`:`<strong>â€”</strong><small class="table-sub">Auditoria pendente</small>`,available.tone,available.reason,'number')}
    <td><button class="plan-button" data-plan="${a.id}" title="Configurar pagamento e limites">âš™</button><button class="arrow-button" data-open="${a.id}">â€º</button></td></tr>`}).join('');
  ...
  accountsFoot.innerHTML=`<tr><td colspan="3">GERAL â€¢ ${list.length} CONTAS</td><td class="number">${brl(deposit)}</td><td class="number">${availableTotal==null?'â€”':brl(availableTotal)}</td><td class="number">${usedTotal==null?'â€”':brl(usedTotal)}</td><td>${allAvailable?'Auditoria concluÃ­da':'Auditoria pendente'}</td></tr>`;
  const filterNames={active:'Contas ativas',empty:'Contas sem saldo',pending:'Auditoria pendente',exceeded:'Acima do limite diÃ¡rio'};
  document.querySelector('#accountCount').textContent=`Exibindo ${list.length} de ${accounts.length} contas${activeSummaryFilter!=='all'?` â€¢ ${filterNames[activeSummaryFilter]}`:''}`;
...
const campaignGoalDefinitions={
  investment:{title:'Meta de investimento diÃ¡rio',fields:[['min','MÃ­nimo por dia (R$)'],['ideal','Ideal â€” valor abaixo de (R$)'],['max','MÃ¡ximo â€” valor acima de (R$)']]},
  results:{title:'Meta de resultados diÃ¡rios',fields:[['min','MÃ­nimo por dia'],['ideal','Ideal â€” valor acima de']]},
  cost:{title:'Meta de custo por resultado',fields:[['ideal','Ideal â€” valor abaixo de (R$)'],['max','MÃ¡ximo â€” valor acima de (R$)']]}
};
function openCampaignGoal(metric){
  ...
  document.querySelector('#campaignGoalFields').innerHTML=definition.fields.map(([name,label])=>`<label>${label}<input type="number" min="0" step="0.01" name="${name}" value="${saved[name]??''}" placeholder="NÃ£o definido" /></label>`).join('');
...
function openAccount(id,showPlan=false){closeCampaignGoal();selectedAccount=accounts.find(a=>a.id===id);document.querySelector('#modalTitle').textContent=selectedAccount.name;document.querySelector('#modalSubtitle').textContent=`${selectedAccount.id} â€¢ ${isCreditAccount(selectedAccount)?'controle de gastos no cartÃ£o':'planejamento conciliado da conta'}`;fillPlan();syncModalDates();renderModal();togglePlan(showPlan);modal.classList.add('open');modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';loadSelectedAccountAudit()}
...
function syncPaymentTypeFields(){const credit=document.querySelector('#paymentType').value==='credit';document.querySelectorAll('[data-prepaid-field]').forEach(field=>{field.hidden=credit;field.querySelector('input,select').required=!credit});document.querySelectorAll('[data-credit-field]').forEach(field=>{field.hidden=!credit;field.querySelector('input,select').required=credit});document.querySelector('#planFormTitle').textContent=credit?'Controle de gastos no cartÃ£o':'Planejamento do depÃ³sito';document.querySelector('#planFormDescription').textContent=credit?'Informe a verba semanal, o limite diÃ¡rio e o horÃ¡rio semanal de reset. O histÃ³rico Ã© preservado; o ciclo exibido volta a zero.':'O ciclo considera somente gastos posteriores Ã  data e ao horÃ¡rio do depÃ³sito.';document.querySelector('#savePaymentPlan').textContent=credit?'Salvar limites':'Salvar planejamento'}
function togglePlan(show){planForm.hidden=!show;document.querySelector('#planToggle').textContent=show?'Fechar configuraÃ§Ã£o':'Configurar pagamento'}
function renderModal(){
  ...
  const credit=m.credit,cycleWindow=weeklyCycleWindow(to,a.plan.weekStartDay),cycleStart=parseDate(localIso(cycleWindow.start)),periodSpend=officialSpendForPeriod(a,from,to),cycleSpend=officialSpendForPeriod(a,cycleStart,to),periodUsed=periodSpend.available?periodSpend.value:null,cycleUsed=cycleSpend.available?cycleSpend.value:null,periodLimit=applicableCreditLimit(from,to,a.plan),periodRemaining=periodUsed==null?null:Math.max(0,periodLimit-periodUsed),cycleRemaining=cycleUsed==null?null:Math.max(0,a.plan.weeklyLimit-cycleUsed),periodRatio=periodUsed!=null&&periodLimit>0?periodUsed/periodLimit:null,cycleRatio=cycleUsed!=null&&a.plan.weeklyLimit>0?cycleUsed/a.plan.weeklyLimit:null,averageDaily=periodUsed==null?null:periodUsed/periodDays,cycleLabel=Number(a.plan.weekStartDay)===0?'Domingo Ã s 23:55':'Segunda Ã s 00:05',spendState=periodSpend.audited?'Auditado pela Meta':periodSpend.provisional?'Oficial da Meta â€¢ provisÃ³rio':'Aguardando Meta',cycleSpendState=cycleSpend.audited?'Auditado pela Meta':cycleSpend.provisional?'Oficial da Meta â€¢ provisÃ³rio':'Aguardando Meta';
  const planStatus=m.start>NOW?'Ainda nÃ£o iniciado':m.calendarDaysRemaining===0?'Finalizado':'Em andamento';
  document.querySelector('#modalSummary').innerHTML=(credit?[
    ['Forma de pagamento','CartÃ£o de crÃ©dito','Conta acompanhada exclusivamente pelo gasto oficial da Meta.'],
    ['Gasto no perÃ­odo',brlExact(periodUsed),`Valor usado na Meta entre ${from.toLocaleDateString('pt-BR')} e ${to.toLocaleDateString('pt-BR')}. Status: ${spendState}.`],
    ['Restante no perÃ­odo',periodRemaining==null?'â€”':brl(periodRemaining),`Limite aplicÃ¡vel de ${brl(periodLimit)} menos o gasto oficial do perÃ­odo.`],
    ['Uso do perÃ­odo',periodRatio==null?'â€”':`${Math.round(periodRatio*100)}%`,'Percentual consumido do limite aplicÃ¡vel ao perÃ­odo selecionado.'],
    ['Gasto no ciclo',brlExact(cycleUsed),`Gasto oficial no ciclo que contÃ©m ${to.toLocaleDateString('pt-BR')}, iniciado ${cycleLabel.toLowerCase()}. Status: ${cycleSpendState}.`],
    ['Restante no ciclo',cycleRemaining==null?'â€”':brl(cycleRemaining),'Verba semanal menos o gasto do ciclo que contÃ©m a data final do filtro.']
  ]:[
    ['Saldo estimado',brl(m.balance),'Verba disponÃ­vel menos o consumo calculado desde o depÃ³sito.'],
    ['Valor depositado',brl(a.plan.deposit),'Valor bruto informado no depÃ³sito da conta.'],
    ['Taxa Meta (12,15%)',brl(m.fee),'Parcela de 12,15% descontada do valor depositado.'],
    ['Verba disponÃ­vel',brl(m.availableBudget),'Valor lÃ­quido que poderÃ¡ ser usado nos anÃºncios.'],
    ['DuraÃ§Ã£o',`${a.plan.plannedDays} dias`,'Quantidade de dias definida para o saldo durar.'],
    ['Fim previsto',fmtDate(m.plannedEnd),'Data do depÃ³sito somada Ã  duraÃ§Ã£o planejada.']
  ]).map(([l,v,h])=>`<div class="modal-stat">${metricLabel(l,h)}<strong>${v}</strong></div>`).join('');
  document.querySelector('#planStrip').innerHTML=credit?`
    <div>${metricLabel('Teto diÃ¡rio','Valor mÃ¡ximo permitido por dia.')}<strong>${brl(a.plan.dailyLimit)}</strong></div>
    <div>${metricLabel('Verba semanal','Valor total disponÃ­vel durante os sete dias do ciclo.')}<strong>${brl(a.plan.weeklyLimit)}</strong></div>
    <div>${metricLabel('MÃ©dia diÃ¡ria','Gasto oficial do perÃ­odo dividido pela quantidade de dias selecionados.')}<strong>${averageDaily==null?'Aguardando Meta':brl(averageDaily)}</strong></div>
    ...
    <div>${metricLabel('PrÃ³ximo reset','Momento em que o painel inicia um novo ciclo sem apagar o histÃ³rico.')}<strong>${fmtDateTime(cycleWindow.nextReset)}</strong></div>`:`
    <div>${metricLabel('Data e hora do depÃ³sito','Momento exato a partir do qual os gastos entram no ciclo.')}<strong>${fmtDateTime(m.start)}</strong></div>
    <div>${metricLabel('Limite diÃ¡rio','Valor mÃ¡ximo planejado para gastar por dia.')}<strong>${brl(a.plan.dailyLimit)}</strong></div>
    <div>${metricLabel('Dias transcorridos','Dias completos desde o depÃ³sito dentro deste planejamento.')}<strong>${Math.min(m.elapsed,a.plan.plannedDays)} de ${a.plan.plannedDays}</strong></div>
    <div>${metricLabel('Status do planejamento','Mostra se o ciclo ainda vai comeÃ§ar, estÃ¡ ativo ou terminou.')}<strong>${planStatus}</strong></div>`;
  ...
  document.querySelector('#campaignBody').innerHTML=p.complete&&p.campaigns?.length?p.campaigns.map(campaign=>{
    ...
    return `<tr><td class="campaign-name">${campaign.name}<small>ID ${campaign.id}</small></td>${heatCell(`<span class="tag campaign-objective-tag">${campaign.objective}</span>`,'neutral',`Neutro: resultado principal mapeado como ${campaign.objective} pelo nome, objetivo e action type da Meta.`)}${heatCell(`<span class="status ${statusCss}">${campaign.status}</span>`,statusTone,`${campaign.status}: status atual retornado pela Meta (${campaign.effectiveStatus}).`)}${heatCell(brl(campaign.spend),investmentState.tone,investmentState.reason,'number')}${heatCell(campaign.results==null?'â€”':num(campaign.results),resultState.tone,resultState.reason,'number')}${heatCell(campaign.costPerResult==null?'â€”':`<strong>${brl(campaign.costPerResult)}</strong>`,costState.tone,costState.reason,'number')}</tr>`;
  }).join(''):`<tr><td colspan="6" class="audit-empty">Dados indisponÃ­veis: este perÃ­odo ainda nÃ£o passou pela auditoria Meta.</td></tr>`;
  const displayedCampaigns=p.complete?(p.campaigns||[]):[],totalSpend=displayedCampaigns.reduce((sum,campaign)=>sum+campaign.spend,0),allResultsAudited=displayedCampaigns.length>0&&displayedCampaigns.every(campaign=>campaign.results!=null),totalResults=allResultsAudited?displayedCampaigns.reduce((sum,campaign)=>sum+campaign.results,0):null,generalCost=totalResults>0?totalSpend/totalResults:null,generalCostLabel=displayedCampaigns.length&&displayedCampaigns.every(campaign=>campaign.objective==='FormulÃ¡rio')?'CPL geral':'Custo/resultado geral';
  document.querySelector('#campaignFoot').innerHTML=displayedCampaigns.length?`<tr><td colspan="3"><strong>TOTAL â€¢ ${displayedCampaigns.length} CAMPANHAS</strong><small>Todas as campanhas com veiculaÃ§Ã£o no perÃ­odo, incluindo pausadas</small></td><td class="number"><strong>${brl(totalSpend)}</strong><small>Valor usado</small></td><td class="number"><strong>${totalResults==null?'â€”':num(totalResults)}</strong><small>Resultados auditados</small></td><td class="number"><strong>${generalCost==null?'â€”':brl(generalCost)}</strong><small>${generalCostLabel}</small></td></tr><tr class="campaign-audit-row"><td colspan="6"><strong>âœ“ AUDITORIA META CONCLUÃDA</strong><small>Conta ${brl(p.accountSpend)} = soma de todas as campanhas com veiculaÃ§Ã£o ${brl(p.campaignSum)}.</small></td></tr>`:'';
...
function renderNetBudgetPreview(){const credit=document.querySelector('#paymentType').value==='credit',deposit=Number(document.querySelector('#depositAmount').value)||0,daily=Number(document.querySelector('#dailyLimit').value)||0,weekly=Number(document.querySelector('#weeklyLimit').value)||0,cycle=Number(document.querySelector('#weekStartDay').value)===0?'domingo Ã s 23:55':'segunda Ã s 00:05';document.querySelector('#netBudgetPreview').innerHTML=credit?`<span>Limite diÃ¡rio: <b>${brl(daily)}</b></span><strong>${brl(weekly)} disponÃ­veis â€¢ reset ${cycle}</strong>`:`<span>Taxa de 12,15%: <b>${brl(feeAmount(deposit))}</b></span><strong>DisponÃ­vel para anÃºncios: ${brl(netBudget(deposit))}</strong>`}
planForm.addEventListener('submit',async e=>{
  ...
  if(!(nextPlan.dailyLimit>0)||paymentType==='credit'&&!(nextPlan.weeklyLimit>0)){document.querySelector('#netBudgetPreview').innerHTML='<strong>Informe limites diÃ¡rio e semanal maiores que zero.</strong>';return}
  selectedAccount.plan=nextPlan;button.disabled=true;button.textContent='Salvando...';
  try{
    await savePlans();
    document.querySelector('#modalSubtitle').textContent=`${selectedAccount.id} â€¢ ${paymentType==='credit'?'controle de gastos no cartÃ£o':'planejamento conciliado da conta'}`;
    ...
    document.querySelector('#netBudgetPreview').innerHTML=`<strong>NÃ£o foi possÃ­vel salvar: ${escapeHtml(error.message)}</strong>`;
  }finally{
...
setInterval(()=>{if(selectedAccount&&!modal.hidden&&activeAccountTab!=='manage'&&isCreditAccount(selectedAccount))loadSelectedAccountAudit()},300000);
...
document.querySelector('#applyDateFilter').onclick=async event=>{const from=parseDate(document.querySelector('#globalDateFrom').value),to=parseDate(document.querySelector('#globalDateTo').value);if(from>to){alert('A data inicial deve ser anterior Ã  data final.');return}if(!applyAccountSelection())return;toggleDatePanel(false);await applyGlobalPeriod(from,to,`${from.toLocaleDateString('pt-BR')} â€“ ${to.toLocaleDateString('pt-BR')}`,event.currentTarget)};
document.querySelector('#toggleAllAccounts').onclick=()=>{const inputs=[...document.querySelectorAll('#accountFilterOptions input')],allChecked=inputs.every(input=>input.checked);inputs.forEach(input=>input.checked=!allChecked);document.querySelector('#toggleAllAccounts').textContent=allChecked?'Selecionar todas':'Limpar seleÃ§Ã£o'};
...
document.querySelector('#saveFilterPreset').onclick=()=>{if(!applyAccountSelection())return;const name=document.querySelector('#presetName').value.trim();if(!name){alert('Digite um nome para a predefiniÃ§Ã£o.');return}const presets=readPresets();presets.items[name]=currentPreset();writePresets(presets);refreshPresetSelect();selectPreset(name);document.querySelector('#presetName').value=''};
document.querySelector('#setDefaultPreset').onclick=()=>{let name=document.querySelector('#savedFilterPreset').value||document.querySelector('#presetName').value.trim();if(!name){alert('Selecione ou salve uma predefiniÃ§Ã£o primeiro.');return}const presets=readPresets();if(!presets.items[name]){if(!applyAccountSelection())return;presets.items[name]=currentPreset()}presets.defaultName=name;writePresets(presets);refreshPresetSelect();selectPreset(name)};
...
function updatePeriodLabel(){const f=document.querySelector('#dateFrom').value,t=document.querySelector('#dateTo').value;document.querySelector('#campaignPeriod').textContent=`PerÃ­odo: ${parseDate(f).toLocaleDateString('pt-BR')} atÃ© ${parseDate(t).toLocaleDateString('pt-BR')}`}
...
const shortName=(value,max=28)=>value.length>max?`${value.slice(0,max-1)}â€¦`:value;
...
let analysisLoaderAnimations=[];
function clearAnalysisLoaders(){analysisLoaderAnimations.forEach(animation=>animation.destroy());analysisLoaderAnimations=[]}
function analysisLoadingChart(){return '<div class="analysis-lottie-loading" role="status" aria-label="Carregando e auditando dados"><div class="analysis-lottie-animation" aria-hidden="true"></div></div>'}
function hydrateAnalysisLoaders(){clearAnalysisLoaders();if(!window.lottie){loadOptionalScript('lottie','https://cdnjs.cloudflare.com/ajax/libs/lottie-web/5.12.2/lottie.min.js').then(hydrateAnalysisLoaders).catch(()=>{});return}document.querySelectorAll('#analysis .analysis-lottie-animation').forEach(container=>analysisLoaderAnimations.push(window.lottie.loadAnimation({container,renderer:'svg',loop:true,autoplay:true,path:'toaster.json?v=20260723-02'})))}
function barChart(items,format=value=>brl(value),selected=null){if(!items.length)return emptyChart('Ainda nÃ£o hÃ¡ dados auditados para este grÃ¡fico.');const max=Math.max(...items.map(item=>item.value),1);return `<div class="bar-chart">${items.map((item,index)=>`<button class="bar-row ${selected===item.id?'selected':''}" type="button" data-chart-id="${escapeHtml(item.id)}" title="${escapeHtml(item.fullLabel||item.label)}: ${escapeHtml(format(item.value))}"><span class="bar-label">${escapeHtml(shortName(item.label))}</span><span class="bar-track"><span class="bar-fill" style="width:${Math.max(1,item.value/max*100)}%;background:${item.color||chartPalette[index%chartPalette.length]}"></span></span><span class="bar-value">${escapeHtml(format(item.value))}</span></button>`).join('')}</div>`}
function trendChart(points){if(!points.length)return emptyChart('A evoluÃ§Ã£o diÃ¡ria aparecerÃ¡ apÃ³s a auditoria do perÃ­odo.');const width=760,height=190,pad=24,max=Math.max(...points.map(point=>point.value),1),step=points.length===1?0:(width-pad*2)/(points.length-1);const coords=points.map((point,index)=>({x:pad+index*step,y:height-pad-(point.value/max)*(height-pad*2),...point}));const line=coords.map((point,index)=>`${index?'L':'M'} ${point.x} ${point.y}`).join(' ');const area=`${line} L ${coords.at(-1).x} ${height-pad} L ${coords[0].x} ${height-pad} Z`;return `<svg class="trend-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="EvoluÃ§Ã£o diÃ¡ria do investimento"><line class="trend-grid" x1="${pad}" y1="${height-pad}" x2="${width-pad}" y2="${height-pad}"/><path class="trend-area" d="${area}"/><path class="trend-line" d="${line}"/>${coords.map((point,index)=>`<g><circle class="trend-point" tabindex="0" data-day="${point.id}" cx="${point.x}" cy="${point.y}" r="5"><title>${point.label}: ${brl(point.value)}</title></circle>${(points.length<=10||index%Math.ceil(points.length/8)===0)?`<text class="trend-label" text-anchor="middle" x="${point.x}" y="${height-5}">${point.label.slice(0,5)}</text>`:''}</g>`).join('')}</svg>`}
function renderGeneralAnalysis(){const root=document.querySelector('#analysis');if(!root||root.hidden)return;const rows=auditedRows(),totalSpend=rows.reduce((sum,row)=>sum+row.performance.spend,0),campaigns=rows.flatMap(row=>(row.performance.campaigns||[]).map(campaign=>({...campaign,accountId:row.account.id,accountName:row.account.name}))),auditedResults=campaigns.filter(campaign=>campaign.results!=null),totalResults=auditedResults.reduce((sum,campaign)=>sum+campaign.results,0),generalCost=totalResults>0?totalSpend/totalResults:null;
  document.querySelector('#analysisPeriod').textContent=`${globalPeriod.from.toLocaleDateString('pt-BR')} atÃ© ${globalPeriod.to.toLocaleDateString('pt-BR')} â€¢ ${rows.length} de ${selectedAccountIds.size} contas reconciliadas`;
  document.querySelector('#analysisKpis').innerHTML=kpiMarkup([{label:'Investimento auditado',value:brl(totalSpend),note:'Soma das contas reconciliadas'},{label:'Resultados auditÃ¡veis',value:auditedResults.length?num(totalResults):'â€”',note:'Somente action types confirmados'},{label:'Custo por resultado',value:generalCost==null?'â€”':brl(generalCost),note:'Investimento Ã· resultados'},{label:'Cobertura da auditoria',value:`${rows.length}/${selectedAccountIds.size}`,note:'Contas concluÃ­das no filtro'}]);
...
function ensureAccountAnalysisUI(){const modalToolbar=document.querySelector('.modal-toolbar'),campaignSection=document.querySelector('.campaign-section');if(!modalToolbar||!campaignSection||document.querySelector('.modal-tabs'))return;const tabs=document.createElement('div');tabs.className='modal-tabs';tabs.innerHTML='<button class="active" type="button" data-account-tab="campaigns">Campanhas</button><button type="button" data-account-tab="analysis">AnÃ¡lise</button>';modalToolbar.before(tabs);campaignSection.id='campaignTabPanel';const panel=document.createElement('div');panel.className='account-analysis';panel.id='accountAnalysisPanel';panel.hidden=true;panel.innerHTML='<div class="account-analysis-head"><div><h3>AnÃ¡lise da conta</h3><p id="accountAnalysisPeriod"></p></div><span class="audit-chip pending" id="accountAnalysisAudit">Auditoria pendente</span></div><div class="analysis-kpis" id="accountAnalysisKpis"></div><div class="analysis-grid"><article class="chart-card chart-wide"><div class="chart-head"><div><h3>Campanhas por investimento</h3><p>Clique para destacar uma campanha.</p></div></div><div class="interactive-chart" id="campaignSpendChart"></div></article><article class="chart-card"><div class="chart-head"><div><h3>Resultados por objetivo</h3><p>Somente resultados retornados pela Meta.</p></div></div><div class="interactive-chart" id="campaignObjectiveChart"></div></article><article class="chart-card"><div class="chart-head"><div><h3>EficiÃªncia</h3><p>Custo por resultado auditado.</p></div></div><div class="interactive-chart" id="campaignEfficiencyChart"></div></article><article class="chart-card chart-wide"><div class="chart-head"><div><h3>ComparaÃ§Ã£o histÃ³rica de atÃ© 90 dias</h3><p>CritÃ©rios por campanhas do mesmo objetivo; nenhuma aÃ§Ã£o Ã© automÃ¡tica.</p></div></div><div id="historicalRecommendations"></div></article></div><div class="analysis-pending"><strong>Leitura responsÃ¡vel</strong><span>Campanhas sem resultados auditÃ¡veis permanecem sem classificaÃ§Ã£o. RecomendaÃ§Ãµes exigem amostra mÃ­nima e comparaÃ§Ã£o com campanhas do mesmo tipo de resultado.</span></div>';campaignSection.after(panel);tabs.querySelectorAll('button').forEach(button=>button.onclick=()=>setAccountTab(button.dataset.accountTab));}
...
async function loadHistorical90(){if(!selectedAccount)return;const period=historicalPeriod(),id=selectedAccount.id;if(HISTORICAL_90_DATA[id])return renderAccountAnalysis();const target=document.querySelector('#historicalRecommendations');if(target)target.innerHTML=emptyChart('Auditando os Ãºltimos 90 dias na Meta...');try{const response=await fetch(`/api/meta-spend?from=${iso(period.from)}&to=${iso(period.to)}&accounts=${encodeURIComponent(id)}`);if(!response.ok)throw new Error();const payload=await response.json(),row=payload.accounts?.[id];HISTORICAL_90_DATA[id]=row?.reconciled?row:{error:true};renderAccountAnalysis()}catch(error){HISTORICAL_90_DATA[id]={error:true};renderAccountAnalysis()}}
function recommendationFor(campaign,peerMedian){if(campaign.results==null)return {tone:'pending',label:'Auditoria incompleta',reason:'Resultado nÃ£o retornado para o action type esperado.'};if(campaign.results===0)return {tone:'danger',label:'Manter pausada / revisar',reason:'Houve investimento e nenhum resultado no perÃ­odo.'};if(peerMedian==null)return {tone:'pending',label:'Amostra insuficiente',reason:'NÃ£o hÃ¡ campanhas comparÃ¡veis do mesmo objetivo.'};const paused=(campaign.effective_status||campaign.status)==='PAUSED';if(paused&&campaign.results>=5&&campaign.cost_per_result<=peerMedian*.8)return {tone:'good',label:'Candidata a reativaÃ§Ã£o',reason:`Ao menos 5 resultados e custo 20% ou mais abaixo da mediana ${brl(peerMedian)}.`};if(paused&&campaign.results>=3&&campaign.cost_per_result<=peerMedian*1.15)return {tone:'warning',label:'Testar nova versÃ£o',reason:`HistÃ³rico competitivo; republicar como teste controlado, sem duplicaÃ§Ã£o automÃ¡tica.`};if(campaign.cost_per_result>peerMedian*1.5)return {tone:'danger',label:'Revisar antes de investir',reason:`Custo mais de 50% acima da mediana ${brl(peerMedian)}.`};return {tone:'neutral',label:paused?'Manter em observaÃ§Ã£o':'Manter e acompanhar',reason:`Custo dentro da faixa comparÃ¡vel Ã  mediana ${brl(peerMedian)}.`}}
function renderHistoricalRecommendations(){const target=document.querySelector('#historicalRecommendations');if(!target||!selectedAccount)return;const raw=HISTORICAL_90_DATA[selectedAccount.id];if(!raw)return target.innerHTML=emptyChart('Abra esta aba para iniciar a auditoria histÃ³rica.');if(raw.error)return target.innerHTML=emptyChart('NÃ£o foi possÃ­vel reconciliar o histÃ³rico de 90 dias; nenhuma recomendaÃ§Ã£o foi gerada.');const campaigns=raw.campaigns||[],medians={};[...new Set(campaigns.map(campaign=>campaign.objective_label))].forEach(objective=>medians[objective]=median(campaigns.filter(campaign=>campaign.objective_label===objective&&campaign.results>0&&campaign.cost_per_result!=null).map(campaign=>Number(campaign.cost_per_result))));const ranked=campaigns.map(campaign=>({...campaign,recommendation:recommendationFor(campaign,medians[campaign.objective_label])})).sort((a,b)=>({good:0,warning:1,neutral:2,danger:3,pending:4}[a.recommendation.tone]-({good:0,warning:1,neutral:2,danger:3,pending:4}[b.recommendation.tone]))).slice(0,20);target.innerHTML=ranked.length?`<table class="analysis-ranking-table"><thead><tr><th>Campanha</th><th>Objetivo</th><th class="number">Valor usado</th><th class="number">Resultados</th><th class="number">Custo/resultado</th><th>CritÃ©rio</th></tr></thead><tbody>${ranked.map(campaign=>`<tr><td><strong>${escapeHtml(campaign.campaign_name)}</strong><small>${escapeHtml(campaign.last_delivery_date||'Data indisponÃ­vel')}</small></td><td>${escapeHtml(campaign.objective_label)}</td><td class="number">${brl(campaign.spend)}</td><td class="number">${campaign.results==null?'â€”':num(campaign.results)}</td><td class="number">${campaign.cost_per_result==null?'â€”':brl(campaign.cost_per_result)}</td><td><span class="recommendation ${campaign.recommendation.tone}" title="${escapeHtml(campaign.recommendation.reason)}">${escapeHtml(campaign.recommendation.label)}</span></td></tr>`).join('')}</tbody></table>`:emptyChart('Nenhuma campanha teve veiculaÃ§Ã£o nos Ãºltimos 90 dias.');}
function renderAccountAnalysis(){const panel=document.querySelector('#accountAnalysisPanel');if(!panel||panel.hidden||!selectedAccount)return;const from=parseDate(document.querySelector('#dateFrom').value),to=parseDate(document.querySelector('#dateTo').value),performance=performanceForPeriod(selectedAccount,from,to),campaigns=performance.complete?(performance.campaigns||[]):[],resultCampaigns=campaigns.filter(campaign=>campaign.results!=null),results=resultCampaigns.reduce((sum,campaign)=>sum+campaign.results,0),cost=results?performance.spend/results:null;document.querySelector('#accountAnalysisPeriod').textContent=`${from.toLocaleDateString('pt-BR')} atÃ© ${to.toLocaleDateString('pt-BR')}`;const audit=document.querySelector('#accountAnalysisAudit');audit.textContent=performance.complete?'Reconciliada com a Meta':'Auditoria pendente';audit.classList.toggle('pending',!performance.complete);document.querySelector('#accountAnalysisKpis').innerHTML=kpiMarkup([{label:'Valor usado',value:performance.complete?brl(performance.spend):'â€”',note:'Total oficial da conta'},{label:'Resultados',value:resultCampaigns.length?num(results):'â€”',note:'Action types confirmados'},{label:'Custo/resultado',value:cost==null?'â€”':brl(cost),note:'MÃ©dia ponderada'},{label:'Campanhas',value:performance.complete?num(campaigns.length):'â€”',note:'Com veiculaÃ§Ã£o no perÃ­odo'}]);document.querySelector('#campaignSpendChart').innerHTML=barChart(campaigns.map(campaign=>({id:campaign.id,label:campaign.name,fullLabel:campaign.name,value:campaign.spend})).sort((a,b)=>b.value-a.value).slice(0,12),brl,accountAnalysisCampaignSelection);const objectiveResults=new Map();resultCampaigns.forEach(campaign=>objectiveResults.set(campaign.objective,(objectiveResults.get(campaign.objective)||0)+campaign.results));document.querySelector('#campaignObjectiveChart').innerHTML=barChart([...objectiveResults].map(([label,value])=>({id:label,label,value})).sort((a,b)=>b.value-a.value),num);document.querySelector('#campaignEfficiencyChart').innerHTML=barChart(campaigns.filter(campaign=>campaign.costPerResult!=null).map(campaign=>({id:campaign.id,label:campaign.name,fullLabel:campaign.name,value:campaign.costPerResult})).sort((a,b)=>a.value-b.value).slice(0,10),brl,accountAnalysisCampaignSelection);panel.querySelectorAll('#campaignSpendChart [data-chart-id],#campaignEfficiencyChart [data-chart-id]').forEach(button=>button.onclick=()=>{accountAnalysisCampaignSelection=accountAnalysisCampaignSelection===button.dataset.chartId?null:button.dataset.chartId;renderAccountAnalysis()});renderHistoricalRecommendations()}
...
/* AnÃ¡lise detalhada por conta e perÃ­odo, baseada em breakdowns auditados. */
...
const inferredProduct=name=>{const text=normalizedCampaignText(name);for(const [label,patterns] of [['Vaga de emprego',[/\bVAGA(?:S)?\b|EMPREGO|RECRUTAMENTO|CONTRATACAO/]],['CrÃ©dito rural',[/CREDITO RURAL|CRED RURAL|RURAL/]],['CaminhÃ£o',[/CAMINH(?:AO|A0|O|ES)?/,/CAMIAO|CAMINAHO|CAMNHAO/]],['MaquinÃ¡rio',[/MAQ(?:UIN|IN|UNI)AR(?:IO|IA)?/,/MAQUINA|EQUIPAMENTO/]],['ImÃ³vel',[/IMOV(?:EL|EIS|EU|IL)/,/APARTAM|TERRENO|CASA\b/]],['VeÃ­culo',[/VEIC(?:ULO|ULOS|OLO)/,/VEICL|VEICUO|AUTOMOVEL/]],['Carro',[/CARRO|CARROS/]],['Moto',[/MOTO|MOTOS/]],['ConsÃ³rcio',[/CONSORC|CONSOC|CONSORSS/]]])if(patterns.some(pattern=>pattern.test(text)))return label;return 'NÃ£o identificado'};
const qualificationTier=product=>({'ImÃ³vel':1,'Carro':2,'VeÃ­culo':2,'Moto':2,'CaminhÃ£o':3,'MaquinÃ¡rio':4,'CrÃ©dito rural':5})[product]||0;
function comparisonSummaries(ads){const groups=new Map();ads.filter(ad=>ad.spend>0&&ad.result_type!=='Sem resultado atribuÃ­do'&&!isEmploymentCampaign(ad)).forEach(ad=>{const key=`${ad.account_id}|${ad.objective}|${ad.result_type}`,row=groups.get(key)||{account_id:ad.account_id,objective:ad.objective,result_type:ad.result_type,spend:0,results:0,impressions:0,clicks:0,products:new Map(),formats:new Map(),ads:[]};row.spend+=Number(ad.spend)||0;row.results+=Number(ad.results)||0;row.impressions+=Number(ad.impressions)||0;row.clicks+=Number(ad.clicks)||0;const product=inferredProduct(ad.campaign_name),format=ad.format||'NÃ£o identificado';row.products.set(product,(row.products.get(product)||0)+Number(ad.spend||0));row.formats.set(format,(row.formats.get(format)||0)+Number(ad.spend||0));row.ads.push(ad);groups.set(key,row)});return [...groups.values()].map(row=>{const top=map=>[...map].sort((a,b)=>b[1]-a[1])[0]||['NÃ£o identificado',0];row.name=monitoredAnalysisAccounts.find(item=>item.id===row.account_id)?.name||row.account_id;row.cpl=row.results?row.spend/row.results:null;row.resultsPer100=row.spend?row.results/row.spend*100:0;row.ctr=row.impressions?row.clicks/row.impressions*100:0;[row.topProduct,row.topProductSpend]=top(row.products);[row.topFormat,row.topFormatSpend]=top(row.formats);row.productShare=row.spend?row.topProductSpend/row.spend*100:0;row.formatShare=row.spend?row.topFormatSpend/row.spend*100:0;row.qualificationTier=qualificationTier(row.topProduct);return row})}
function comparisonVerdict(first,second){let firstWins=0,secondWins=0;const reasons=[];if(first.cpl!=null||second.cpl!=null){const costWinner=first.cpl!=null&&(second.cpl==null||first.cpl<second.cpl)?first:second;costWinner===first?firstWins++:secondWins++;reasons.push(`menor CPL: ${costWinner.name}`)}if(first.results!==second.results){const volumeWinner=first.results>second.results?first:second;volumeWinner===first?firstWins++:secondWins++;reasons.push(`maior volume: ${volumeWinner.name}`)}if(first.qualificationTier&&second.qualificationTier&&first.qualificationTier!==second.qualificationTier){const qualityWinner=first.qualificationTier>second.qualificationTier?first:second;qualityWinner===first?firstWins++:secondWins++;reasons.push(`maior qualificaÃ§Ã£o esperada: ${qualityWinner.name}`)}const winner=firstWins===secondWins?(first.cpl!=null&&(second.cpl==null||first.cpl<second.cpl)?first:second):(firstWins>secondWins?first:second);return{winner,loser:winner===first?second:first,reasons}}
function buildComparisons(data){const summaries=comparisonSummaries(data.ads),groups=new Map(),pairs=[];summaries.forEach(row=>{const key=`${row.objective}|${row.result_type}`,list=groups.get(key)||[];list.push(row);groups.set(key,list)});groups.forEach(list=>{for(let i=0;i<list.length;i++)for(let j=i+1;j<list.length;j++){const first=list[i],second=list[j];if(first.results<1&&second.results<1)continue;const verdict=comparisonVerdict(first,second),winner=verdict.winner,loser=verdict.loser,efficiencyGap=loser.cpl&&winner.cpl?Math.abs(loser.cpl-winner.cpl)/Math.max(loser.cpl,winner.cpl)*100:100,pair={a:loser,b:winner,winner,loser,efficiencyGap,criteriaSummary:verdict.reasons.join(' â€¢ ')};const segmentContext=winner.topProduct!==loser.topProduct&&winner.qualificationTier&&loser.qualificationTier?winner.qualificationTier>loser.qualificationTier?`${winner.topProduct} tem maior qualificaÃ§Ã£o esperada que ${loser.topProduct}.`:`${winner.topProduct} costuma gerar lead mais barato, porÃ©m menos qualificado que ${loser.topProduct}; valide a qualidade comercial.`:'';pair.hypothesis=winner.topProduct!==loser.topProduct?`${winner.name} concentrou ${winner.productShare.toFixed(0)}% da verba identificada em ${winner.topProduct}; ${loser.name}, em ${loser.topProduct}. ${segmentContext}`:winner.topFormat!==loser.topFormat?`${winner.name} concentrou ${winner.formatShare.toFixed(0)}% em ${winner.topFormat}; ${loser.name}, em ${loser.topFormat}.`:`As duas contas tÃªm produto e formato predominantes semelhantes; investigue pÃºblico, criativo e posicionamento.`;pairs.push(pair)}});return pairs.sort((x,y)=>y.efficiencyGap-x.efficiencyGap).slice(0,8)}
...
function openComparisonModal(index){const pair=analysisComparisons[index];if(!pair)return;const {a,b,winner}=pair,modal=document.querySelector('#comparisonModal');document.querySelector('#comparisonModalTitle').textContent=`${a.name} Ã— ${b.name}`;document.querySelector('#comparisonModalPeriod').textContent=`${analysisFrom.toLocaleDateString('pt-BR')} atÃ© ${analysisTo.toLocaleDateString('pt-BR')} â€¢ ${analysisObjectiveName(a.objective)} â€¢ ${a.result_type}`;document.querySelector('#comparisonModalBody').innerHTML=`<div class="comparison-detail-verdict"><span>Melhor equilÃ­brio observado</span><strong>${escapeHtml(winner.name)}</strong><p>${escapeHtml(pair.hypothesis)}</p><small>${escapeHtml(pair.criteriaSummary)}. AssociaÃ§Ã£o observada, nÃ£o prova de causalidade. Vagas de emprego nÃ£o participam da comparaÃ§Ã£o.</small></div><div class="comparison-detail-grid">${[a,b].map(row=>`<article><h3>${escapeHtml(row.name)}</h3><dl><div><dt>Resultados</dt><dd>${num(row.results)}</dd></div><div><dt>Valor usado</dt><dd>${brl(row.spend)}</dd></div><div><dt>Custo/resultado</dt><dd>${row.cpl==null?'â€”':brl(row.cpl)}</dd></div><div><dt>Resultados/R$ 100</dt><dd>${row.resultsPer100.toLocaleString('pt-BR',{maximumFractionDigits:2})}</dd></div><div><dt>CTR</dt><dd>${row.ctr.toLocaleString('pt-BR',{maximumFractionDigits:2})}%</dd></div><div><dt>Produto predominante</dt><dd>${escapeHtml(row.topProduct)} (${row.productShare.toFixed(0)}%)</dd></div><div><dt>Formato predominante</dt><dd>${escapeHtml(row.topFormat)} (${row.formatShare.toFixed(0)}%)</dd></div></dl></article>`).join('')}</div><div class="comparison-evidence"><h3>DistribuiÃ§Ã£o observada</h3>${miniMetric('Resultados',a.results,b.results)}${miniMetric('Custo/resultado',a.cpl||0,b.cpl||0,true)}${miniMetric('Resultados por R$ 100',a.resultsPer100,b.resultsPer100)}</div>`;modal.classList.add('open');modal.setAttribute('aria-hidden','false')}
...
let comparisonArrowAnimations=[];
function clearComparisonArrows(){comparisonArrowAnimations.forEach(animation=>animation.destroy());comparisonArrowAnimations=[]}
function hydrateComparisonArrows(){clearComparisonArrows();if(!window.lottie){loadOptionalScript('lottie','https://cdnjs.cloudflare.com/ajax/libs/lottie-web/5.12.2/lottie.min.js').then(hydrateComparisonArrows).catch(()=>{});return}document.querySelectorAll('#analysisObjectiveComparison .comparison-arrow-animation').forEach(container=>comparisonArrowAnimations.push(window.lottie.loadAnimation({container,renderer:'svg',loop:true,autoplay:true,path:'comparison-arrow.json?v=20260723-01'})))}
function renderObjectiveComparison(data){clearComparisonArrows();const target=document.querySelector('#analysisObjectiveComparison');if(analysisSelectedAccount!=='all'){target.innerHTML=emptyChart('Selecione â€œComparar todas as contasâ€ para visualizar comparaÃ§Ãµes entre contas.');return}analysisComparisons=buildComparisons(data);target.innerHTML=analysisComparisons.length?`<div class="comparison-list">${analysisComparisons.map((pair,index)=>{const {a,b,winner}=pair,maxResults=Math.max(a.results,b.results,1);return `<button type="button" class="comparison-card" data-comparison-index="${index}"><div class="comparison-card-head"><span>${escapeHtml(analysisObjectiveName(a.objective))} â€¢ ${escapeHtml(a.result_type)}</span><b>Ver detalhes â†’</b></div><div class="comparison-sides"><div><strong>${escapeHtml(a.name)}</strong><span>${num(a.results)} resultados â€¢ ${a.cpl==null?'â€”':brl(a.cpl)}</span><i><em style="width:${a.results/maxResults*100}%"></em></i></div><span class="comparison-arrow" role="img" aria-label="Da conta com pior equilÃ­brio para a conta com melhor equilÃ­brio"><span class="comparison-arrow-animation" aria-hidden="true"></span></span><div class="winner"><strong>${escapeHtml(b.name)}</strong><span>${num(b.results)} resultados â€¢ ${b.cpl==null?'â€”':brl(b.cpl)}</span><i><em style="width:${b.results/maxResults*100}%"></em></i></div></div><p><strong>${escapeHtml(winner.name)}</strong> apresentou o melhor equilÃ­brio nos critÃ©rios observados. ${escapeHtml(pair.hypothesis)}</p><small>${escapeHtml(pair.criteriaSummary)} â€¢ vagas de emprego excluÃ­das â€¢ mÃ©tricas oficiais da Meta</small></button>`}).join('')}</div>`:emptyChart('NÃ£o hÃ¡ duas contas comparÃ¡veis com o mesmo objetivo e tipo de resultado auditado neste perÃ­odo. Campanhas de vagas de emprego sÃ£o excluÃ­das.');target.querySelectorAll('[data-comparison-index]').forEach(button=>button.onclick=()=>openComparisonModal(Number(button.dataset.comparisonIndex)));requestAnimationFrame(hydrateComparisonArrows)}
function renderDetailedAnalysis(data){clearAnalysisLoaders();const chip=document.querySelector('#analysisAuditChip'),name=analysisSelectedAccount==='all'?'VisÃ£o consolidada':(monitoredAnalysisAccounts.find(item=>item.id===analysisSelectedAccount)?.name||'Conta');document.querySelector('#analysisPeriod').textContent=`${name} â€¢ ${analysisFrom.toLocaleDateString('pt-BR')} atÃ© ${analysisTo.toLocaleDateString('pt-BR')}`;if(!data){chip.textContent=analysisLoading?'Consultando a Meta...':'Dados indisponÃ­veis';chip.classList.add('pending');document.querySelector('#analysisKpis').innerHTML='';document.querySelector('#analysisGeographyHighlight').innerHTML='';['analysisObjectiveComparison','analysisAgeChart','analysisFormatChart','analysisGeographyChart','analysisPlacementChart','analysisCtrChart','analysisCplChart','analysisAdsRanking'].forEach(id=>document.querySelector(`#${id}`).innerHTML=analysisLoading?analysisLoadingChart():emptyChart('NÃ£o foi possÃ­vel carregar esta anÃ¡lise.'));if(analysisLoading)requestAnimationFrame(hydrateAnalysisLoaders);return}chip.textContent=data.partial?'AnÃ¡lise parcial: hÃ¡ contas sem auditoria':data.reconciled?'Gasto reconciliado com a Meta':'Auditoria pendente';chip.classList.toggle('pending',!data.reconciled);const m=data.account;document.querySelector('#analysisKpis').innerHTML=analysisKpiMarkup([
    {label:'Valor usado',value:brl(m.spend),note:'Investimento oficial',help:'Total usado pela conta no perÃ­odo selecionado.'},{label:'Resultados',value:num(m.results),note:'FormulÃ¡rios e conversas mapeados',help:'Resultados atribuÃ­dos aos action types confirmados da Meta.'},{label:'CPL / custo por resultado',value:m.cost_per_result==null?'â€”':brl(m.cost_per_result),note:'MÃ©dia ponderada',help:'Valor usado dividido pelos resultados mapeados.'},{label:'ImpressÃµes',value:num(m.impressions),note:'ExibiÃ§Ãµes dos anÃºncios',help:'Quantidade total de vezes que os anÃºncios foram exibidos.'},{label:'Alcance',value:num(m.reach),note:'Pessoas alcanÃ§adas',help:'Quantidade estimada de pessoas que viram os anÃºncios.'},{label:'Cliques',value:num(m.clicks),note:'Cliques registrados',help:'Cliques contabilizados pela Meta no perÃ­odo.'},{label:'CTR',value:`${Number(m.ctr).toLocaleString('pt-BR',{maximumFractionDigits:2})}%`,note:'Cliques Ã· impressÃµes',help:'Percentual de impressÃµes que geraram clique.'},{label:'CPM',value:brl(m.cpm),note:'Custo por mil impressÃµes',help:'Valor mÃ©dio pago a cada mil impressÃµes.'},{label:'CPC',value:brl(m.cpc),note:'Custo por clique',help:'Valor usado dividido pela quantidade de cliques.'}
  ]);renderObjectiveComparison(data);document.querySelector('#analysisAgeChart').innerHTML=data.age.reconciled?donutChart(data.age.rows,data.age.rows.some(row=>row.results>0)?'results':'impressions','age'):emptyChart('Idade ainda nÃ£o reconciliou com o gasto da conta.');document.querySelector('#analysisFormatChart').innerHTML=data.format.reconciled?donutChart(data.format.rows,'impressions','format'):emptyChart('Formato ainda nÃ£o reconciliou com o gasto da conta.');const geoRows=[...(data.geography?.rows||[])].sort((a,b)=>(b.results-a.results)||(b.spend-a.spend)),geoHasResults=geoRows.some(row=>row.results>0),geoTop=geoRows[0],geoLevel=data.geography?.level==='city'?'Cidade':'RegiÃ£o';document.querySelector('#analysisGeographyHighlight').innerHTML=data.geography?.reconciled&&geoTop?`<span>${geoLevel} em destaque</span><strong>${escapeHtml(geoTop.region)}</strong><small>${geoHasResults?`${num(geoTop.results)} resultados â€¢ ${geoTop.cost_per_result==null?'Custo indisponÃ­vel':brl(geoTop.cost_per_result)+' por resultado'}`:`${brl(geoTop.spend)} usados â€¢ sem resultado atribuÃ­do`}</small>`:'';document.querySelector('#analysisGeographyChart').innerHTML=data.geography?.reconciled?barChart(geoRows.slice(0,15).map(row=>({id:row.region,label:row.region,value:geoHasResults?row.results:row.spend,fullLabel:`${row.region} â€¢ ${num(row.results)} resultados â€¢ ${brl(row.spend)} usados â€¢ CTR ${Number(row.ctr).toLocaleString('pt-BR',{maximumFractionDigits:2})}%`})),geoHasResults?num:brl):emptyChart('LocalizaÃ§Ã£o ainda nÃ£o reconciliou com o gasto da conta.');const placements=data.placement.rows.map(row=>({...row,label:`${placementName(row.publisher_platform)} â€¢ ${placementName(row.platform_position)}`})).sort((a,b)=>b.spend-a.spend);document.querySelector('#analysisPlacementChart').innerHTML=data.placement.reconciled?barChart(placements.map(row=>({id:row.label,label:row.label,value:row.spend,fullLabel:`${row.label} â€¢ ${num(row.results)} resultados â€¢ CTR ${row.ctr.toFixed(2)}%`})),brl):emptyChart('Posicionamentos ainda nÃ£o reconciliaram com o gasto da conta.');document.querySelector('#analysisCtrChart').innerHTML=data.format.reconciled?barChart(data.format.rows.map(row=>({id:row.format,label:row.format,value:row.ctr})).sort((a,b)=>b.value-a.value),value=>`${Number(value).toLocaleString('pt-BR',{maximumFractionDigits:2})}%`):emptyChart('CTR por formato aguardando auditoria.');document.querySelector('#analysisCplChart').innerHTML=data.age.reconciled?barChart(data.age.rows.filter(row=>row.cost_per_result!=null).map(row=>({id:row.age,label:row.age,value:row.cost_per_result})).sort((a,b)=>a.value-b.value),brl):emptyChart('CPL por idade aguardando auditoria.');const ads=[...data.ads].filter(ad=>ad.spend>0).sort((a,b)=>(b.results-a.results)||(a.cost_per_result??Infinity)-(b.cost_per_result??Infinity)).slice(0,30);document.querySelector('#analysisAdsRanking').innerHTML=ads.length?`<div class="analysis-table-scroll"><table class="analysis-ranking-table"><thead><tr><th>AnÃºncio</th><th>Formato</th><th class="number">Valor usado</th><th class="number">Resultados</th><th class="number">CPL</th><th class="number">CTR</th><th class="number">CPM</th></tr></thead><tbody>${ads.map(ad=>{const accountName=monitoredAnalysisAccounts.find(item=>item.id===ad.account_id)?.name||ad.account_id;return `<tr><td><span class="ranking-account-name">${escapeHtml(accountName)}</span><strong>${escapeHtml(ad.ad_name||'Sem nome')}</strong><small>ID ${escapeHtml(ad.ad_id)}</small></td><td>${escapeHtml(ad.format)}</td><td class="number">${brl(ad.spend)}</td><td class="number">${num(ad.results)}</td><td class="number">${ad.cost_per_result==null?'â€”':brl(ad.cost_per_result)}</td><td class="number">${Number(ad.ctr).toLocaleString('pt-BR',{maximumFractionDigits:2})}%</td><td class="number">${brl(ad.cpm)}</td></tr>`}).join('')}</tbody></table></div>`:emptyChart('Nenhum anÃºncio teve entrega no perÃ­odo.');document.querySelector('#analysisNotice span').textContent=data.reconciled&&data.age.reconciled&&data.geography?.reconciled&&data.placement.reconciled&&data.format.reconciled?'Conta, anÃºncios, idade, localizaÃ§Ã£o, posicionamento e formato reconciliaram com o valor usado da Meta.':'Algum breakdown ainda nÃ£o reconciliou; o grÃ¡fico correspondente foi bloqueado.'}
...
function initializeReportAnimation(){if(reportLottieAnimation)return;if(!window.lottie){loadOptionalScript('lottie','https://cdnjs.cloudflare.com/ajax/libs/lottie-web/5.12.2/lottie.min.js').then(initializeReportAnimation).catch(()=>{});return}reportLottieAnimation=window.lottie.loadAnimation({container:document.querySelector('#reportLottie'),renderer:'svg',loop:true,autoplay:false,path:'report-loader.json'})}
let reportMemberScope='',reportMemberSelection=new Set(),reportMemberOpen=false;
function reportMemberAccounts(){return [...document.querySelectorAll('#reportAccountOptions input')].map(input=>({id:input.value,name:input.nextElementSibling.textContent,selected:input.checked,picture:input.dataset.businessPicture||''}))}
function reportMemberName(name){return String(name).replace(/^(?:\[CA\]|CA(?=\s|[-â€“:]))\s*[-â€“:]?\s*/i,'').trim()||name}
function reportMemberAvatar(a){let url='';try{const parsed=new URL(a.picture);if(parsed.protocol==='https:')url=parsed.href}catch{}return '<span class="account-member-initials">'+escapeHtml(reportMemberInitials(a.name))+'</span>'+(url?'<img class="account-member-photo" src="'+escapeHtml(url)+'" alt="" referrerpolicy="no-referrer" loading="lazy">':'')}
function reportMemberInitials(name){return reportMemberName(name).split(/\s+/).slice(0,2).map(n=>n[0]).join('').toUpperCase()}
function setReportMemberAccounts(scoped){const signature=scoped.map(a=>a.id).sort().join('|');if(signature!==reportMemberScope){reportMemberScope=signature;let saved;try{saved=JSON.parse(userStorage.getItem("hurtz-report-selection-v1")||"null")}catch{}reportMemberSelection=new Set(saved?.scope===signature&&Array.isArray(saved.ids)?saved.ids:scoped.map(a=>String(a.id)))}document.querySelector('#reportAccountOptions').innerHTML=scoped.map(a=>`<label><input type="checkbox" value="${escapeHtml(a.id)}" data-business-picture="${escapeHtml(a.businessPicture||a.business_profile_picture_uri||'')}" ${reportMemberSelection.has(String(a.id))?'checked':''}><span>${escapeHtml(a.name||a.id)}</span></label>`).join('');renderReportMembers()}
function renderReportMembers(){const all=reportMemberAccounts(),sorted=[...all].sort((a,b)=>Number(b.selected)-Number(a.selected)),selected=all.filter(a=>a.selected);reportMemberSelection=new Set(selected.map(a=>a.id));if(all.length)try{userStorage.setItem("hurtz-report-selection-v1",JSON.stringify({scope:reportMemberScope,ids:[...reportMemberSelection]}))}catch{}
 document.querySelector('#reportMemberStrip').innerHTML=sorted.slice(0,5).map(a=>`<button type="button" class="account-member-avatar ${a.selected?'selected':''}" data-report-member="${escapeHtml(a.id)}" aria-pressed="${a.selected}" aria-label="${a.selected?'Remover':'Selecionar'} ${escapeHtml(a.name)}" title="${escapeHtml(a.name)} Â· ${escapeHtml(a.id)}"><span class="account-member-circle">${reportMemberAvatar(a)}<span class="account-member-mark" aria-hidden="true">${a.selected?'âœ“':'+'}</span></span><span class="account-member-name">${escapeHtml(reportMemberName(a.name))}</span></button>`).join('')+`<button type="button" id="reportMemberAdd" class="account-member-avatar account-member-add" aria-expanded="${reportMemberOpen}" aria-controls="reportMemberDropdown"><span class="account-member-circle" aria-hidden="true">${reportMemberOpen?'Ã—':'+'}</span><span>Adicionar${selected.length>5?' (+'+(selected.length-5)+')':''}</span></button>`;
 document.querySelector('#reportMemberSummary').textContent=all.length?`${selected.length} de ${all.length} contas selecionadas`:'Nenhuma conta disponÃ­vel. Conecte o Facebook ou revise o filtro principal.';
 document.querySelector('#reportToggleAccounts').textContent=selected.length?'Limpar todas':'Selecionar todas';document.querySelector('#reportToggleAccounts').disabled=!all.length;document.querySelector('#reportMemberAdd').disabled=!all.length;renderReportMemberList();
}
function renderReportMemberList(){const list=document.querySelector('#reportMemberList'),scrollTop=list.scrollTop;const query=document.querySelector('#reportMemberSearch').value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase(),rows=reportMemberAccounts().filter(a=>(a.name+' '+a.id).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().includes(query));list.innerHTML=rows.length?rows.map(a=>`<button type="button" class="account-member-option ${a.selected?'selected':''}" data-report-member="${escapeHtml(a.id)}" aria-pressed="${a.selected}"><span class="account-member-small" aria-hidden="true">${reportMemberAvatar(a)}</span><span class="account-member-copy"><strong>${escapeHtml(a.name)}</strong><small>${escapeHtml(a.id)}</small></span><span class="account-member-check" aria-hidden="true">${a.selected?'âœ“':''}</span></button>`).join(''):'<p class="account-member-empty">Nenhuma conta encontrada.</p>';list.scrollTop=scrollTop}
function closeReportMembers(restoreFocus=false){reportMemberOpen=false;document.querySelector('#reportMemberDropdown').hidden=true;document.querySelector('#reportMemberSearch').value='';renderReportMembers();if(restoreFocus)document.querySelector('#reportMemberAdd').focus()}
document.querySelector('#reportMemberSelector').addEventListener('click',event=>{const toggle=event.target.closest('[data-report-member]');if(toggle){const id=toggle.dataset.reportMember,inDropdown=Boolean(toggle.closest('#reportMemberList')),input=[...document.querySelectorAll('#reportAccountOptions input')].find(el=>el.value===id);if(!input)return;input.checked=!input.checked;renderReportMembers();[...document.querySelectorAll((inDropdown?'#reportMemberList':'#reportMemberStrip')+' [data-report-member]')].find(el=>el.dataset.reportMember===id)?.focus({preventScroll:true});return}if(event.target.closest('#reportMemberAdd')){reportMemberOpen=!reportMemberOpen;document.querySelector('#reportMemberDropdown').hidden=!reportMemberOpen;renderReportMembers();if(reportMemberOpen)document.querySelector('#reportMemberSearch').focus()}});
document.querySelector('#reportMemberSelector').addEventListener('error',event=>{if(event.target.matches('.account-member-photo'))event.target.remove()},true);
document.querySelector('#reportMemberSearch').addEventListener('input',()=>{document.querySelector('#reportMemberList').scrollTop=0;renderReportMemberList()});
document.querySelector('#reportMemberSearch').addEventListener('keydown',event=>{if(event.key==='Enter')event.preventDefault();if(event.key==='ArrowDown'){event.preventDefault();document.querySelector('#reportMemberList button')?.focus()}});
document.querySelector('#reportMemberSelector').addEventListener('keydown',event=>{if(event.key==='Escape'&&reportMemberOpen){event.preventDefault();closeReportMembers(true)}});
document.addEventListener('pointerdown',event=>{if(reportMemberOpen&&!event.target.closest('#reportMemberSelector'))closeReportMembers()});
document.querySelector('#reportMemberSelector').addEventListener('focusout',event=>{if(reportMemberOpen&&event.relatedTarget&&!event.currentTarget.contains(event.relatedTarget))closeReportMembers()});
async function initializeReports(){if(reportInitialized)return;reportInitialized=true;initializeReportAnimation();const yesterday=new Date(NOW);yesterday.setDate(yesterday.getDate()-1);document.querySelector('#reportDateFrom').value=localIso(yesterday);document.querySelector('#reportDateTo').value=localIso(yesterday);try{const response=await fetch('/api/meta-monitor-config'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao carregar contas');if(!accounts.length&&payload.accounts?.length)setReportMemberAccounts(payload.accounts)}catch(error){reportInitialized=false;document.querySelector('#reportStatus').textContent=error.message||'NÃ£o foi possÃ­vel carregar as contas monitoradas.'}}
function syncReportAccountsWithDashboard(){
  ...
  if(!scoped.length&&root.children.length)return;
  setReportMemberAccounts(scoped);
  const status=document.querySelector('#reportStatus');
  if(status&&!scoped.length)status.textContent='O filtro principal nÃ£o possui contas selecionadas.';
}
...
function reportGroupName(campaign){const product=inferredProduct(campaign.campaign_name||campaign.name),channel=campaign.no_delivery_in_period?'SEM VEICULAÃ‡ÃƒO NO PERÃODO':String(campaign.objective_label||'Resultado nÃ£o identificado').toUpperCase();return `${product.toUpperCase()} - ${channel}`}

function reportCampaignCity(text){const normalized=String(text||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase();if(/\bCANAA(?:\s+DOS\s+CARAJAS)?\b/.test(normalized))return 'CANAÃƒ';if(/\b(?:PARAUAPEBAS|PARAUPEBAS)\b/.test(normalized))return 'PARAUAPEBAS';return ''}
...
function createPngReportEdit(accountId,displayName,row,groups,insights,context){
  const products=[...new Set(groups.map(group=>group.productLabel||group.name.split(' - ')[0]).filter(name=>name!=='NÃƒO IDENTIFICADO'))].join(' â€¢ ')||'NÃƒO IDENTIFICADO';
  const summaryText=`${displayName} gerou ${num(insights.totalResults)} resultados confirmados, com investimento total de ${brl(row.spend)} entre ${context.from.toLocaleDateString('pt-BR')} e ${context.to.toLocaleDateString('pt-BR')}.`;
  const lowerCards=[
    {title:'Resumo do perÃ­odo',text:summaryText,icon:'document'},
    {title:'O que estÃ¡ bom',text:insights.good,icon:'clipboard'},
    {title:'O que pode melhorar',text:insights.improve,icon:'trend'}
  ];
  return {reportTitle:'RelatÃ³rio de Performance',accountName:displayName,period:`${context.from.toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})} a ${context.to.toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})}`,campaignsLabel:'Campanhas:',products,leadsLabel:'RESULTADOS',leadsValue:num(insights.totalResults),cplLabel:'CUSTO POR RESULTADO',cplValue:insights.overallCpl==null?'â€”':brl(insights.overallCpl),spendLabel:'VALOR GASTO',spendValue:brl(row.spend),extraMetrics:[],lowerCards,groupWidth:55,groupTitle:'DESEMPENHO POR GRUPO DE CAMPANHA',confirmedLabel:'TOTAL CONFIRMADO',confirmedValue:num(insights.totalResults),confirmedUnit:'resultados',investmentText:`Investimento total de ${brl(row.spend)} no perÃ­odo analisado.`,groups:groups.map(group=>({name:group.name,results:group.complete?num(group.results):'â€”',cpl:group.complete&&group.results?brl(group.spend/group.results):'â€”'})),summaryTitle:'Resumo do perÃ­odo',summaryText,goodTitle:'O que estÃ¡ bom',goodText:insights.good,improveTitle:'O que pode melhorar',improveText:insights.improve};
}
function pngReportEditKey(accountId,context=currentReportContext){return JSON.stringify([personalIdentity?.user?.id||'anonymous',accountId,localIso(context.from),localIso(context.to)])}
function pngReportEdit(accountId,displayName,row,groups,insights,context){const key=pngReportEditKey(accountId,context);if(!reportPngEdits.has(key))reportPngEdits.set(key,createPngReportEdit(accountId,displayName,row,groups,insights,context));const edit=reportPngEdits.get(key);edit.accountName=displayName;return edit}
...
function pngInsights(row,analysisRow,groups){const totalResults=groups.filter(group=>group.complete).reduce((sum,group)=>sum+group.results,0),overallCpl=totalResults?Number(row.spend)/totalResults:null,valid=groups.filter(group=>group.complete&&group.results>0),best=[...valid].sort((a,b)=>(a.spend/a.results)-(b.spend/b.results))[0],worst=[...valid].sort((a,b)=>(b.spend/b.results)-(a.spend/a.results))[0],zero=groups.find(group=>group.complete&&group.results===0&&group.spend>0),ctr=Number(analysisRow?.account?.ctr||0);let good=best?`${best.name} liderou a eficiÃªncia, com ${num(best.results)} resultados a ${brl(best.spend/best.results)} por resultado.`:totalResults?`A conta confirmou ${num(totalResults)} resultados com investimento total de ${brl(row.spend)}.`:'NÃ£o houve resultado confirmado suficiente para destacar um grupo.';if(ctr>=2)good+=` O CTR de ${ctr.toLocaleString('pt-BR',{maximumFractionDigits:2})}% reforÃ§a boa atraÃ§Ã£o dos anÃºncios.`;let improve;if(zero)improve=`${zero.name} consumiu ${brl(zero.spend)} sem resultado confirmado. Revisar antes de ampliar o investimento.`;else if(worst&&overallCpl&&worst.spend/worst.results>overallCpl*1.25)improve=`${worst.name} ficou acima do CPL mÃ©dio da conta. Revisar criativo, pÃºblico e distribuiÃ§Ã£o antes de escalar.`;else if(ctr>0&&ctr<1)improve=`CTR de ${ctr.toLocaleString('pt-BR',{maximumFractionDigits:2})}% indica baixa atraÃ§Ã£o. Priorizar novos criativos e mensagens de abertura.`;else improve='Manter a estrutura eficiente e testar variaÃ§Ãµes controladas de criativo, pÃºblico e posicionamento sem elevar o CPL.';return {good,improve,totalResults,overallCpl,ctr}}
...
function wrapCanvasText(ctx,text,x,y,maxWidth,lineHeight,maxLines=4){const words=String(text).split(/\s+/),lines=[];let line='';for(const word of words){const test=line?`${line} ${word}`:word;if(ctx.measureText(test).width>maxWidth&&line){lines.push(line);line=word;if(lines.length===maxLines-1)break}else line=test}if(line&&lines.length<maxLines)lines.push(line);if(words.length&&lines.length===maxLines){while(ctx.measureText(lines[maxLines-1]+'â€¦').width>maxWidth)lines[maxLines-1]=lines[maxLines-1].slice(0,-1);lines[maxLines-1]+='â€¦'}lines.forEach((value,index)=>ctx.fillText(value,x,y+index*lineHeight));return lines.length}
...
const drawReportIconBase=drawReportIcon;
drawReportIcon=function(ctx,type,cx,cy,color,scale=1){
  if(type!=='eye')return drawReportIconBase(ctx,type,cx,cy,color,scale);
  ...
  ctx.beginPath();ctx.moveTo(-24,0);ctx.quadraticCurveTo(0,-21,24,0);ctx.quadraticCurveTo(0,21,-24,0);ctx.stroke();
...
async function drawPngReport(accountId,displayName){const context=currentReportContext,row=context?.payload.accounts?.[accountId],analysisRow=context?.analysis?.accounts?.[accountId];if(!row)return;const canvas=document.querySelector('#pngReportCanvas'),ctx=canvas.getContext('2d'),groups=reportGroupsForRow(row),insights=pngInsights(row,analysisRow,groups),orange='#ff4b22',navy='#062b70',ink='#030316',muted='#566071',bg='#f8f9fc';ctx.clearRect(0,0,1600,900);ctx.fillStyle=bg;ctx.fillRect(0,0,1600,900);ctx.fillStyle=orange;ctx.fillRect(0,0,1600,8);ctx.fillStyle=ink;fitCanvasText(ctx,'RelatÃ³rio de Performance',930,54,'800');ctx.fillText('RelatÃ³rio de Performance',40,122);ctx.fillStyle=navy;const nameSize=fitCanvasText(ctx,displayName,900,29,'500');ctx.font=`500 ${nameSize}px Arial`;ctx.fillText(displayName,40,171);ctx.strokeStyle='#d7dce3';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(1232,52);ctx.lineTo(1232,204);ctx.stroke();try{const logo=await loadCanvasImage('hurtz-report-logo.png');ctx.drawImage(logo,1325,42,170,170)}catch{ctx.fillStyle=orange;ctx.font='800 28px Arial';ctx.fillText('HURTZ',1360,135)}const period=`${context.from.toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})} a ${context.to.toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})}`;ctx.fillStyle=orange;ctx.font='700 25px Arial';ctx.fillText('â–£',62,248);ctx.fillStyle=ink;ctx.font='700 16px Arial';ctx.fillText('PerÃ­odo:',101,245);ctx.font='400 15px Arial';ctx.fillText(period,178,245);const productNames=[...new Set(groups.map(group=>group.productLabel||group.name.split(' - ')[0]).filter(name=>name!=='NÃƒO IDENTIFICADO'))].join(' â€¢ ')||'NÃƒO IDENTIFICADO';ctx.fillStyle=orange;ctx.font='700 24px Arial';ctx.fillText('âŒ‚',430,248);ctx.fillStyle=ink;ctx.font='700 16px Arial';ctx.fillText('Campanhas:',468,245);ctx.fillStyle=orange;const productSize=fitCanvasText(ctx,productNames,550,16,'800');ctx.font=`800 ${productSize}px Arial`;drawReportProducts(ctx,productNames);const cardXs=[40,260,480],cardLabels=['RESULTADOS','CUSTO POR RESULTADO','VALOR GASTO'],cardValues=[num(insights.totalResults),insights.overallCpl==null?'â€”':brl(insights.overallCpl),brl(row.spend)];cardXs.forEach((x,index)=>{roundRect(ctx,x,276,204,374,10,'#fff','#edf0f4');ctx.fillStyle='#fff0eb';ctx.beginPath();ctx.arc(x+102,345,40,0,Math.PI*2);ctx.fill();ctx.fillStyle=orange;ctx.font='800 26px Arial';ctx.textAlign='center';ctx.fillText(index===0?'â—â—â—':index===1?'â—Ž':'â—‰',x+102,354);ctx.fillStyle=ink;ctx.font='800 14px Arial';ctx.fillText(cardLabels[index],x+102,430);ctx.strokeStyle='#ffaf97';ctx.beginPath();ctx.moveTo(x+64,458);ctx.lineTo(x+140,458);ctx.stroke();ctx.fillStyle=orange;const valueSize=fitCanvasText(ctx,cardValues[index],174,32,'800');ctx.font=`800 ${valueSize}px Arial`;ctx.fillText(cardValues[index],x+102,515);ctx.fillRect(x+40,596,30,34);ctx.fillRect(x+85,572,30,58);ctx.fillRect(x+130,545,30,85);ctx.textAlign='left'});ctx.fillStyle=navy;ctx.beginPath();ctx.arc(750,326,22,0,Math.PI*2);ctx.fill();ctx.fillStyle=navy;ctx.font='800 20px Arial';ctx.fillText('DESEMPENHO POR GRUPO DE CAMPANHA',783,334);roundRect(ctx,728,372,245,254,12,'#fff','#f0f2f5');ctx.fillStyle=navy;ctx.font='800 15px Arial';ctx.fillText('TOTAL CONFIRMADO',748,414);ctx.fillStyle=orange;ctx.font='800 52px Arial';ctx.fillText(num(insights.totalResults),748,480);ctx.font='700 16px Arial';ctx.fillText('resultados',748,507);ctx.strokeStyle='#ffac91';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(748,528);ctx.lineTo(950,528);ctx.stroke();ctx.fillStyle=ink;ctx.font='400 15px Arial';wrapCanvasText(ctx,`Investimento total de ${brl(row.spend)} no perÃ­odo analisado.`,748,555,190,20,3);const gx=1005,gy=385,gw=545,gh=230,columns=groups.length>4?2:1,rows=Math.max(1,Math.ceil(groups.length/columns)),cellW=gw/columns,cellH=gh/rows,labelSize=Math.max(11,Math.min(17,19-groups.length));groups.forEach((group,index)=>{const col=Math.floor(index/rows),rowIndex=index%rows,x=gx+col*cellW,y=gy+rowIndex*cellH;ctx.fillStyle=ink;const size=fitCanvasText(ctx,group.name,cellW-145,labelSize,'800');ctx.font=`800 ${size}px Arial`;ctx.fillText(group.name,x,y+30);ctx.fillStyle=navy;ctx.font='400 11px Arial';ctx.fillText('RESULTADOS',x+cellW-135,y+10);ctx.fillStyle=orange;ctx.font='800 18px Arial';ctx.fillText(group.complete?num(group.results):'â€”',x+cellW-135,y+35);ctx.fillStyle=navy;ctx.font='400 11px Arial';ctx.fillText('CPL',x+cellW-72,y+10);ctx.fillStyle=orange;const cpl=group.complete&&group.results?brl(group.spend/group.results):'â€”';fitCanvasText(ctx,cpl,70,18,'800');ctx.fillText(cpl,x+cellW-72,y+35)});const panels=[{x:40,w:590,title:'Resumo do perÃ­odo',text:`${displayName} gerou ${num(insights.totalResults)} resultados confirmados, com investimento total de ${brl(row.spend)} entre ${context.from.toLocaleDateString('pt-BR')} e ${context.to.toLocaleDateString('pt-BR')}.`},{x:646,w:450,title:'O que estÃ¡ bom',text:insights.good},{x:1112,w:448,title:'O que pode melhorar',text:insights.improve}];panels.forEach((panel,index)=>{roundRect(ctx,panel.x,670,panel.w,172,10,'#fff','#dde3ea');ctx.fillStyle=index===0?navy:'#fff0eb';ctx.beginPath();ctx.arc(panel.x+69,755,40,0,Math.PI*2);ctx.fill();ctx.fillStyle=ink;ctx.font='800 16px Arial';ctx.fillText(panel.title,panel.x+134,720);ctx.fillStyle=orange;ctx.fillRect(panel.x+134,735,42,2);ctx.fillStyle=ink;ctx.font='700 14px Arial';wrapCanvasText(ctx,panel.text,panel.x+134,770,panel.w-160,19,4)});ctx.strokeStyle='#ffb49d';ctx.beginPath();ctx.moveTo(135,875);ctx.lineTo(648,875);ctx.moveTo(952,875);ctx.lineTo(1465,875);ctx.stroke();ctx.fillStyle='#414855';ctx.font='400 13px Arial';ctx.fillText('RelatÃ³rio desenvolvido por',686,880);ctx.fillStyle=orange;ctx.font='700 13px Arial';ctx.fillText('Hurtz',862,880)}
...
drawPngReport=async function(accountId,displayName){
  ...
  // Substitui os caracteres provisÃ³rios do cabeÃ§alho por Ã­cones vetoriais.
  ...
  // Ãcones dos indicadores principais, seguindo a linguagem da referÃªncia.
  ...
  // O CPL usa um grÃ¡fico circular, enquanto volume e investimento usam barras.
  ctx.fillStyle='#fff';
  ...
  // Organiza grupos, leads e CPL em uma grade comparÃ¡vel linha a linha.
  ...
  ctx.fillText('RESULTADOS',gridX+campaignCol+leadsCol/2,gridY+23);
  ...
  if(gridGroups.length){
    ...
    gridGroups.forEach((group,index)=>{
      ...
      ctx.fillText(group.complete?num(group.results):'â€”',gridX+campaignCol+leadsCol/2,centerY+5);
      const groupCpl=group.complete&&group.results?brl(group.spend/group.results):'â€”';
    ...
    ctx.fillText('Nenhuma campanha com gasto no perÃ­odo.',gridX+gridW/2,gridY+headerH+106);
    ctx.textAlign='left';
  }
  ...
  // Centraliza o crÃ©dito e posiciona a marca imediatamente apÃ³s o texto.
  const footerLead='RelatÃ³rio desenvolvido por',footerBrand='Hurtz';
...
function reportFittedName(ctx,value,width){const text=String(value??'');if(ctx.measureText(text).width<=width)return text;const chars=Array.from(text);while(chars.length&&ctx.measureText(chars.join('')+'â€¦').width>width)chars.pop();return chars.length?chars.join('')+'â€¦':''}
function drawReportProducts(ctx,value){
  ctx.save();ctx.fillStyle='#f8f9fc';ctx.fillRect(578,213,ctx.canvas.width-578,48);
  ctx.textAlign='left';ctx.fillStyle='#ff4b22';const size=fitCanvasText(ctx,value,625,16,'800');ctx.font=`800 ${size}px Arial`;
  ctx.fillText(reportFittedName(ctx,value,625),578,245,625);ctx.restore();
}
...
function drawEditableReportBlocks(ctx,edit){
  ...
  ctx.fillStyle=navy;ctx.beginPath();ctx.arc(groupX+22,326,22,0,Math.PI*2);ctx.fill();drawReportSpecificIcon(ctx,edit.groupIcon||'layers',groupX+22,326,'#fff',.52);
...
drawPngReport=async function(accountId,displayName){
  ...
  ctx.fillStyle=ink;ctx.font='700 16px Arial';ctx.fillText('PerÃ­odo:',101,245);ctx.font='400 15px Arial';ctx.fillText(edit.period,178,245);
  ctx.font='700 16px Arial';ctx.fillText(edit.campaignsLabel,468,245);drawReportProducts(ctx,edit.products);

  drawEditableReportBlocks(ctx,edit);

  const panels=Array.isArray(edit.lowerCards)?edit.lowerCards:[{title:edit.summaryTitle,text:edit.summaryText,icon:'document'},{title:edit.goodTitle,text:edit.goodText,icon:'clipboard'},{title:edit.improveTitle,text:edit.improveText,icon:'trend'}];
  ctx.fillStyle=bg;ctx.fillRect(0,650,1600,200);
  const gap=16,panelW=(1520-gap*Math.max(0,panels.length-1))/Math.max(1,panels.length);
  panels.forEach((panel,index)=>{const x=40+index*(panelW+gap),iconBg=index===0?navy:'#fff0eb',iconColor=index===0?'#fff':navy;
    roundRect(ctx,x,670,panelW,172,10,'#fff','#dde3ea');ctx.fillStyle=iconBg;ctx.beginPath();ctx.arc(x+Math.min(69,panelW*.16),755,Math.min(40,panelW*.095),0,Math.PI*2);ctx.fill();
    drawReportIcon(ctx,panel.icon||'document',x+Math.min(69,panelW*.16),755,iconColor,Math.min(.9,panelW/450));
    const textX=x+Math.min(134,panelW*.30),textW=panelW-Math.min(160,panelW*.36);drawReportCellText(ctx,panel.title,textX,720,textW,16,ink);
    ctx.fillStyle=orange;ctx.fillRect(textX,735,42,2);ctx.fillStyle=ink;ctx.font='700 14px Arial';wrapCanvasText(ctx,panel.text,textX,770,textW,19,4);
  });
};
...
function renderReport(payload,from,to,selectedIds){const rows=selectedIds.map(id=>payload.accounts?.[id]).filter(Boolean),failures=[];rows.forEach(row=>{const campaigns=row.campaigns||[],sum=campaigns.reduce((total,campaign)=>total+Number(campaign.spend||0),0);if(!row.reconciled||Math.abs(Number(row.spend||0)-sum)>=.01)failures.push(row.name||row.id)});if(rows.length!==selectedIds.length)failures.push('Conta sem retorno da API');if(failures.length)throw new Error(`RelatÃ³rio bloqueado: auditoria incompleta em ${failures.join(', ')}.`);const accountHtml=rows.map(row=>{const groups=reportGroupsForRow(row);return `<section class="report-account" data-report-account="${escapeHtml(row.id)}"><div class="report-account-head"><div><span>CONTA DE ANÃšNCIO</span><h3>${escapeHtml(row.name||row.id)}</h3></div><div class="report-account-head-actions"><b>${row.result_reconciled===false?'Gasto auditado Â· conferir atribuiÃ§Ã£o de resultados':'Auditada com a Meta'}</b><button class="report-view-ads-button" type="button" data-view-report-ads="${escapeHtml(row.id)}">Visualizar anÃºncios</button><button type="button" data-create-png="${row.id}">Criar relatÃ³rio em PNG</button></div></div><div class="report-groups">${groups.map(group=>`<article><strong>${escapeHtml(group.name)}</strong><div><span>RESULTADOS</span><b>${group.complete?num(group.results):'Sem dados de lead'}</b></div><div><span>Custo por resultado</span><b>${group.complete&&group.results?brl(group.spend/group.results):'â€”'}</b></div>${!group.complete?'<small>Coleta de resultado incompleta; nenhum valor foi estimado.</small>':''}</article>`).join('')||'<p>A Meta nÃ£o retornou campanhas para esta conta e perÃ­odo.</p>'}</div><footer><span>Valor gasto</span><strong>${brl(row.spend)}</strong></footer></section>`}).join('');return `<div class="report-document"><header><div><p>HURTZ â€¢ PERFORMANCE</p><h1>RelatÃ³rio Meta Ads</h1><span>PerÃ­odo: ${from.toLocaleDateString('pt-BR')} atÃ© ${to.toLocaleDateString('pt-BR')}</span></div><button type="button" id="printReportButton">Baixar / imprimir PDF</button></header>${accountHtml}<div class="report-audit-footer">Dados da API Meta â€¢ ${rows.length} conta(s) reconciliada(s) â€¢ valores nÃ£o estimados</div></div>`}
async function createReport(event){const selectedIds=[...document.querySelectorAll('#reportAccountOptions input:checked')].map(input=>input.value),from=parseDate(document.querySelector('#reportDateFrom').value),to=parseDate(document.querySelector('#reportDateTo').value),status=document.querySelector('#reportStatus'),output=document.querySelector('#reportOutput');if(!selectedIds.length)return alert('Selecione pelo menos uma conta.');if(from>to)return alert('A data inicial deve ser anterior Ã  data final.');showReportLoader(true);event.currentTarget.disabled=true;status.textContent='Consultando e auditando campanhas...';try{const query=`from=${iso(from)}&to=${iso(to)}&accounts=${encodeURIComponent(selectedIds.join(','))}`,[spendResponse,analysisResponse]=await Promise.all([fetch(`/api/meta-spend?${query}`),fetch(`/api/meta-analysis?${query}`)]),payload=await spendResponse.json(),analysis=await analysisResponse.json();if(!spendResponse.ok)throw new Error(payload.error||'Falha ao consultar a Meta.');if(!analysisResponse.ok)throw new Error(analysis.error||'Falha ao consultar mÃ©tricas para o relatÃ³rio PNG.');const analysisFailures=selectedIds.filter(id=>!analysis.accounts?.[id]?.reconciled);if(analysisFailures.length)throw new Error('RelatÃ³rio PNG bloqueado: mÃ©tricas analÃ­ticas ainda nÃ£o reconciliadas.');currentReportContext={payload,analysis,from,to,selectedIds};output.innerHTML=renderReport(payload,from,to,selectedIds);output.hidden=false;status.textContent=`RelatÃ³rio pronto: ${selectedIds.length} conta(s), perÃ­odo ${from.toLocaleDateString('pt-BR')} a ${to.toLocaleDateString('pt-BR')}.`;document.querySelector('#printReportButton').onclick=()=>window.print();output.querySelectorAll('[data-create-png]').forEach(button=>button.onclick=()=>openPngReport(button.dataset.createPng))}catch(error){output.hidden=true;status.textContent=error.message}finally{showReportLoader(false);event.currentTarget.disabled=false}}
document.querySelector('#analysisAccountSelect').onchange=event=>{analysisSelectedAccount=event.target.value;document.querySelector('#analysisAccountsButton').hidden=analysisSelectedAccount!=='all';document.querySelector('#analysisAccountPanel').hidden=true;loadDetailedAnalysis()};document.querySelectorAll('[data-analysis-range]').forEach(button=>button.onclick=event=>setAnalysisRange(button.dataset.analysisRange,true,event.currentTarget));document.querySelector('#applyAnalysisFilter').onclick=event=>{const from=parseDate(document.querySelector('#analysisDateFrom').value),to=parseDate(document.querySelector('#analysisDateTo').value);if(from>to)return alert('A data inicial nÃ£o pode ser posterior Ã  data final.');analysisFrom=from;analysisTo=to;document.querySelectorAll('[data-analysis-range]').forEach(button=>button.classList.remove('active'));return loadDetailedAnalysis(event.currentTarget)};
...
function setReportLoaderProgress(done,total,detail='Conciliando campanhas com a Meta...'){
  ...
  if(title)title.textContent=`${done} de ${total} relatÃ³rios auditados`;
...
async function fetchReportBatch(accountIds,from,to){
  ...
  try{
    ...
    if(!analysisResponse.ok)throw new Error(analysis.error||'Falha ao consultar mÃ©tricas analÃ­ticas.');
    if([spend,analysis].some(result=>result.since!==localIso(from)||result.until!==localIso(to)))throw new Error('A consulta retornou um perÃ­odo diferente do selecionado. Gere o relatÃ³rio novamente.');
    const missing=accountIds.filter(id=>!spend.accounts?.[id]||!analysis.accounts?.[id]);
    if(missing.length)throw new Error(`A Meta nÃ£o retornou ${missing.length} conta(s) deste lote.`);
    ...
    if(error.name==='AbortError')throw new Error('A consulta excedeu o limite de tempo. Tente novamente; os lotes concluÃ­dos permanecem identificados no contador.');
    throw error;
...
async function createReportInBatches(event){
  ...
  if(from>to)return alert('A data inicial deve ser anterior Ã  data final.');
  ...
  try{
    for(let index=0;index<selectedIds.length;index+=batchSize){
      ...
      status.textContent=`Auditando lote ${number} de ${totalBatches} (${completed} de ${selectedIds.length} contas concluÃ­das)...`;
      setReportLoaderProgress(completed,selectedIds.length,`Consultando lote ${number} de ${totalBatches} â€¢ ${batch.length} conta(s)`);
      ...
      setReportLoaderProgress(completed,selectedIds.length,completed===selectedIds.length?'Montando os relatÃ³rios finais...':`Lote ${number} auditado. Preparando o prÃ³ximo...`);
    ...
    status.textContent=`RelatÃ³rios prontos: ${selectedIds.length} conta(s) auditadas, perÃ­odo ${from.toLocaleDateString('pt-BR')} a ${to.toLocaleDateString('pt-BR')}.`;
    ...
    output.hidden=true;status.textContent=`CriaÃ§Ã£o interrompida apÃ³s ${completed} de ${selectedIds.length} contas: ${error.message}`;
...
function renderReportRunSummary(total,ready,failures){
  ...
  root.hidden=false;
  root.innerHTML=`<span>Selecionadas <b>${total}</b></span><span class="success">RelatÃ³rios prontos <b>${ready}</b></span><span class="failed">NÃ£o passaram <b>${failures.length}</b></span>${failures.length?`<details><summary>Ver contas que nÃ£o passaram</summary><ul>${failures.map(item=>`<li><b>${escapeHtml(item.name)}</b>: ${escapeHtml(item.reason)}</li>`).join('')}</ul></details>`:''}`;
}
...
function setReportLoaderOutcome(ready,failed,processed,total,detail){
  const loader=document.querySelector('#reportLoader'),title=loader?.querySelector('strong'),description=loader?.querySelector('span');
  if(title)title.textContent=`${ready} prontos â€¢ ${failed} nÃ£o passaram`;
  if(description)description.textContent=`${processed} de ${total} contas verificadas â€¢ ${detail}`;
}
async function createReportResilient(event){
  ...
  try{
    ...
    const eligible=[];
    selectedIds.forEach(id=>{if(allowed.has(id))eligible.push(id);else{failures.push({id,name:reportAccountName(id),reason:'Conta ainda nÃ£o habilitada na lista de monitoramento auditado.'});processed++}});
    ...
    for(let index=0;index<eligible.length;index+=batchSize){
      ...
      status.textContent=`Auditando lote ${number} de ${totalBatches} â€¢ ${successfulIds.length} relatÃ³rio(s) pronto(s) â€¢ ${failures.length} nÃ£o passaram.`;
      ...
      try{
        ...
        for(const id of batch){
          ...
          processed++;renderReportRunSummary(selectedIds.length,successfulIds.length,failures);setReportLoaderOutcome(successfulIds.length,failures.length,processed,selectedIds.length,'validaÃ§Ã£o individual em andamento');
          await new Promise(resolve=>setTimeout(resolve,250));
        }
      }
      renderReportRunSummary(selectedIds.length,successfulIds.length,failures);setReportLoaderOutcome(successfulIds.length,failures.length,processed,selectedIds.length,processed===selectedIds.length?'montando relatÃ³rios finais':'preparando prÃ³ximo lote');
      if(processed<selectedIds.length)await new Promise(resolve=>setTimeout(resolve,450));
    }
    const auditedIds=successfulIds.filter(id=>{const row=payload.accounts[id],campaignSum=(row?.campaigns||[]).reduce((sum,campaign)=>sum+Number(campaign.spend||0),0),valid=row?.reconciled&&analysis.accounts[id]?.reconciled&&Math.abs(Number(row.spend||0)-campaignSum)<.01;if(!valid)failures.push({id,name:reportAccountName(id),reason:'A reconciliaÃ§Ã£o final da conta nÃ£o coincidiu com a soma das campanhas.'});return valid});
    renderReportRunSummary(selectedIds.length,auditedIds.length,failures);
    if(!auditedIds.length){output.hidden=true;status.textContent=`Nenhum relatÃ³rio foi liberado. ${failures.length} conta(s) nÃ£o passaram pela auditoria.`;return}
    ...
    status.textContent=`Processo concluÃ­do: ${auditedIds.length} relatÃ³rio(s) pronto(s) e ${failures.length} conta(s) nÃ£o passaram.`;
    document.querySelector('#printReportButton').onclick=()=>window.print();
    output.querySelectorAll('[data-create-png]').forEach(reportButton=>reportButton.onclick=()=>openPngReport(reportButton.dataset.createPng));
  }catch(error){status.textContent=`Falha geral antes da conclusÃ£o: ${error.message}`;renderReportRunSummary(selectedIds.length,successfulIds.length,failures)}
  finally{showReportLoader(false);button.disabled=false}
}
...
renderReport=function(...args){return renderReportWithoutAuditTooltip(...args).replaceAll('<b>Auditada com a Meta</b>','<b class="report-audit-badge" tabindex="0" aria-label="Auditoria concluÃ­da com a Meta" data-tooltip="Auditoria concluÃ­da: o valor usado da conta foi comparado com a soma das campanhas retornadas pela API da Meta. Se houver divergÃªncia, o relatÃ³rio Ã© bloqueado.">Auditada com a Meta <span aria-hidden="true">â“˜</span></b>')};
...
function safeReportFileName(value){const clean=String(value||'Conta de anÃºncio').replace(/[<>:"/\\|?*\u0000-\u001F]/g,'-').replace(/[. ]+$/g,'').trim();return clean||'Conta de anÃºncio'}
function canvasAsPngBlob(){return new Promise((resolve,reject)=>document.querySelector('#pngReportCanvas').toBlob(blob=>blob?resolve(blob):reject(new Error('NÃ£o foi possÃ­vel gerar o PNG.')),'image/png'))}
...
openPngReport=async function(accountId){pngReportMode='single';document.querySelector('#bulkPngEditor').hidden=true;document.querySelector('#pngReportModalTitle').textContent='RelatÃ³rio de Performance';document.querySelector('#downloadPngReport').textContent='Baixar PNG';await openPngReportWithDefaultName(accountId);const custom=reportDisplayNameOverrides.get(accountId);if(custom){document.querySelector('#pngReportName').value=custom;await drawPngReport(accountId,custom)}};
function renderBulkPngNames(){const root=document.querySelector('#bulkPngNames'),ids=currentReportContext?.selectedIds||[],counter=document.querySelector('#bulkPngAccountCount');if(counter)counter.textContent=`${ids.length} ${ids.length===1?'conta incluÃ­da':'contas incluÃ­das'} â€¢ selecione uma conta para editar todos os seus dados.`;root.innerHTML=ids.map((id,index)=>{const row=currentReportContext.payload.accounts[id],displayName=reportDisplayNameOverrides.get(id)||row?.name||id;return `<article class="bulk-png-name-row ${id===currentPngAccountId?'active':''}" data-bulk-png-row="${id}"><span class="bulk-png-index">${index+1}</span><label><small>Nome exibido no PNG</small><input maxlength="70" value="${escapeHtml(displayName)}" data-bulk-png-name="${id}" aria-label="Nome exibido para ${escapeHtml(row?.name||id)}"></label><button type="button" data-preview-png="${id}">${id===currentPngAccountId?'Editando agora':'Editar dados'}</button></article>`}).join('')}
...
async function openBulkPngEditor(){if(!currentReportContext?.selectedIds?.length)return alert('Crie os relatÃ³rios auditados antes de preparar o ZIP.');pngReportMode='bulk';document.querySelector('#bulkPngEditor').hidden=false;document.querySelector('#pngReportModalTitle').textContent='Preparar relatÃ³rios em PNG';document.querySelector('#downloadPngReport').textContent='Baixar todos em ZIP';document.querySelector('#pngReportModal').classList.add('open');document.querySelector('#pngReportModal').setAttribute('aria-hidden','false');await previewBulkPng(currentReportContext.selectedIds[0])}
...
document.querySelector('#bulkPngNames').addEventListener('click',event=>{const button=event.target.closest('[data-preview-png]'),row=event.target.closest('[data-bulk-png-row]');if(button||row&&!event.target.matches('input'))previewBulkPng(button?.dataset.previewPng||row.dataset.bulkPngRow)});
document.querySelector('#updatePngReport').onclick=async()=>{const displayName=document.querySelector('#pngReportName').value.trim()||'Conta de anÃºncio';reportDisplayNameOverrides.set(currentPngAccountId,displayName);if(pngReportMode==='bulk')renderBulkPngNames();await drawPngReport(currentPngAccountId,displayName)};
document.querySelector('#downloadPngReport').onclick=async event=>{if(pngReportMode==='bulk')return downloadAllPngReports(event.currentTarget);const displayName=document.querySelector('#pngReportName').value.trim()||'Conta de anÃºncio',row=currentReportContext?.payload.accounts?.[currentPngAccountId];reportDisplayNameOverrides.set(currentPngAccountId,displayName);await drawPngReport(currentPngAccountId,displayName);downloadBlob(await canvasAsPngBlob(),`${safeReportFileName(row?.name||currentPngAccountId)}.png`)};
async function downloadAllPngReports(button){
  if(!currentReportContext?.selectedIds?.length)return alert('Crie os relatÃ³rios auditados antes de baixar o ZIP.');
  try{await loadOptionalScript('JSZip','https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js')}catch(error){return alert(error.message)}
  ...
  try{
    for(let index=0;index<ids.length;index++){
      ...
      setReportLoaderProgress(index+1,ids.length,index+1===ids.length?'Compactando os arquivos...':'Preparando o prÃ³ximo PNG...');await new Promise(resolve=>setTimeout(resolve,35));
    }
    const blob=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}},metadata=>setReportLoaderProgress(ids.length,ids.length,`Compactando ZIP â€¢ ${Math.round(metadata.percent)}%`));
    downloadBlob(blob,`RelatÃ³rios Meta Ads - ${currentReportContext.from.toLocaleDateString('pt-BR').replaceAll('/','-')} a ${currentReportContext.to.toLocaleDateString('pt-BR').replaceAll('/','-')}.zip`);
  }catch(error){alert(`NÃ£o foi possÃ­vel gerar o ZIP: ${error.message}`)}finally{showReportLoader(false);button.disabled=false}
}
...
function pngMetricIconControl(number,value,field,index,bottom=false){
  const icons=[['people','Pessoas'],['target','Alvo'],['money','Dinheiro'],['trend','Crescimento'],['eye','Olho'],['bank','Banco'],['document','Documento'],['clipboard','Prancheta'],['calendar','CalendÃ¡rio'],['campaign','Campanha']];
  ...
  return `<label class="png-edit-control wide"><span>Ãcone ${bottom?'inferior':'superior'} da mÃ©trica ${number}</span><select ${attributes}>${icons.map(([key,label])=>`<option value="${key}" ${key===value?'selected':''}>${label}</option>`).join('')}</select></label>`;
}
function pngLowerCardControls(card,index){
  const icons=[['document','Documento'],['clipboard','Prancheta'],['trend','Crescimento'],['eye','Olho'],['people','Pessoas'],['target','Alvo'],['money','Dinheiro'],['calendar','CalendÃ¡rio'],['campaign','Campanha']];
  return `<div class="png-lower-card-editor"><strong>Card ${index+1}</strong><label class="png-edit-control"><span>TÃ­tulo</span><input data-png-lower-card-index="${index}" data-png-lower-card-field="title" value="${escapeHtml(card.title)}"></label><label class="png-edit-control wide"><span>Texto</span><textarea rows="3" data-png-lower-card-index="${index}" data-png-lower-card-field="text">${escapeHtml(card.text)}</textarea></label><label class="png-edit-control"><span>Ãcone</span><select data-png-lower-card-index="${index}" data-png-lower-card-field="icon">${icons.map(([key,label])=>`<option value="${key}" ${key===(card.icon||'document')?'selected':''}>${label}</option>`).join('')}</select></label><button type="button" class="png-group-add" data-remove-png-lower-card="${index}">Remover card ${index+1}</button></div>`;
}
...
function renderPngReportFields(accountId=currentPngAccountId){
  ...
  root.innerHTML=`
    <fieldset><legend>CabeÃ§alho</legend><div class="png-edit-grid">
      ${pngEditorControl('TÃ­tulo do relatÃ³rio','reportTitle',edit.reportTitle)}
      ${pngEditorControl('Nome da conta','accountName',edit.accountName)}
      ${pngEditorControl('PerÃ­odo exibido','period',edit.period)}
      ${pngEditorControl('RÃ³tulo de campanhas','campaignsLabel',edit.campaignsLabel)}
      ${pngEditorControl('Produtos / campanhas reconhecidas','products',edit.products,{wide:true})}
    </div></fieldset>
    <fieldset><legend>MÃ©tricas principais</legend><div class="png-edit-grid metrics">
      ${pngEditorControl('Nome da mÃ©trica 1','leadsLabel',edit.leadsLabel)}${pngEditorControl('Valor da mÃ©trica 1','leadsValue',edit.leadsValue)}${pngMetricIconControl(1,edit.leadsIcon||'people','leadsIcon')}${pngMetricIconControl(1,edit.leadsBottomIcon||'bars','leadsBottomIcon',null,true)}
      ${pngEditorControl('Nome da mÃ©trica 2','cplLabel',edit.cplLabel)}${pngEditorControl('Valor da mÃ©trica 2','cplValue',edit.cplValue)}${pngMetricIconControl(2,edit.cplIcon||'target','cplIcon')}${pngMetricIconControl(2,edit.cplBottomIcon||'donut','cplBottomIcon',null,true)}
      ${pngEditorControl('Nome da mÃ©trica 3','spendLabel',edit.spendLabel)}${pngEditorControl('Valor da mÃ©trica 3','spendValue',edit.spendValue)}${pngMetricIconControl(3,edit.spendIcon||'money','spendIcon')}${pngMetricIconControl(3,edit.spendBottomIcon||'bars','spendBottomIcon',null,true)}
      ${(edit.extraMetrics||[]).map((metric,index)=>`<label class="png-edit-control"><span>Nome da mÃ©trica ${index+4}</span><input data-png-metric-index="${index}" data-png-metric-field="label" value="${escapeHtml(metric.label)}"></label><label class="png-edit-control"><span>Valor da mÃ©trica ${index+4}</span><input data-png-metric-index="${index}" data-png-metric-field="value" value="${escapeHtml(metric.value)}"></label>${pngMetricIconControl(index+4,metric.icon||'trend',null,index)}${pngMetricIconControl(index+4,metric.bottomIcon||'bars',null,index,true)}<button type="button" class="png-group-add" data-remove-png-metric="${index}">Remover mÃ©trica ${index+4}</button>`).join('')}
      <button type="button" class="png-group-add" data-add-png-metric ${(edit.extraMetrics||[]).length>=3?'disabled':''}>+ Adicionar mÃ©trica</button>
    </div></fieldset>
    <fieldset><legend>Card â€œTotal confirmadoâ€</legend><div class="png-edit-grid">
      ${pngEditorControl('TÃ­tulo do card','confirmedLabel',edit.confirmedLabel)}
      ${pngEditorControl('NÃºmero principal','confirmedValue',edit.confirmedValue)}
      ${pngEditorControl('Unidade abaixo do nÃºmero','confirmedUnit',edit.confirmedUnit)}
      ${pngEditorControl('Texto descritivo','investmentText',edit.investmentText,{wide:true,multiline:true})}
    </div></fieldset>
    <fieldset><legend>Layout do relatÃ³rio</legend><label class="png-edit-control"><span>Largura do bloco de campanhas: <output id="pngGroupWidthValue">${pngReportLayout(edit).groupPercent}%</output></span><input type="range" min="35" max="60" step="1" value="${pngReportLayout(edit).groupPercent}" data-png-field="groupWidth" aria-label="Largura do bloco de campanhas"></label></fieldset>
    <fieldset><legend>Desempenho por grupo de campanha</legend><div class="png-edit-grid">
      ${pngEditorControl('TÃ­tulo do bloco','groupTitle',edit.groupTitle,{wide:true})}
      <label class="png-edit-control"><span>Ãcone do bloco</span><select data-png-field="groupIcon"><option value="layers" ${(edit.groupIcon||'layers')==='layers'?'selected':''}>Camadas</option><option value="eye" ${edit.groupIcon==='eye'?'selected':''}>Olho</option><option value="campaign" ${edit.groupIcon==='campaign'?'selected':''}>Campanha</option><option value="target" ${edit.groupIcon==='target'?'selected':''}>Alvo</option></select></label>
    </div><div class="png-group-editor"><div class="png-group-editor-header"><span>Nome reconhecido / nÃ£o reconhecido</span><span>Leads</span><span>CPL</span><span class="png-group-action-title">AÃ§Ã£o</span></div>
      ${(edit.groups||[]).map((group,index)=>`<div class="png-group-edit-row"><input value="${escapeHtml(group.name)}" data-png-group-index="${index}" data-png-group-field="name" aria-label="Nome do grupo ${index+1}"><input value="${escapeHtml(group.results)}" data-png-group-index="${index}" data-png-group-field="results" aria-label="Leads do grupo ${index+1}"><input value="${escapeHtml(group.cpl)}" data-png-group-index="${index}" data-png-group-field="cpl" aria-label="CPL do grupo ${index+1}"><button class="png-group-remove" type="button" data-remove-png-group="${index}" title="Apagar esta campanha do relatÃ³rio" aria-label="Apagar ${escapeHtml(group.name||`campanha ${index+1}`)} do relatÃ³rio"><svg viewBox="0 0 20 20" aria-hidden="true"><path d="M4 6h12M8 3h4l1 3H7l1-3Zm-2 3 1 11h6l1-11M9 9v5m2-5v5"/></svg></button></div>`).join('')||'<p class="png-edit-empty">Nenhuma campanha serÃ¡ exibida. Use â€œRestaurar dados auditadosâ€ para recuperar as linhas.</p>'}
      <button class="png-group-add" type="button" data-add-png-group><span>+</span> Adicionar campanha</button>
    </div></fieldset>
    <fieldset><legend>Cards inferiores</legend><div class="png-lower-cards">
      ${(edit.lowerCards||[]).map(pngLowerCardControls).join('')}
      <button type="button" class="png-group-add" data-add-png-lower-card ${(edit.lowerCards||[]).length>=4?'disabled':''}>+ Adicionar card inferior</button>
    </div></fieldset>`;
}
...
document.querySelector('#pngReportFields').addEventListener('input',event=>{
  ...
  const lowerCardIndex=Number(event.target.dataset.pngLowerCardIndex),lowerCardField=event.target.dataset.pngLowerCardField;if(lowerCardField&&edit.lowerCards?.[lowerCardIndex])edit.lowerCards[lowerCardIndex][lowerCardField]=event.target.value;
...
document.querySelector('#pngReportFields').addEventListener('click',event=>{
  ...
  if(add){if(edit.extraMetrics.length>=3)return;edit.extraMetrics.push({label:'NOVA MÃ‰TRICA',value:'â€”'})}
...
document.querySelector('#pngReportFields').addEventListener('click',event=>{
  const add=event.target.closest('[data-add-png-lower-card]'),remove=event.target.closest('[data-remove-png-lower-card]');if(!add&&!remove)return;
  const edit=currentPngEdit();if(!edit)return;edit.lowerCards=edit.lowerCards||[];
  if(add){if(edit.lowerCards.length>=4)return;edit.lowerCards.push({title:'Novo card',text:'Escreva aqui a informaÃ§Ã£o que deseja destacar.',icon:'document'});}else edit.lowerCards.splice(Number(remove.dataset.removePngLowerCard),1);
  renderPngReportFields();schedulePngEditorRedraw();
  document.querySelector(add?`[data-png-lower-card-index="${edit.lowerCards.length-1}"][data-png-lower-card-field="title"]`:'[data-add-png-lower-card]')?.focus();
});
document.querySelector('#resetPngReportEdits').onclick=async()=>{
  ...
  reportPngEdits.delete(pngReportEditKey(currentPngAccountId));reportDisplayNameOverrides.delete(currentPngAccountId);const displayName=row.name||row.id;document.querySelector('#pngReportName').value=displayName;renderPngReportFields();if(pngReportMode==='bulk')renderBulkPngNames();await drawPngReport(currentPngAccountId,displayName);
};
...
let alertAuthMode='login',recoveryAccessToken='';
const alertAuthCopy={login:['Entrar no Traffic pocket','Entre com seu e-mail para continuar a consulta.'],signup:['Criar sua conta','Use um e-mail vÃ¡lido. VocÃª precisarÃ¡ confirmÃ¡-lo antes de entrar.'],recover:['Recuperar senha','Enviaremos um link para o e-mail cadastrado.'],reset:['Definir nova senha','Escolha uma nova senha com pelo menos 8 caracteres.']};
function setAlertAuthStatus(message='',success=false){const status=document.querySelector('#alertLoginStatus');status.textContent=message;status.classList.toggle('success',success)}
function setAlertAuthMode(mode,message='',success=false){
  alertAuthMode=mode;document.querySelectorAll('[data-auth-panel]').forEach(panel=>panel.hidden=panel.dataset.authPanel!==mode);
  const copy=alertAuthCopy[mode]||alertAuthCopy.login;document.querySelector('#alertAuthTitle').textContent=copy[0];document.querySelector('#alertAuthDescription').textContent=copy[1];setAlertAuthStatus(message,success);
  const focusId={login:'alertLoginUser',signup:'alertSignupEmail',recover:'alertRecoverEmail',reset:'alertResetPassword'}[mode];setTimeout(()=>document.querySelector(`#${focusId}`)?.focus(),50);
}
...
async function alertAuthPost(action,body){let response;try{response=await browserFetch(`${ALERT_API_BASE}/api/auth/${action}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})}catch{throw new Error('A API de acesso nÃ£o respondeu.')}const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||'NÃ£o foi possÃ­vel concluir esta operaÃ§Ã£o.');return payload}
async function alertFetchJson(url,options={}){const token=userStorage.getItem(ALERT_SESSION_KEY);if(ALERT_API_BASE&&!token){showAlertLogin();throw new Error('FaÃ§a login para acessar o monitoramento.')}let response;try{response=await fetch(`${ALERT_API_BASE}${url}`,{...options,headers:{...(options.headers||{}),...(token?{Authorization:`Bearer ${token}`}:{})}})}catch{throw new Error('A API segura da VPS nÃ£o respondeu.')}const type=response.headers.get('content-type')||'';if(response.status===401){userStorage.removeItem(ALERT_SESSION_KEY);showAlertLogin('Sua sessÃ£o expirou. Entre novamente.');throw new Error('SessÃ£o expirada.')}if(!type.includes('application/json'))throw new Error('A API de monitoramento respondeu em formato invÃ¡lido.');const payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha na consulta.');return payload}
function alertKindLabel(kind){return ({daily_limit:'Limite diÃ¡rio',spend_velocity:'Ritmo de gasto',opportunity:'Oportunidade',balance_report:'RelatÃ³rio geral',performance_report:'Resumo de 3 dias',test:'Teste',technical:'Monitoramento tÃ©cnico'})[kind]||'NotificaÃ§Ã£o'}
...
function renderAlertHistory(payload){
  if(payload.monitor_error)document.querySelector('#alertFormStatus').textContent=payload.monitor_error;
  ...
  document.querySelector('#alertStats').innerHTML=[['Alertas hoje',todayCount],['Entregues',delivered],['Falhas',failed],['Ãšltima execuÃ§Ã£o',payload.last_run?new Date(payload.last_run).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):'â€”']].map(([label,value])=>`<article class="alert-stat"><span>${label}</span><strong>${value}</strong></article>`).join('');
  ...
  document.querySelector('#alertBalanceReport').checked=config.balance_report_enabled===true;
  document.querySelector('#alertPerformanceReport').checked=config.performance_report_enabled===true;
  ...
  document.querySelector('#alertQuietStart').value=config.quiet_start||'21:00';document.querySelector('#alertQuietEnd').value=config.quiet_end||'07:00';document.querySelector('#alertSummaryTime').value=config.daily_summary_time||'19:30';document.querySelector('#alertPerformanceTime').value=config.performance_report_time||'08:30';
  if(config.evolution_instance){const instanceSelect=document.querySelector('#alertEvolutionInstance'),destination=document.querySelector('#whatsappDestination');instanceSelect.innerHTML=`<option value="${escapeHtml(config.evolution_instance)}">${escapeHtml(config.evolution_instance)}</option>`;document.querySelector('#alertEvolutionGroup').innerHTML=`<option value="${escapeHtml(config.evolution_group_jid||'')}">${escapeHtml(config.evolution_group_name||'Grupo configurado')}</option>`;destination.hidden=false;document.querySelector('#whatsappConnectionResult').textContent=`NÃºmero conectado e configurado para ${config.evolution_group_name||'o grupo selecionado'}.`;document.querySelector('#whatsappConnectionResult').className='whatsapp-connection-result success'}
}
async function loadAlerts(trigger=null){
  if(trigger)setButtonLoading(trigger,true,'Atualizando alertas...');
  try{renderAlertHistory(await alertFetchJson('/api/alerts'))}catch(error){document.querySelector('#alertConnection').textContent='API de monitoramento indisponÃ­vel';document.querySelector('#alertHistory').innerHTML=`<div class="alert-empty">${escapeHtml(error.message)}</div>`}finally{if(trigger)setButtonLoading(trigger,false)}
}
...
document.querySelector('#alertSignupForm').addEventListener('submit',async event=>{event.preventDefault();const button=event.submitter||event.currentTarget.querySelector('[type="submit"]'),email=document.querySelector('#alertSignupEmail').value.trim(),password=document.querySelector('#alertSignupPassword').value,confirmation=document.querySelector('#alertSignupPasswordConfirm').value;if(password!==confirmation)return setAlertAuthStatus('As senhas nÃ£o coincidem.');button.disabled=true;setAlertAuthStatus('Criando conta...');try{const payload=await alertAuthPost('signup',{email,password});if(payload.token){completeMonitorLogin(payload.token);hideAlertLogin()}else setAlertAuthMode('login',payload.message||'Conta criada. Confirme o e-mail antes de entrar.',true)}catch(error){setAlertAuthStatus(error.message)}finally{button.disabled=false}});
...
document.querySelector('#alertResetForm').addEventListener('submit',async event=>{event.preventDefault();const button=event.submitter||event.currentTarget.querySelector('[type="submit"]'),password=document.querySelector('#alertResetPassword').value,confirmation=document.querySelector('#alertResetPasswordConfirm').value;if(password!==confirmation)return setAlertAuthStatus('As senhas nÃ£o coincidem.');button.disabled=true;setAlertAuthStatus('Atualizando senha...');try{const payload=await alertAuthPost('update-password',{password,access_token:recoveryAccessToken});recoveryAccessToken='';history.replaceState(null,'',location.pathname);setAlertAuthMode('login',payload.message,true)}catch(error){setAlertAuthStatus(error.message)}finally{button.disabled=false}});
...
document.querySelector('#confirmAlertTest').onclick=async event=>{const button=event.currentTarget,status=document.querySelector('#alertTestStatus'),message=document.querySelector('#alertTestMessage').value.trim(),config=currentAlertDestinationConfig();if(!message)return status.textContent='Digite a mensagem que serÃ¡ enviada.';if(!config.evolution_instance||!config.evolution_group_jid)return status.textContent='Selecione novamente a instÃ¢ncia e o grupo.';setButtonLoading(button,true,'Enviando teste...');try{await alertFetchJson('/api/alerts/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(config)});const payload=await alertFetchJson('/api/alerts/test',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message})});if(!payload.ok)throw new Error('A Evolution API nÃ£o confirmou o envio.');document.querySelector('#alertFormStatus').textContent='Mensagem de teste enviada ao grupo.';closeAlertTest();await loadAlerts()}catch(error){status.textContent=error.message||'Falha no envio.'}finally{setButtonLoading(button,false)}};
let whatsappQrTimer=null,currentWhatsappQrInstance='';
async function finishWhatsappQrConnection(payload){clearInterval(whatsappQrTimer);whatsappQrTimer=null;const result=document.querySelector('#whatsappConnectionResult'),destination=document.querySelector('#whatsappDestination'),instanceSelect=document.querySelector('#alertEvolutionInstance');document.querySelector('#alertWhatsAppPhone').value=payload.phone||'';instanceSelect.innerHTML=`<option value="${escapeHtml(payload.instance)}">${escapeHtml(payload.instance)}${payload.phone?` â€¢ ${escapeHtml(payload.phone)}`:''}</option>`;destination.hidden=false;document.querySelector('#whatsappQrStatus').textContent='WhatsApp conectado com sucesso.';result.textContent=`WhatsApp ${payload.phone||''} conectado. Agora selecione o grupo que receberÃ¡ os alertas.`;result.className='whatsapp-connection-result success';await loadEvolutionGroups(payload.instance);setTimeout(()=>document.querySelector('#whatsappQrPanel').hidden=true,1200)}
async function pollWhatsappQr(){if(!currentWhatsappQrInstance)return;try{const payload=await alertFetchJson(`/api/evolution/status?instance=${encodeURIComponent(currentWhatsappQrInstance)}`);document.querySelector('#whatsappQrStatus').textContent=payload.connected?'WhatsApp conectado com sucesso.':`Aguardando leitura â€¢ estado: ${payload.state||'conectando'}`;if(payload.connected)await finishWhatsappQrConnection(payload)}catch(error){document.querySelector('#whatsappQrStatus').textContent=error.message}}
async function showWhatsappQr(instance,qr){if(!qr)throw new Error('A Evolution API nÃ£o retornou o QR Code.');currentWhatsappQrInstance=instance;document.querySelector('#whatsappQrImage').src=qr;document.querySelector('#whatsappQrPanel').hidden=false;document.querySelector('#whatsappQrStatus').textContent='Aguardando leitura...';clearInterval(whatsappQrTimer);whatsappQrTimer=setInterval(pollWhatsappQr,2500)}
document.querySelector('#connectWhatsAppQr').onclick=async event=>{const button=event.currentTarget,result=document.querySelector('#whatsappConnectionResult'),label=document.querySelector('#alertWhatsAppLabel').value.trim()||'Alertas Hurtz';result.className='whatsapp-connection-result';setButtonLoading(button,true,'Gerando QR Code...');try{const payload=await alertFetchJson('/api/evolution/instance',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label})});await showWhatsappQr(payload.instance,payload.qr);result.textContent='QR Code gerado. Leia pelo WhatsApp para concluir.'}catch(error){result.textContent=error.message;result.classList.add('error')}finally{setButtonLoading(button,false)}};
document.querySelector('#refreshWhatsAppQr').onclick=async()=>{if(!currentWhatsappQrInstance)return;try{const payload=await alertFetchJson(`/api/evolution/qr?instance=${encodeURIComponent(currentWhatsappQrInstance)}`);await showWhatsappQr(payload.instance,payload.qr)}catch(error){document.querySelector('#whatsappQrStatus').textContent=error.message}};
...
function taskKanbanCard(task) {
  ...
  return `<article class="task-card" data-task-id="${task.id}">
    <div class="task-card-chips"><span class="task-kanban-priority ${priority}"><i data-lucide="flag" aria-hidden="true"></i>${labels[priority]}</span>${(task.labels || []).map(label => `<span class="task-kanban-category"><i data-lucide="tag" aria-hidden="true"></i>${escapeHtml(label)}</span>`).join('')}<button class="task-card-settings" type="button" data-task-card-menu="${task.id}" aria-label="OpÃ§Ãµes de ${escapeHtml(task.title)}" aria-expanded="${openTaskCardMenuId === task.id}"><i data-lucide="ellipsis" aria-hidden="true"></i></button></div>
...
function bindTaskKanban(board) {
  ...
  if (!document.querySelector('#taskKanbanHelp')) {
    ...
    help.innerHTML = '<span id="taskKanbanHelp">Pressione espaÃ§o para pegar, use as setas para mover, espaÃ§o para soltar e Escape para cancelar. No celular, arraste pela alÃ§a.</span><span id="taskKanbanAnnouncement" role="status" aria-live="polite" aria-atomic="true"></span>';
    board.before(help);
  }
  ...
  const finish = async cancel => {
    ...
    if (cancel) {
      ...
      clear(); state.card.querySelector('.task-kanban-handle')?.focus(); announce('MovimentaÃ§Ã£o cancelada.'); return;
    }
    ...
    try {
      ...
      nativeTasks.tasks.sort((a, b) => a.position - b.position);
      renderNativeTasks(); focus(); announce(`NÃ£o foi possÃ­vel mover a tarefa. ${error.message}`);
  ...
  const key = event => {
    ...
    announce(`PosiÃ§Ã£o ${[...card.parentElement.children].indexOf(card) + 1} em ${card.closest('[data-task-column]').querySelector('.task-column-name').textContent}.`);
  };
...
const taskApi=async(url,options={})=>{const response=await fetch(url,options),payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||'NÃ£o foi possÃ­vel atualizar as tarefas');if((options.method||'GET').toUpperCase()!=='GET')taskMutationRevision++;return payload};
const taskDateLabel=value=>value?new Date(`${value}T12:00:00`).toLocaleDateString('pt-BR'):'';
const taskExpiryLabel=value=>{if(!value)return'';const hours=Math.max(0,Math.ceil((new Date(value)-new Date())/3600000));return hours<=1?'expira em menos de 1h':`expira em ${hours}h`};
const taskColumnRole=column=>column?.role||(/conclu/i.test(column?.title||'')?'completed':/revis/i.test(column?.title||'')?'review':/bloque/i.test(column?.title||'')?'blocked':/andamento|progresso/i.test(column?.title||'')?'in_progress':'standard');
const taskColumnRoleLabel=role=>({standard:'Normal',in_progress:'Em andamento',review:'RevisÃ£o',blocked:'Bloqueada',completed:'ConcluÃ­da'})[role]||'Normal';
function closeTaskColumnSettings(){document.querySelector('#taskColumnSettingsModal').hidden=true}
function openTaskColumnSettings(columnId){
  const modal=document.querySelector('#taskColumnSettingsModal'),column=nativeTasks.columns.find(item=>item.id===columnId);if(!modal||!column)return;
  const taskCount=nativeTasks.tasks.filter(task=>task.column_id===column.id).length,deleteButton=document.querySelector('#deleteTaskColumn');
  document.querySelector('#taskColumnSettingsId').value=column.id;document.querySelector('#taskColumnSettingsTitle').value=column.title;
  const roleInput=document.querySelector(`[name="taskColumnSettingsRole"][value="${taskColumnRole(column)}"]`);if(roleInput)roleInput.checked=true;
  document.querySelector('#taskColumnSettingsSummary').innerHTML=`<span><b>${taskCount}</b> ${taskCount===1?'tarefa nesta etapa':'tarefas nesta etapa'}</span><span>FunÃ§Ã£o atual: <b>${taskColumnRoleLabel(taskColumnRole(column))}</b></span>`;
  document.querySelector('#taskColumnSettingsStatus').textContent='';deleteButton.disabled=taskCount>0||nativeTasks.columns.length<=1;
  document.querySelector('#taskColumnDeleteHint').textContent=nativeTasks.columns.length<=1?'O quadro precisa manter pelo menos uma etapa.':taskCount>0?`Mova ou exclua ${taskCount===1?'a tarefa':'as tarefas'} antes de excluir esta etapa.`:'Esta etapa estÃ¡ vazia e pode ser excluÃ­da.';
  let order=modal.querySelector('.task-stage-order');if(!order){order=document.createElement('div');order.className='task-stage-order';document.querySelector('#taskColumnSettingsSummary').after(order)}
  const position=nativeTasks.columns.findIndex(item=>item.id===columnId);
  order.innerHTML=`<button type="button" ${position===0?'disabled':''} data-stage-direction="-1">&larr; Mover para a esquerda</button><button type="button" ${position===nativeTasks.columns.length-1?'disabled':''} data-stage-direction="1">Mover para a direita &rarr;</button>`;
  order.querySelectorAll('button').forEach(button=>button.onclick=async()=>{const next=[...nativeTasks.columns],[moved]=next.splice(position,1);next.splice(position+Number(button.dataset.stageDirection),0,moved);order.querySelectorAll('button').forEach(item=>item.disabled=true);const status=document.querySelector('#taskColumnSettingsStatus');status.textContent='Salvando ordem...';try{await Promise.all(next.map((item,index)=>taskApi('/api/task-columns',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:item.id,title:item.title,role:taskColumnRole(item),position:index})})));await loadNativeTasks();openTaskColumnSettings(columnId)}catch(error){await loadNativeTasks();openTaskColumnSettings(columnId);status.textContent=error.message}});
  modal.hidden=false;requestAnimationFrame(()=>document.querySelector('#taskColumnSettingsTitle').focus());
}
function initializeTaskColumnSettings(){
  const modal=document.querySelector('#taskColumnSettingsModal'),form=document.querySelector('#taskColumnSettingsForm'),status=document.querySelector('#taskColumnSettingsStatus');if(!modal||!form)return;
  document.querySelector('#closeTaskColumnSettings').onclick=closeTaskColumnSettings;modal.onclick=event=>{if(event.target===modal)closeTaskColumnSettings()};
  form.onsubmit=async event=>{event.preventDefault();const id=document.querySelector('#taskColumnSettingsId').value,column=nativeTasks.columns.find(item=>item.id===id),title=document.querySelector('#taskColumnSettingsTitle').value.trim(),role=form.querySelector('[name="taskColumnSettingsRole"]:checked')?.value,button=event.submitter;if(!column||!title||!role)return;button.disabled=true;status.textContent='Salvando configuraÃ§Ãµes...';try{await taskApi('/api/task-columns',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,title,position:column.position,role})});closeTaskColumnSettings();await loadNativeTasks()}catch(error){status.textContent=error.message}finally{button.disabled=false}};
  document.querySelector('#deleteTaskColumn').onclick=async()=>{const id=document.querySelector('#taskColumnSettingsId').value,column=nativeTasks.columns.find(item=>item.id===id),taskCount=nativeTasks.tasks.filter(task=>task.column_id===id).length;if(!column||taskCount||nativeTasks.columns.length<=1)return;if(!confirm(`Excluir permanentemente a etapa â€œ${column.title}â€?`))return;status.textContent='Excluindo etapa...';try{await taskApi(`/api/task-columns?id=${encodeURIComponent(id)}`,{method:'DELETE'});closeTaskColumnSettings();await loadNativeTasks()}catch(error){status.textContent=error.message}};
}
function celebrateTaskCompletion(){
  if(matchMedia('(prefers-reduced-motion: reduce)').matches)return;
  const layer=document.createElement('div');layer.className='task-confetti-layer';layer.setAttribute('aria-hidden','true');document.body.appendChild(layer);
  const colors=['#ef7618','#ffb347','#12a66a','#163c78','#f7d35c'],end=Date.now()+5000;
  const burst=()=>{for(let i=0;i<18;i++){const piece=document.createElement('i');piece.style.setProperty('--x',`${Math.random()*100}vw`);piece.style.setProperty('--drift',`${(Math.random()-.5)*180}px`);piece.style.setProperty('--spin',`${360+Math.random()*720}deg`);piece.style.setProperty('--delay',`${Math.random()*.28}s`);piece.style.setProperty('--duration',`${1.7+Math.random()*1.2}s`);piece.style.background=colors[i%colors.length];piece.className=i%3===0?'round':'';layer.appendChild(piece);setTimeout(()=>piece.remove(),3300)}};burst();const timer=setInterval(()=>{if(Date.now()>=end){clearInterval(timer);setTimeout(()=>layer.remove(),3200)}else burst()},650);
}
function taskInlineForm(task=null,columnId=''){
  return `<form class="task-inline-editor" data-inline-task-form="${task?.id||''}" data-inline-column="${columnId||task?.column_id||''}"><label>TÃ­tulo<input name="title" maxlength="180" value="${escapeHtml(task?.title||'Nova tarefa')}" required></label><label>DescriÃ§Ã£o<textarea name="description" maxlength="2000" rows="3" placeholder="Descreva a tarefa. Use @ para mencionar alguÃ©m.">${escapeHtml(task?.description||'')}</textarea></label><div class="task-inline-editor-grid"><label>â–£ Prazo final<input name="due_date" type="date" value="${task?.due_date||''}"></label><label>â—· Tempo (min)<input name="estimate_minutes" type="number" min="0" max="100000" value="${task?.estimate_minutes||''}" placeholder="60"></label></div><div class="task-inline-editor-actions"><button class="save" type="submit">Salvar</button><button type="button" data-cancel-inline-task>Cancelar</button></div><span class="task-inline-editor-status"></span></form>`;
}
...
function renderNativeTasks(){
  ...
  const completedColumns=nativeTasks.columns.filter(column=>taskColumnRole(column)==='completed'),completedIds=new Set(completedColumns.map(column=>column.id)),completedColumn=completedColumns[0],completed=nativeTasks.tasks.filter(task=>completedIds.has(task.column_id)).length,overdue=nativeTasks.tasks.filter(task=>task.due_date&&!completedIds.has(task.column_id)&&new Date(`${task.due_date}T23:59:59`)<NOW).length;
  count.textContent=`${nativeTasks.tasks.length} ${nativeTasks.tasks.length===1?'tarefa':'tarefas'}`;
  document.querySelector('#taskSummary').textContent=`${completed} concluÃ­da${completed===1?'':'s'}${overdue?` â€¢ ${overdue} atrasada${overdue===1?'':'s'}`:''}${visibleTasks.length!==nativeTasks.tasks.length?` â€¢ ${visibleTasks.length} exibida${visibleTasks.length===1?'':'s'}`:''}`;
  ...
  board.querySelectorAll('[data-inline-edit-task]').forEach(button=>button.onclick=()=>{inlineNewColumnId=null;inlineEditingTaskId=button.dataset.inlineEditTask;openTaskCardMenuId=null;renderNativeTasks();board.querySelector(`[data-inline-task-form="${inlineEditingTaskId}"] input[name="title"]`)?.focus()});
  board.querySelectorAll('[data-task-card-menu]').forEach(button=>button.onclick=event=>{event.stopPropagation();openTaskCardMenuId=openTaskCardMenuId===button.dataset.taskCardMenu?null:button.dataset.taskCardMenu;renderNativeTasks()});
  board.querySelectorAll('[data-delete-card]').forEach(button=>button.onclick=async()=>{if(!confirm('Excluir esta tarefa?'))return;await taskApi(`/api/tasks?id=${encodeURIComponent(button.dataset.deleteCard)}`,{method:'DELETE'});openTaskCardMenuId=null;await loadNativeTasks()});
  board.querySelectorAll('[data-cancel-inline-task]').forEach(button=>button.onclick=()=>{inlineEditingTaskId=null;inlineNewColumnId=null;renderNativeTasks()});
  ...
  document.querySelector('#taskAddColumn').onclick=openTaskStageCreator;
...
function taskPropertyTitle(list,id){return list.find(item=>item.id===id)?.title||'â€”'}
function taskProgress(task){const items=nativeTasks.subtasks.filter(item=>item.task_id===task.id);return items.length?Math.round(items.filter(item=>item.is_done).length/items.length*100):0}
function bindTaskEditButtons(root=document){root.querySelectorAll('[data-edit-task]').forEach(button=>button.onclick=()=>openTaskModal(nativeTasks.tasks.find(task=>task.id===button.dataset.editTask)))}
function renderTaskDeadlineAlerts(){
  const target=document.querySelector('#taskDeadlineAlerts'),completed=nativeTasks.columns.find(column=>/conclu/i.test(column.title));
  const soon=nativeTasks.tasks.filter(task=>task.due_date&&task.column_id!==completed?.id).filter(task=>{const days=Math.ceil((new Date(`${task.due_date}T23:59:59`)-NOW)/86400000);return days<=2}).sort((a,b)=>String(a.due_date).localeCompare(String(b.due_date)));
  target.hidden=!soon.length;if(!soon.length)return;
  const late=soon.filter(task=>new Date(`${task.due_date}T23:59:59`)<NOW).length;
  target.innerHTML=`<strong>${late?`${late} tarefa${late===1?'':'s'} atrasada${late===1?'':'s'}`:`${soon.length} prazo${soon.length===1?'':'s'} prÃ³ximo${soon.length===1?'':'s'}`}</strong><span>${soon.slice(0,3).map(task=>escapeHtml(task.title)).join(' â€¢ ')}</span><button type="button" id="showDeadlineTasks">Ver tarefas</button>`;
  document.querySelector('#showDeadlineTasks').onclick=()=>{document.querySelector('#taskSearch').value='';document.querySelector('#taskPriorityFilter').value='';taskView='list';document.querySelectorAll('[data-task-view]').forEach(button=>button.classList.toggle('active',button.dataset.taskView==='list'));renderTaskAlternativeView(soon)};
}
function renderTaskAlternativeView(tasks){
  const board=document.querySelector('#tasksBoard');
  if(taskView==='list'){
    board.className='tasks-board task-list-view';board.innerHTML=`<div class="task-table"><div class="task-table-head"><span>Tarefa</span><span>Etapa</span><span>Projeto</span><span>ResponsÃ¡vel</span><span>Prioridade</span><span>Prazo</span><span>Progresso</span></div>${tasks.map(task=>`<button type="button" class="task-table-row" data-edit-task="${task.id}"><span><b>${escapeHtml(task.title)}</b><small>${escapeHtml((task.labels||[]).join(' â€¢ '))}</small></span><span>${escapeHtml(taskPropertyTitle(nativeTasks.columns,task.column_id))}</span><span>${escapeHtml(taskPropertyTitle(nativeTasks.projects,task.project_id))}</span><span>${escapeHtml(task.assignee||'â€”')}</span><span class="task-priority ${task.priority}">${task.priority==='high'?'Alta':task.priority==='low'?'Baixa':'MÃ©dia'}</span><span>${task.due_date?taskDateLabel(task.due_date):'â€”'}</span><span>${taskProgress(task)}%</span></button>`).join('')||'<div class="task-empty">Nenhuma tarefa encontrada.</div>'}</div>`;bindTaskEditButtons(board);return;
  }
  if(taskView==='calendar'){
    const dated=tasks.filter(task=>task.due_date).sort((a,b)=>a.due_date.localeCompare(b.due_date)),groups=Object.groupBy?Object.groupBy(dated,task=>task.due_date):dated.reduce((all,task)=>((all[task.due_date]??=[]).push(task),all),{});
    board.className='tasks-board task-calendar-view';board.innerHTML=`<div class="task-calendar-grid">${Object.entries(groups).map(([date,items])=>`<section><header><b>${taskDateLabel(date)}</b><span>${items.length}</span></header>${items.map(task=>`<button type="button" data-edit-task="${task.id}"><i class="${task.priority}"></i><span>${escapeHtml(task.title)}</span><small>${escapeHtml(task.assignee||'Sem responsÃ¡vel')}</small></button>`).join('')}</section>`).join('')||'<div class="task-empty">Defina prazos para visualizar o calendÃ¡rio.</div>'}</div>`;bindTaskEditButtons(board);return;
  }
  const completed=nativeTasks.columns.find(column=>/conclu/i.test(column.title)),done=tasks.filter(task=>task.column_id===completed?.id).length,late=tasks.filter(task=>task.due_date&&task.column_id!==completed?.id&&new Date(`${task.due_date}T23:59:59`)<NOW).length,estimated=tasks.reduce((sum,task)=>sum+(Number(task.estimate_minutes)||0),0),byAssignee=Object.entries(tasks.reduce((all,task)=>{const key=task.assignee||'Sem responsÃ¡vel';all[key]=(all[key]||0)+1;return all},{})).sort((a,b)=>b[1]-a[1]);
  board.className='tasks-board task-analytics-view';board.innerHTML=`<div class="task-kpis"><article><span>Total</span><b>${tasks.length}</b><small>tarefas no recorte</small></article><article><span>ConcluÃ­das</span><b>${done}</b><small>${tasks.length?Math.round(done/tasks.length*100):0}% do total</small></article><article><span>Atrasadas</span><b>${late}</b><small>exigem atenÃ§Ã£o</small></article><article><span>Estimativa</span><b>${Math.round(estimated/60)}h</b><small>de trabalho planejado</small></article></div><div class="task-analytics-panels"><section><h3>DistribuiÃ§Ã£o por etapa</h3>${nativeTasks.columns.map(column=>{const value=tasks.filter(task=>task.column_id===column.id).length,percent=tasks.length?value/tasks.length*100:0;return `<div class="task-progress-row"><span>${escapeHtml(column.title)}</span><i><b style="width:${percent}%"></b></i><strong>${value}</strong></div>`}).join('')}</section><section><h3>Carga por responsÃ¡vel</h3>${byAssignee.slice(0,8).map(([name,value])=>`<div class="task-progress-row"><span>${escapeHtml(name)}</span><i><b style="width:${tasks.length?value/tasks.length*100:0}%"></b></i><strong>${value}</strong></div>`).join('')}</section></div>`;
}
function loadNativeTasks(){if(taskLoadPromise)return taskLoadPromise;taskLoadPromise=(async()=>{let revision;do{revision=taskMutationRevision;await refreshNativeTasks()}while(revision!==taskMutationRevision)})().finally(()=>{taskLoadPromise=null});return taskLoadPromise}
async function refreshNativeTasks(){
  ...
  try{const revision=taskMutationRevision,updated=await taskApi('/api/tasks');if(revision!==taskMutationRevision)return;nativeTasks=updated;delete board.dataset.syncError;board.removeAttribute('aria-label');nativeTasks.tasks=(nativeTasks.tasks||[]).filter(task=>!task.expires_at||new Date(task.expires_at)>new Date());taskLoadedAt=Date.now();try{userStorage.setItem(TASK_LOCAL_CACHE_KEY,JSON.stringify({savedAt:taskLoadedAt,data:nativeTasks}))}catch{}const select=document.querySelector('#taskAssigneeFilter'),selected=select.value,assignees=[...new Set(nativeTasks.tasks.map(task=>task.assignee).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR'));select.innerHTML='<option value="">Todos os responsÃ¡veis</option>'+assignees.map(name=>`<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('');if(assignees.includes(selected))select.value=selected;
    ...
    fill('#taskProjectFilter','Todos os projetos',nativeTasks.projects||[]);fill('#taskModuleFilter','Todos os mÃ³dulos',nativeTasks.modules||[]);fill('#taskCycleFilter','Todos os ciclos',nativeTasks.cycles||[]);renderTaskNotificationCount();renderNativeTasks()
  }catch(error){if(!nativeTasks.columns.length)board.innerHTML=`<div class="task-error">${escapeHtml(error.message)}</div>`;else{board.dataset.syncError=error.message;board.setAttribute('aria-label','Quadro salvo. AtualizaÃ§Ã£o indisponÃ­vel: '+error.message)}}
}
function openTaskModal(task=null,columnId=''){
  ...
  document.querySelector('#taskProject').innerHTML='<option value="">Sem projeto</option>'+nativeTasks.projects.map(item=>`<option value="${item.id}">${escapeHtml(item.title)}</option>`).join('');
  ...
  document.querySelector('#taskLabels').value=(task?.labels||[]).join(', ');document.querySelector('#taskColumn').value=task?.column_id||columnId||nativeTasks.columns[0]?.id||'';
  document.querySelector('#taskProject').value=task?.project_id||nativeTasks.projects[0]?.id||'';updateTaskRelationOptions(task?.module_id||'',task?.cycle_id||'');document.querySelector('#taskEstimate').value=task?.estimate_minutes||'';
  document.querySelector('#taskDetailsSuite').hidden=!task;if(task){activeTaskDetail='checklist';document.querySelectorAll('[data-task-detail]').forEach(button=>button.classList.toggle('active',button.dataset.taskDetail==='checklist'));renderTaskDetailPanel(task.id)}
  updateTaskMentionMenu();
}
function taskMentionNames(){return [...new Set(nativeTasks.tasks.map(task=>task.assignee).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR'))}
function taskMentions(value){const names=taskMentionNames();return names.filter(name=>new RegExp(`@${name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}(?=\\s|$|[.,;:!?])`,'i').test(value||''))}
function updateTaskMentionMenu(){const textarea=document.querySelector('#taskDescription'),menu=document.querySelector('#taskMentionMenu');if(!textarea||!menu)return;const cursor=textarea.selectionStart??textarea.value.length,before=textarea.value.slice(0,cursor),match=before.match(/@([^@\n]*)$/),search=match?.[1]?.trim().toLocaleLowerCase('pt-BR');if(!match||match[1].includes(',')||match[1].length>100){menu.hidden=true;return}const matches=taskMentionNames().filter(name=>name.toLocaleLowerCase('pt-BR').includes(search)).slice(0,8);menu.hidden=!matches.length;menu.innerHTML=matches.map(name=>`<button type="button" data-task-mention="${escapeHtml(name)}"><span>${escapeHtml(name.slice(0,1).toUpperCase())}</span>${escapeHtml(name)}</button>`).join('');menu.querySelectorAll('[data-task-mention]').forEach(button=>button.onclick=()=>{const start=before.lastIndexOf('@'),replacement=`@${button.dataset.taskMention} `;textarea.value=textarea.value.slice(0,start)+replacement+textarea.value.slice(cursor);const next=start+replacement.length;textarea.focus();textarea.setSelectionRange(next,next);menu.hidden=true})}
function updateTaskRelationOptions(moduleValue='',cycleValue=''){const projectId=document.querySelector('#taskProject').value,modules=nativeTasks.modules.filter(item=>!projectId||item.project_id===projectId),cycles=nativeTasks.cycles.filter(item=>!projectId||item.project_id===projectId);document.querySelector('#taskModule').innerHTML='<option value="">Sem mÃ³dulo</option>'+modules.map(item=>`<option value="${item.id}">${escapeHtml(item.title)}</option>`).join('');document.querySelector('#taskCycle').innerHTML='<option value="">Sem ciclo</option>'+cycles.map(item=>`<option value="${item.id}">${escapeHtml(item.title)}</option>`).join('');document.querySelector('#taskModule').value=moduleValue;document.querySelector('#taskCycle').value=cycleValue}
function taskActivityLabel(action){return ({task_created:'Tarefa criada',task_updated:'Tarefa atualizada',subtask_created:'Subtarefa criada',subtask_updated:'Subtarefa atualizada',subtask_deleted:'Subtarefa excluÃ­da',comment_created:'ComentÃ¡rio adicionado',attachment_added:'Anexo enviado',attachment_deleted:'Anexo excluÃ­do'})[action]||action.replaceAll('_',' ')}
function renderTaskDetailPanel(taskId){
  const panel=document.querySelector('#taskDetailPanel');
  if(activeTaskDetail==='checklist'){const items=nativeTasks.subtasks.filter(item=>item.task_id===taskId);panel.innerHTML=`<form class="task-inline-form" id="taskSubtaskForm"><input maxlength="180" placeholder="Adicionar item ao checklist..." required><button type="submit">Adicionar</button></form><div class="task-checklist">${items.map(item=>`<label><input type="checkbox" data-toggle-subtask="${item.id}" ${item.is_done?'checked':''}><span>${escapeHtml(item.title)}</span><button type="button" data-delete-subtask="${item.id}">Ã—</button></label>`).join('')||'<div class="task-detail-empty">Nenhum item no checklist.</div>'}</div>`;document.querySelector('#taskSubtaskForm').onsubmit=async event=>{event.preventDefault();const input=event.currentTarget.querySelector('input');await taskApi('/api/task-subtasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task_id:taskId,title:input.value,position:items.length})});await loadNativeTasks();renderTaskDetailPanel(taskId)};panel.querySelectorAll('[data-toggle-subtask]').forEach(input=>input.onchange=async()=>{await taskApi('/api/task-subtasks',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:input.dataset.toggleSubtask,task_id:taskId,is_done:input.checked})});await loadNativeTasks();renderTaskDetailPanel(taskId)});panel.querySelectorAll('[data-delete-subtask]').forEach(button=>button.onclick=async()=>{await taskApi(`/api/task-subtasks?id=${button.dataset.deleteSubtask}&task_id=${taskId}`,{method:'DELETE'});await loadNativeTasks();renderTaskDetailPanel(taskId)});return}
  if(activeTaskDetail==='comments'){const items=nativeTasks.comments.filter(item=>item.task_id===taskId);panel.innerHTML=`<div class="task-comments">${items.map(item=>`<article><b>${escapeHtml(item.author)}</b><time>${new Date(item.created_at).toLocaleString('pt-BR')}</time><p>${escapeHtml(item.body)}</p></article>`).join('')||'<div class="task-detail-empty">Nenhum comentÃ¡rio.</div>'}</div><form class="task-comment-form" id="taskCommentForm"><input id="taskCommentAuthor" maxlength="100" placeholder="Seu nome" value="Equipe Hurtz"><textarea maxlength="3000" placeholder="Escreva um comentÃ¡rio..." required></textarea><button type="submit">Comentar</button></form>`;document.querySelector('#taskCommentForm').onsubmit=async event=>{event.preventDefault();const form=event.currentTarget;await taskApi('/api/task-comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task_id:taskId,author:form.querySelector('input').value,body:form.querySelector('textarea').value})});await loadNativeTasks();renderTaskDetailPanel(taskId)};return}
  if(activeTaskDetail==='attachments'){const items=nativeTasks.attachments.filter(item=>item.task_id===taskId);panel.innerHTML=`<label class="task-upload"><input id="taskAttachmentInput" type="file" hidden><span>+ Enviar arquivo</span><small>AtÃ© 5 MB</small></label><div class="task-attachments">${items.map(item=>`<article><button type="button" data-download-attachment="${item.id}" data-file-name="${escapeHtml(item.file_name)}"><b>${escapeHtml(item.file_name)}</b><small>${Math.max(1,Math.round(item.size_bytes/1024))} KB</small></button><button type="button" data-delete-attachment="${item.id}">Ã—</button></article>`).join('')||'<div class="task-detail-empty">Nenhum anexo.</div>'}</div>`;document.querySelector('#taskAttachmentInput').onchange=async event=>{const file=event.target.files[0];if(!file)return;if(file.size>5*1024*1024)return alert('O arquivo deve ter no mÃ¡ximo 5 MB.');const data=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(',')[1]);reader.onerror=reject;reader.readAsDataURL(file)});await taskApi('/api/task-attachments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task_id:taskId,file_name:file.name,mime_type:file.type,data})});await loadNativeTasks();renderTaskDetailPanel(taskId)};panel.querySelectorAll('[data-download-attachment]').forEach(button=>button.onclick=async()=>{const response=await fetch(`/api/task-attachments?id=${button.dataset.downloadAttachment}`);if(!response.ok)return alert('NÃ£o foi possÃ­vel baixar o anexo.');const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=button.dataset.fileName;link.click();URL.revokeObjectURL(url)});panel.querySelectorAll('[data-delete-attachment]').forEach(button=>button.onclick=async()=>{if(!confirm('Excluir este anexo?'))return;await taskApi(`/api/task-attachments?id=${button.dataset.deleteAttachment}`,{method:'DELETE'});await loadNativeTasks();renderTaskDetailPanel(taskId)});return}
  const items=nativeTasks.activities.filter(item=>item.task_id===taskId);panel.innerHTML=`<div class="task-activity">${items.map(item=>`<article><i></i><div><b>${escapeHtml(taskActivityLabel(item.action))}</b><span>${escapeHtml(item.actor)} â€¢ ${new Date(item.created_at).toLocaleString('pt-BR')}</span></div></article>`).join('')||'<div class="task-detail-empty">O histÃ³rico comeÃ§arÃ¡ na prÃ³xima alteraÃ§Ã£o.</div>'}</div>`;
}
function closeTaskModal(){document.querySelector('#taskModal').hidden=true}
function renderTaskNotificationCount(){const target=document.querySelector('#taskNotificationCount');if(target)target.textContent=String((nativeTasks.notifications||[]).filter(item=>!item.is_read).length)}
function openTaskNotifications(){const modal=document.querySelector('#taskNotificationsModal'),list=document.querySelector('#taskNotificationList'),items=nativeTasks.notifications||[];list.innerHTML=items.map(item=>{const task=nativeTasks.tasks.find(task=>task.id===item.task_id);return `<button type="button" data-notification-task="${item.task_id}"><span>@${escapeHtml(item.recipient_name)}</span><b>${escapeHtml(task?.title||'Tarefa removida')}</b><small>${new Date(item.created_at).toLocaleString('pt-BR')}</small></button>`}).join('')||'<div class="task-detail-empty">Nenhuma menÃ§Ã£o registrada.</div>';list.querySelectorAll('[data-notification-task]').forEach(button=>button.onclick=()=>{const task=nativeTasks.tasks.find(item=>item.id===button.dataset.notificationTask);if(task){modal.hidden=true;openTaskModal(task)}});modal.hidden=false}
async function initializeNativeTasks(){
  if(tasksInitialized){if(Date.now()-taskLoadedAt>30000)void loadNativeTasks();return}tasksInitialized=true;
  try{const cached=JSON.parse(userStorage.getItem(TASK_LOCAL_CACHE_KEY)||'null');if(cached?.data&&Date.now()-cached.savedAt<7*86400000){nativeTasks=cached.data;taskLoadedAt=cached.savedAt;nativeTasks.tasks=(nativeTasks.tasks||[]).filter(task=>!task.expires_at||new Date(task.expires_at)>new Date());renderNativeTasks()}}catch{}
  ...
  ['#taskSearch','#taskPriorityFilter','#taskAssigneeFilter','#taskProjectFilter','#taskModuleFilter','#taskCycleFilter'].forEach(selector=>{const element=document.querySelector(selector);element[element.tagName==='INPUT'?'oninput':'onchange']=renderNativeTasks});
  document.querySelectorAll('[data-task-view]').forEach(button=>button.onclick=()=>{taskView=button.dataset.taskView;document.querySelectorAll('[data-task-view]').forEach(item=>item.classList.toggle('active',item===button));renderNativeTasks()});
  document.querySelector('#taskProject').onchange=()=>updateTaskRelationOptions();
  document.querySelector('#taskDescription').addEventListener('input',updateTaskMentionMenu);document.querySelector('#taskDescription').addEventListener('click',updateTaskMentionMenu);document.querySelector('#taskDescription').addEventListener('blur',()=>setTimeout(()=>document.querySelector('#taskMentionMenu').hidden=true,150));
  document.querySelectorAll('[data-task-detail]').forEach(button=>button.onclick=()=>{activeTaskDetail=button.dataset.taskDetail;document.querySelectorAll('[data-task-detail]').forEach(item=>item.classList.toggle('active',item===button));renderTaskDetailPanel(document.querySelector('#taskId').value)});
  document.querySelector('#closeTaskModal').onclick=closeTaskModal;
  document.querySelector('#taskModal').onclick=event=>{if(event.target.id==='taskModal')closeTaskModal()};
  document.querySelector('#taskForm').onsubmit=async event=>{
    event.preventDefault();const id=document.querySelector('#taskId').value,status=document.querySelector('#taskFormStatus'),button=event.submitter,columnId=document.querySelector('#taskColumn').value,column=nativeTasks.columns.find(item=>item.id===columnId);
    const payload={id,title:document.querySelector('#taskTitle').value,description:document.querySelector('#taskDescription').value,assignee:document.querySelector('#taskAssignee').value,priority:document.querySelector('#taskPriority').value,due_date:document.querySelector('#taskDueDate').value||null,labels:document.querySelector('#taskLabels').value.split(',').map(item=>item.trim()).filter(Boolean),column_id:columnId,project_id:document.querySelector('#taskProject').value||null,module_id:document.querySelector('#taskModule').value||null,cycle_id:document.querySelector('#taskCycle').value||null,estimate_minutes:Number(document.querySelector('#taskEstimate').value)||null,completed_at:taskColumnRole(column)==='completed'?new Date().toISOString():null,position:nativeTasks.tasks.filter(task=>task.column_id===columnId).length};
    button.disabled=true;status.textContent='Salvando...';try{const saved=await taskApi('/api/tasks',{method:id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),taskId=id||saved.id,recipients=taskMentions(payload.description);if(taskId&&recipients.length)await taskApi('/api/task-notifications',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task_id:taskId,recipients})});closeTaskModal();await loadNativeTasks()}catch(error){status.textContent=error.message}finally{button.disabled=false}
  };
  document.querySelector('#deleteTask').onclick=async()=>{const id=document.querySelector('#taskId').value;if(!id||!confirm('Excluir esta tarefa?'))return;const status=document.querySelector('#taskFormStatus');status.textContent='Excluindo...';try{await taskApi(`/api/tasks?id=${encodeURIComponent(id)}`,{method:'DELETE'});closeTaskModal();await loadNativeTasks()}catch(error){status.textContent=error.message}};
  initializeTaskStructures();
  initializeTaskColumnSettings();
  await loadNativeTasks();
}
function openTaskStageCreator(){
  const button=document.querySelector('#taskAddColumn'),section=button?.parentElement;
  if(!section||section.querySelector('form'))return;
  button.hidden=true;
  const form=document.createElement('form');form.id='taskStageInlineForm';form.className='task-inline-editor';
  form.innerHTML='<label>Nome da etapa<input name="title" maxlength="80" placeholder="Ex.: Em revisÃ£o" required></label><label>FunÃ§Ã£o<select name="role"><option value="standard">Normal</option><option value="in_progress">Em andamento</option><option value="review">RevisÃ£o</option><option value="blocked">Bloqueada</option><option value="completed">ConcluÃ­da</option></select></label><div class="task-inline-editor-actions"><button class="save" type="submit">Criar etapa</button><button type="button" data-cancel-stage>Cancelar</button></div><span class="task-inline-editor-status" role="status"></span>';
  section.append(form);form.elements.title.focus();
  const cancel=()=>{form.remove();button.hidden=false;button.focus()};
  form.querySelector('[data-cancel-stage]').onclick=cancel;
  form.onkeydown=event=>{if(event.key==='Escape'){event.preventDefault();cancel()}};
  form.onsubmit=async event=>{event.preventDefault();const title=form.elements.title.value.trim(),status=form.querySelector('[role="status"]');if(!title){form.elements.title.focus();return}const submit=form.querySelector('[type="submit"]');submit.disabled=true;status.textContent='Criando etapa...';try{await taskApi('/api/task-columns',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,role:form.elements.role.value,position:nativeTasks.columns.length})});await loadNativeTasks();document.querySelector('#taskAddColumn')?.focus()}catch(error){status.textContent=error.message;submit.disabled=false}};
}
function refreshTaskStructureProjects(){
  for(const id of ['newTaskModuleProject','newTaskCycleProject']){
    const select=document.getElementById(id),previous=select.value,hasProjects=nativeTasks.projects.length>0;
    select.innerHTML='<option value="">'+(hasProjects?'Selecione um projeto':'Crie um projeto primeiro')+'</option>'+nativeTasks.projects.map(item=>`<option value="${item.id}">${escapeHtml(item.title)}</option>`).join('');
    if(nativeTasks.projects.some(item=>item.id===previous))select.value=previous;
    select.disabled=!hasProjects;select.closest('form').querySelector('[type="submit"]').disabled=!hasProjects;
  }
}
function initializeTaskStructures(){
  const modal=document.querySelector('#taskStructureModal'),status=document.querySelector('#taskStructureStatus');
  const close=()=>{modal.hidden=true;document.querySelector('#taskStructureButton').focus()};
  document.querySelector('#taskStructureButton').onclick=()=>{refreshTaskStructureProjects();renderTaskStructures();status.textContent='';modal.hidden=false;document.querySelector('#newTaskProjectTitle').focus()};
  document.querySelector('#closeTaskStructure').onclick=close;modal.onclick=event=>{if(event.target===modal)close()};
  document.addEventListener('keydown',event=>{if(modal.hidden)return;if(event.key==='Escape'){event.preventDefault();close()}if(event.key==='Tab'){const controls=[...modal.querySelectorAll('button,input,select,[tabindex="0"]')].filter(item=>!item.disabled&&item.getClientRects().length),first=controls[0],last=controls.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}}});
  modal.querySelectorAll('[data-structure-form]').forEach(form=>form.onsubmit=async event=>{
    event.preventDefault();const type=form.dataset.structureForm,data=new FormData(form),title=String(data.get('title')||'').trim(),button=form.querySelector('[type="submit"]');
    if(!title){form.elements.title.focus();return}
    const payload={title};if(type!=='project'){payload.project_id=data.get('project_id');if(!payload.project_id)return}
    if(type==='cycle'){payload.starts_on=data.get('starts_on')||null;payload.ends_on=data.get('ends_on')||null;if(payload.starts_on&&payload.ends_on&&payload.starts_on>payload.ends_on){status.textContent='O fim do ciclo deve ser igual ou posterior ao inÃ­cio.';return}}
    button.disabled=true;status.textContent='Salvando...';
    try{await taskApi(`/api/task-${type}s`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});form.reset();await loadNativeTasks();refreshTaskStructureProjects();renderTaskStructures();status.textContent=({project:'Projeto criado.',module:'MÃ³dulo criado.',cycle:'Ciclo criado.'})[type]}
    catch(error){status.textContent=error.message}finally{button.disabled=type!=='project'&&!nativeTasks.projects.length}
  });
}
function renderTaskStructures(){
  const target=document.querySelector('#taskStructureExisting'),status=document.querySelector('#taskStructureStatus'),groups=[['Projetos','project',nativeTasks.projects],['MÃ³dulos','module',nativeTasks.modules],['Ciclos','cycle',nativeTasks.cycles]];
  target.innerHTML=groups.map(([title,type,items])=>`<section class="task-structure-${type}s"><h4>${title} <small>${items.length}</small></h4>${items.map(item=>`<div><span title="${escapeHtml(item.title)}">${escapeHtml(item.title)}${item.project_id?`<small>${escapeHtml(nativeTasks.projects.find(project=>project.id===item.project_id)?.title||'')}</small>`:''}</span><button type="button" data-edit-structure="${type}" data-id="${item.id}" data-title="${escapeHtml(item.title)}" aria-label="Editar ${escapeHtml(item.title)}">Editar</button><button type="button" data-delete-structure="${type}" data-id="${item.id}" aria-label="Excluir ${escapeHtml(item.title)}">Excluir</button></div>`).join('')||'<p class="task-structure-empty">Nenhum item criado.</p>'}</section>`).join('');
  target.querySelectorAll('[data-edit-structure]').forEach(button=>button.onclick=async()=>{const title=prompt('Novo nome:',button.dataset.title);if(!title?.trim())return;button.disabled=true;try{await taskApi(`/api/task-${button.dataset.editStructure}s`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:button.dataset.id,title:title.trim()})});await loadNativeTasks();refreshTaskStructureProjects();renderTaskStructures();status.textContent='Nome atualizado.'}catch(error){status.textContent=error.message;button.disabled=false}});
  target.querySelectorAll('[data-delete-structure]').forEach(button=>button.onclick=async()=>{if(!confirm('Excluir este item? As tarefas serÃ£o preservadas sem este vÃ­nculo.'))return;button.disabled=true;try{await taskApi(`/api/task-${button.dataset.deleteStructure}s?id=${button.dataset.id}`,{method:'DELETE'});await loadNativeTasks();refreshTaskStructureProjects();renderTaskStructures();status.textContent='Item excluÃ­do.'}catch(error){status.textContent=error.message;button.disabled=false}});
}
...
async function initializeEmailAuthCallback(){
  ...
  if(accessToken){document.querySelector('#alertLoginModal').hidden=false;setAlertAuthMode('login','Confirmando seu e-mail...');try{const payload=await alertAuthPost('exchange',{access_token:accessToken});completeMonitorLogin(payload.token);history.replaceState(null,'',location.pathname);hideAlertLogin()}catch(error){setAlertAuthStatus(error.message)}}
  else if(query.get('auth')==='confirmed'){document.querySelector('#alertLoginModal').hidden=false;setAlertAuthMode('login','E-mail confirmado. Entre para continuar.',true);history.replaceState(null,'',location.pathname)}
}
initializeEmailAuthCallback();

let creativeLibraryInitialized=false,creativeVideos=[];
const compactMetric=value=>{const number=Number(value);if(!Number.isFinite(number)||number<0)return 'â€”';return number>=1e6?`${(number/1e6).toLocaleString('pt-BR',{maximumFractionDigits:1})} mi`:number>=1e3?`${(number/1e3).toLocaleString('pt-BR',{maximumFractionDigits:1})} mil`:number.toLocaleString('pt-BR')};
function renderCreativeLibrary(){
  const root=document.querySelector('#creativeLibrary'),status=document.querySelector('#creativeLibraryStatus'),query=document.querySelector('#creativeLibrarySearch').value.trim().toLocaleLowerCase('pt-BR'),sort=document.querySelector('#creativeSort').value;
  let rows=creativeVideos.filter(video=>!query||[video.title,video.creator_name,video.search_term,video.product].some(value=>String(value||'').toLocaleLowerCase('pt-BR').includes(query)));
  rows=[...rows].sort((a,b)=>sort==='views'?Number(b.view_count||0)-Number(a.view_count||0):sort==='likes'?Number(b.like_count||0)-Number(a.like_count||0):new Date(b.created_at)-new Date(a.created_at));
  status.textContent=`${rows.length} de ${creativeVideos.length} vÃ­deo(s) salvos`;
  root.innerHTML=rows.map(video=>`<article class="creative-video-card" data-creative-id="${video.id}"><a class="creative-thumb" href="${escapeHtml(video.video_url)}" target="_blank" rel="noopener noreferrer">${video.thumbnail_url?`<img src="${escapeHtml(video.thumbnail_url)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:'<span>â–¶</span>'}<b>Abrir no TikTok</b></a><div class="creative-video-body"><div class="creative-video-tags"><span>${escapeHtml(video.product||'Sem produto')}</span><span>${escapeHtml(video.search_term||'Pesquisa manual')}</span></div><h4>${escapeHtml(video.title||'VÃ­deo do TikTok')}</h4><a href="${escapeHtml(video.creator_url||video.video_url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(video.creator_name||'Criador nÃ£o identificado')}</a><div class="creative-video-metrics"><span>VisualizaÃ§Ãµes <b>${compactMetric(video.view_count)}</b></span><span>Curtidas <b>${compactMetric(video.like_count)}</b></span><span>ComentÃ¡rios <b>${compactMetric(video.comment_count)}</b></span></div><div class="creative-video-actions"><button type="button" data-edit-creative="${video.id}">Editar</button><button type="button" data-delete-creative="${video.id}">Excluir</button></div></div></article>`).join('')||'<div class="creative-library-empty"><strong>Nenhum vÃ­deo encontrado</strong><span>FaÃ§a uma pesquisa e use a extensÃ£o Hurtz no TikTok para salvar os resultados visÃ­veis.</span></div>';
}
async function loadCreativeLibrary(){const status=document.querySelector('#creativeLibraryStatus');status.textContent='Atualizando biblioteca...';try{const response=await fetch('/api/creative-videos'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao carregar vÃ­deos.');creativeVideos=payload.videos||[];document.querySelector('#creativeExtensionStatus').textContent='Biblioteca conectada';document.querySelector('#creativeExtensionStatus').classList.add('connected');renderCreativeLibrary()}catch(error){status.textContent=error.message;document.querySelector('#creativeLibrary').innerHTML='<div class="creative-library-empty"><strong>Biblioteca indisponÃ­vel</strong><span>Verifique a conexÃ£o do Traffic pocket com o banco.</span></div>'}}
function initializeCreativeLibrary(){if(creativeLibraryInitialized)return;creativeLibraryInitialized=true;document.querySelector('#creativeSearchForm').onsubmit=event=>{event.preventDefault();const term=document.querySelector('#creativeSearchTerm').value.trim();if(!term)return;userStorage.setItem('hurtz-creative-last-search',JSON.stringify({search_term:term,product:document.querySelector('#creativeProduct').value}));window.open(`https://www.tiktok.com/search?q=${encodeURIComponent(term)}`,'_blank','noopener')};document.querySelector('#refreshCreativeLibrary').onclick=loadCreativeLibrary;document.querySelector('#creativeSort').onchange=renderCreativeLibrary;document.querySelector('#creativeLibrarySearch').oninput=renderCreativeLibrary;document.querySelector('#copyCreativeConnection').onclick=async event=>{const token=userStorage.getItem(MONITOR_SESSION_KEY);if(!token)return alert('Entre novamente no Traffic pocket para gerar a conexÃ£o da extensÃ£o.');const config=JSON.stringify({api_url:MONITOR_API_BASE||location.origin,token},null,2);await navigator.clipboard.writeText(config);event.currentTarget.textContent='ConexÃ£o copiada';setTimeout(()=>event.currentTarget.textContent='Copiar conexÃ£o da extensÃ£o',1800)};document.querySelector('#creativeLibrary').onclick=async event=>{const edit=event.target.closest('[data-edit-creative]'),remove=event.target.closest('[data-delete-creative]');if(edit){const video=creativeVideos.find(item=>item.id===edit.dataset.editCreative),title=prompt('TÃ­tulo do vÃ­deo:',video?.title||'');if(title===null)return;const product=prompt('Produto:',video?.product||'');if(product===null)return;await fetch('/api/creative-videos',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:video.id,title,product})});await loadCreativeLibrary()}if(remove&&confirm('Excluir esta referÃªncia da biblioteca?')){await fetch(`/api/creative-videos?id=${encodeURIComponent(remove.dataset.deleteCreative)}`,{method:'DELETE'});await loadCreativeLibrary()}};loadCreativeLibrary()}
...
renderCreativeLibrary=function(){
  const root=document.querySelector('#creativeLibrary'),status=document.querySelector('#creativeLibraryStatus');status.textContent=creativeVideos.length?`${creativeVideos.length} vÃ­deo(s) encontrados â€¢ resultados nÃ£o armazenados`:'Nenhum resultado nesta sessÃ£o.';
  root.innerHTML=creativeVideos.map((video,index)=>{const platform=video.platform==='instagram'?'instagram':'tiktok',platformName=platform==='instagram'?'Instagram':'TikTok',coverTag=video.cover_text_detected===false?'<span class="cover-clean">Sem texto detectado</span>':video.cover_text_detected===true?'<span class="cover-text">Texto na capa</span>':video.cover_ocr_checked?'<span class="cover-unknown">Capa nÃ£o legÃ­vel</span>':'';return `<article class="creative-video-card" data-creative-index="${index}"><label class="creative-select"><input type="checkbox" checked data-select-creative="${index}"><span>Selecionar</span></label><button class="creative-thumb" type="button" data-preview-creative="${index}">${video.thumbnail_url?`<img src="${escapeHtml(video.thumbnail_url)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:'<span>â–¶</span>'}<b>Assistir preview</b></button><div class="creative-video-body"><div class="creative-video-tags"><span class="platform-${platform}">${platformName}</span><span>${escapeHtml(video.search_term||'Busca relacionada')}</span><span>${video.duration?`${video.duration}s`:'VÃ­deo'}</span>${coverTag}${video.visual_verified?`<span class="verified">âœ“ ${video.detected_type==='car'?'Carro confirmado':'ImÃ³vel confirmado'}</span>`:''}</div><h4>${escapeHtml(video.title||`VÃ­deo do ${platformName}`)}</h4><a href="${escapeHtml(video.creator_url||video.video_url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(video.creator_name||'Criador nÃ£o identificado')}</a><div class="creative-video-metrics"><span>VisualizaÃ§Ãµes <b>${compactMetric(video.view_count)}</b></span><span>Curtidas <b>${compactMetric(video.like_count)}</b></span><span>ComentÃ¡rios <b>${compactMetric(video.comment_count)}</b></span></div><div class="creative-video-actions"><a href="${escapeHtml(video.video_url)}" target="_blank" rel="noopener noreferrer">Abrir no ${platformName}</a>${video.download_url?`<a class="primary" href="${escapeHtml(video.download_url)}" target="_blank" rel="noopener noreferrer" download="${platform}-${escapeHtml(video.id)}.mp4">Baixar vÃ­deo</a>`:'<span>Download indisponÃ­vel</span>'}</div></div></article>`}).join('')||'<div class="creative-library-empty"><strong>Nenhum vÃ­deo encontrado</strong><span>Tente termos mais especÃ­ficos ou escolha outra plataforma.</span></div>';
};
const prepareCreativeCover=async url=>{const response=await fetch(url);if(!response.ok)throw new Error('Capa indisponÃ­vel');const bitmap=await createImageBitmap(await response.blob()),scale=Math.min(1,720/bitmap.width),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));const context=canvas.getContext('2d',{alpha:false});context.fillStyle='#fff';context.fillRect(0,0,canvas.width,canvas.height);context.drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close();return canvas};
const prioritizeCreativeCoversWithoutText=async(videos,onProgress)=>{if(!videos.length)return videos;await loadOptionalScript('Tesseract','https://cdn.jsdelivr.net/npm/tesseract.js@6/dist/tesseract.min.js');const workers=[];let next=0,done=0;try{workers.push(...await Promise.all(Array.from({length:Math.min(2,videos.length)},()=>Tesseract.createWorker('por'))));await Promise.all(workers.map(async worker=>{while(next<videos.length){const video=videos[next++];try{if(!video.thumbnail_proxy_url)throw new Error('Sem capa');const canvas=await prepareCreativeCover(video.thumbnail_proxy_url),result=await worker.recognize(canvas),text=String(result?.data?.text||'').replace(/[^\p{L}\p{N}]+/gu,' ').trim(),confidence=Number(result?.data?.confidence)||0;video.cover_ocr_checked=true;video.cover_ocr_confidence=Math.round(confidence);video.cover_text_detected=confidence>=35&&text.replace(/\s/g,'').length>=4}catch{video.cover_ocr_checked=true;video.cover_text_detected=null}onProgress?.(++done,videos.length)}}))}finally{await Promise.all(workers.map(worker=>worker.terminate().catch(()=>{})))}const weight=video=>video.cover_text_detected===false?0:video.cover_text_detected===true?2:1;return videos.map((video,index)=>({video,index})).sort((a,b)=>weight(a.video)-weight(b.video)||a.index-b.index).map(item=>item.video)};
initializeCreativeLibrary=function(){
  ...
  const resumeCreativeSearch=async saved=>{
    ...
    button.disabled=true;progress.hidden=false;progressTitle.textContent='Retomando pesquisa de vÃ­deos...';
    try{
      let payload;
      do{
        const response=await fetch(`/api/creative-audit/status?id=${encodeURIComponent(saved.id)}`);payload=await response.json();
        if(!response.ok)throw new Error(payload.error||'A pesquisa temporÃ¡ria nÃ£o estÃ¡ mais disponÃ­vel.');
        progressTitle.textContent=payload.status==='complete'?'Finalizando resultados...':'Preparando resultados...';
        progressDetail.textContent='Preparando os vÃ­deos encontrados no servidor.';
        document.querySelector('#creativeLibraryStatus').textContent='Preparando vÃ­deos encontrados...';
      ...
      document.querySelector('#creativeLibraryStatus').textContent=`${creativeVideos.length} vÃ­deo(s) encontrados â€¢ pesquisa retomada â€¢ nada foi armazenado`;
  ...
  const renderCreativePresets=(selectedId='')=>{presetSelect.innerHTML='<option value="">Selecione uma predefiniÃ§Ã£o</option>'+creativePresets.map(preset=>`<option value="${escapeHtml(preset.id)}">${escapeHtml(preset.name)}</option>`).join('');presetSelect.value=selectedId;deletePreset.disabled=!selectedId;deleteAllPresets.disabled=!creativePresets.length;savePreset.textContent=selectedId?'Atualizar termos':'Salvar nova'};
  const loadCreativePresets=async()=>{try{const response=await fetch('/api/creative-search-settings'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'NÃ£o foi possÃ­vel carregar os termos salvos.');creativePresets=Array.isArray(payload.presets)?payload.presets:[];renderCreativePresets()}catch(error){showPresetStatus(error.message,'error')}};
  presetSelect.onchange=()=>{const preset=creativePresets.find(item=>item.id===presetSelect.value);deletePreset.disabled=!preset;savePreset.textContent=preset?'Atualizar termos':'Salvar nova';if(!preset){presetName.value='';return showPresetStatus('Edite os termos acima e salve uma nova predefiniÃ§Ã£o.')}termsField.value=(preset.terms||[]).join('\n');presetName.value=preset.name;showPresetStatus(`Edite, adicione ou remova termos e clique em â€œAtualizar termosâ€.`,'success')};
  savePreset.onclick=async()=>{const id=presetSelect.value,name=presetName.value.trim(),terms=[...new Set(termsField.value.split(/\r?\n|,/).map(value=>value.trim()).filter(Boolean))];if(!name)return showPresetStatus('Digite um nome para a predefiniÃ§Ã£o.','error');if(!terms.length)return showPresetStatus('Informe pelo menos um termo para salvar. Para apagar a predefiniÃ§Ã£o, use â€œExcluir estaâ€.','error');savePreset.disabled=true;showPresetStatus(id?'Atualizando...':'Salvando...');try{const response=await fetch('/api/creative-search-settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,name,terms})}),preset=await response.json();if(!response.ok)throw new Error(preset.error||'NÃ£o foi possÃ­vel salvar.');creativePresets=creativePresets.filter(item=>item.id!==preset.id&&item.name.toLowerCase()!==preset.name.toLowerCase());creativePresets.push(preset);renderCreativePresets(preset.id);showPresetStatus(id?'PredefiniÃ§Ã£o atualizada com sucesso.':'Termos salvos com sucesso.','success')}catch(error){showPresetStatus(error.message,'error')}finally{savePreset.disabled=false}};
  deletePreset.onclick=async()=>{const id=presetSelect.value,preset=creativePresets.find(item=>item.id===id);if(!preset||!confirm(`Excluir a predefiniÃ§Ã£o â€œ${preset.name}â€?`))return;deletePreset.disabled=true;showPresetStatus('Excluindo...');try{const response=await fetch(`/api/creative-search-settings?id=${encodeURIComponent(id)}`,{method:'DELETE'}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'NÃ£o foi possÃ­vel excluir.');creativePresets=creativePresets.filter(item=>item.id!==id);renderCreativePresets();presetName.value='';showPresetStatus('PredefiniÃ§Ã£o excluÃ­da.','success')}catch(error){showPresetStatus(error.message,'error');deletePreset.disabled=false}};
  deleteAllPresets.onclick=async()=>{if(!creativePresets.length||!confirm(`Excluir permanentemente todas as ${creativePresets.length} predefiniÃ§Ãµes de termos?`))return;deleteAllPresets.disabled=true;showPresetStatus('Excluindo todas...');try{const response=await fetch('/api/creative-search-settings?all=1',{method:'DELETE'}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'NÃ£o foi possÃ­vel excluir todas.');creativePresets=[];renderCreativePresets();presetName.value='';showPresetStatus('Todas as predefiniÃ§Ãµes foram excluÃ­das.','success')}catch(error){showPresetStatus(error.message,'error');deleteAllPresets.disabled=false}};
  loadCreativePresets();
  ...
  form.onsubmit=async event=>{event.preventDefault();const terms=[...new Set(document.querySelector('#creativeSearchTerms').value.split(/\r?\n|,/).map(value=>value.trim()).filter(Boolean))],limit=Math.min(30,Math.max(1,Number(document.querySelector('#creativeResultLimit').value)||10)),preferNoText=document.querySelector('#creativePreferNoText').checked;if(!terms.length)return alert('Informe pelo menos um termo, usando uma linha para cada pesquisa.');if(terms.length*limit>150)return alert('O mÃ¡ximo por execuÃ§Ã£o Ã© 150 vÃ­deos. Reduza os termos ou a quantidade por termo.');const button=form.querySelector('[type="submit"]'),progressTitle=progress.querySelector('strong'),progressDetail=progress.querySelector('small');button.disabled=true;progress.hidden=false;progressTitle.textContent='Pesquisando vÃ­deos...';progressDetail.textContent='A anÃ¡lise quadro a quadro estÃ¡ desativada.';document.querySelector('#creativeLibraryStatus').textContent=`Pesquisando ${terms.length} termo(s)...`;try{const response=await fetch('/api/creative-search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({terms,limit,platform:document.querySelector('#creativeSearchPlatform').value,content_type:document.querySelector('#creativeContentType').value,sorting:document.querySelector('#creativeSearchSorting').value,period:document.querySelector('#creativeSearchPeriod').value})}),started=await response.json();if(!response.ok)throw new Error(started.error||'A pesquisa nÃ£o foi iniciada.');let payload=started;while(payload.status!=='complete'){await new Promise(resolve=>setTimeout(resolve,2500));const statusResponse=await fetch(`/api/creative-audit/status?id=${encodeURIComponent(started.id)}`);payload=await statusResponse.json();if(!statusResponse.ok)throw new Error(payload.error||'A pesquisa foi interrompida.');progressTitle.textContent='Preparando resultados...';progressDetail.textContent='A pesquisa estÃ¡ sendo processada no servidor.';document.querySelector('#creativeLibraryStatus').textContent='Preparando vÃ­deos encontrados...'};creativeVideos=payload.approved||[];renderCreativeLibrary();if(preferNoText&&creativeVideos.length){progressTitle.textContent='Priorizando capas sem texto...';progressDetail.textContent='Lendo somente a primeira imagem de cada vÃ­deo.';creativeVideos=await prioritizeCreativeCoversWithoutText(creativeVideos,(done,total)=>{document.querySelector('#creativeLibraryStatus').textContent=`Verificando capas: ${done} de ${total}`});renderCreativeLibrary()}document.querySelector('#creativeLibraryStatus').textContent=preferNoText?`${creativeVideos.length} vÃ­deo(s) encontrados â€¢ capas sem texto priorizadas â€¢ vÃ­deo completo nÃ£o analisado`:(payload.visual_audit_enabled===false?`${creativeVideos.length} vÃ­deo(s) encontrados â€¢ anÃ¡lise visual desativada â€¢ nada foi armazenado`:`${creativeVideos.length} vÃ­deo(s) aprovados â€¢ ${payload.rejected||0} sem relaÃ§Ã£o removido(s) â€¢ nada foi armazenado`);clear.hidden=false;downloadSelected.hidden=!creativeVideos.some(video=>video.download_url)}catch(error){document.querySelector('#creativeLibraryStatus').textContent=error.message;document.querySelector('#creativeLibrary').innerHTML=`<div class="creative-library-empty"><strong>NÃ£o foi possÃ­vel concluir</strong><span>${escapeHtml(error.message)}</span></div>`}finally{progress.hidden=true;button.disabled=false}};
  ...
  document.querySelector('#creativeLibrary').onclick=async event=>{const preview=event.target.closest('[data-preview-creative]');if(preview){const video=creativeVideos[Number(preview.dataset.previewCreative)];preview.outerHTML=`<div class="creative-thumb creative-embed"><iframe src="${escapeHtml(video.embed_url)}" title="Preview do ${video.platform==='instagram'?'Instagram':'TikTok'}" allow="encrypted-media; picture-in-picture" allowfullscreen loading="lazy"></iframe></div>`;return}const download=event.target.closest('.creative-video-actions a.primary');if(download){event.preventDefault();const index=Number(download.closest('[data-creative-index]')?.dataset.creativeIndex),video=creativeVideos[index];if(!video?.download_url)return;download.style.pointerEvents='none';download.textContent='Preparando...';try{const response=await fetch(video.download_url);if(!response.ok)throw new Error('NÃ£o foi possÃ­vel preparar este vÃ­deo.');const url=URL.createObjectURL(await response.blob()),link=document.createElement('a');link.href=url;link.download=`${video.platform||'video'}-${video.id}.mp4`;link.click();setTimeout(()=>URL.revokeObjectURL(url),30000)}catch(error){alert(error.message)}finally{download.style.pointerEvents='';download.textContent='Baixar vÃ­deo'}}};
  downloadSelected.onclick=async()=>{const selected=[...document.querySelectorAll('[data-select-creative]:checked')].map(input=>creativeVideos[Number(input.dataset.selectCreative)]).filter(video=>video?.download_url);if(!selected.length)return alert('Selecione pelo menos um vÃ­deo com download disponÃ­vel.');downloadSelected.disabled=true;try{for(const video of selected){const response=await fetch(video.download_url);if(!response.ok)throw new Error(`NÃ£o foi possÃ­vel baixar ${video.title||video.id}.`);const url=URL.createObjectURL(await response.blob()),link=document.createElement('a');link.href=url;link.download=`${video.platform||'video'}-${video.id}.mp4`;link.click();setTimeout(()=>URL.revokeObjectURL(url),30000)}}catch(error){alert(error.message)}finally{downloadSelected.disabled=false}};
};
...
function reportAdsForAccount(accountId){const analysis=currentReportContext?.analysis?.accounts?.[accountId];return (analysis?.ads||[]).sort((a,b)=>String(a.campaign_name||'').localeCompare(String(b.campaign_name||''),'pt-BR')||Number(b.spend)-Number(a.spend))}
function reportAdStatus(ad){const status=String(ad.effective_status||ad.status||'UNKNOWN').toUpperCase();return {active:status==='ACTIVE',label:status==='ACTIVE'?'ATIVO':({PAUSED:'PAUSADO',CAMPAIGN_PAUSED:'CAMPANHA PAUSADA',ADSET_PAUSED:'CONJUNTO PAUSADO',ARCHIVED:'ARQUIVADO',DELETED:'EXCLUÃDO'}[status]||'STATUS INDISPONÃVEL')}}
function reportAdCard(accountId,ad,index){
  const key=reportAdKey(accountId,ad),edit=reportAdPresentationEdits.get(key)||{},status=reportAdStatus(ad),leads=Number(ad.results||0),cpl=ad.cost_per_result==null&&leads?Number(ad.spend)/leads:ad.cost_per_result,name=edit.name??ad.ad_name??'AnÃºncio sem nome',campaign=edit.campaign??ad.campaign_name??'Campanha sem nome';
  const preview=ad.preview_url||ad.thumbnail_url;
  return `<article class="report-ad-card" data-report-ad="${escapeHtml(key)}" data-account-id="${escapeHtml(accountId)}" data-ad-index="${index}"><button class="report-ad-creative" type="button" ${preview?`data-open-ad-preview="${escapeHtml(preview)}" aria-label="Ampliar prÃ©via do anÃºncio ${escapeHtml(name)}"`:'disabled'}>${preview?`<img src="${escapeHtml(preview)}" alt="PrÃ©via do anÃºncio ${escapeHtml(name)}" loading="lazy" referrerpolicy="no-referrer">`:`<span class="report-ad-no-preview" aria-label="PrÃ©via indisponÃ­vel"><span>â–§</span><b>PrÃ©via indisponÃ­vel</b><small>A Meta nÃ£o retornou uma imagem para esta peÃ§a.</small></span>`}<span class="report-ad-format">${escapeHtml(ad.format||'Formato nÃ£o identificado')}</span>${preview?'<span class="report-ad-expand">â›¶ Ampliar capa</span>':''}</button><div class="report-ad-content"><div class="report-ad-heading"><div><span class="report-ad-campaign" contenteditable="true" spellcheck="false" data-ad-edit="campaign">${escapeHtml(campaign)}</span><h4 contenteditable="true" spellcheck="false" data-ad-edit="name">${escapeHtml(name)}</h4><small>ID ${escapeHtml(ad.ad_id||'nÃ£o informado')}</small></div><span class="report-ad-status ${status.active?'active':'inactive'}"><i></i>${status.label}</span></div><div class="report-ad-metrics"><div><span>RESULTADOS</span><strong>${num(leads)}</strong><small>${escapeHtml(ad.result_type||'Resultado atribuÃ­do')}</small></div><div><span>VALOR GASTO</span><strong>${brl(ad.spend)}</strong><small>Investimento auditado</small></div><div><span>CUSTO POR RESULTADO</span><strong>${cpl==null?'â€”':brl(cpl)}</strong><small>Gasto Ã· leads</small></div></div><div class="report-ad-actions"><span>AlteraÃ§Ãµes de texto afetam somente a exportaÃ§Ã£o.</span><button type="button" data-reset-ad>Restaurar</button><button type="button" class="primary" data-download-ad>Baixar este anÃºncio</button></div></div></article>`
}
let reportAdsModalTrigger=null;
function openReportAdsModal(accountId,trigger){
  const context=currentReportContext,row=context?.payload.accounts?.[accountId],modal=document.querySelector('#reportAdsModal');
  if(!row||!modal||!context.selectedIds.includes(accountId))return;
  const ads=reportAdsForAccount(accountId),title=row.name||row.id;
  reportAdsModalTrigger=trigger||document.activeElement;
  document.querySelector('#reportAdsModalTitle').textContent=title;
  document.querySelector('#reportAdsModalMeta').textContent=`${context.from.toLocaleDateString('pt-BR')} a ${context.to.toLocaleDateString('pt-BR')} â€¢ ${ads.length} ${ads.length===1?'anÃºncio':'anÃºncios'} no perÃ­odo`;
  document.querySelector('#reportAdsModalBody').innerHTML=`<section class="report-ads-detail"><div class="report-ads-title"><div><span>DETALHAMENTO POR ANÃšNCIO</span><h4>Campanhas e peÃ§as veiculadas</h4><p>AnÃºncios retornados pela Meta no perÃ­odo, inclusive resultados atribuÃ­dos sem gasto. O status indica a situaÃ§Ã£o atual.</p></div><b>${ads.length} ${ads.length===1?'anÃºncio':'anÃºncios'}</b></div><div class="report-ad-list">${ads.map((ad,index)=>reportAdCard(accountId,ad,index)).join('')||'<p class="report-ads-empty">Nenhum anÃºncio teve veiculaÃ§Ã£o no perÃ­odo selecionado.</p>'}</div></section>`;
  modal.classList.add('open');modal.setAttribute('aria-hidden','false');document.body.classList.add('report-ads-modal-open');document.querySelector('#closeReportAdsModal').focus();
}
function closeReportAdsModal(){
  const modal=document.querySelector('#reportAdsModal');if(!modal?.classList.contains('open'))return;
  closeReportAdPreview();modal.classList.remove('open');modal.setAttribute('aria-hidden','true');document.body.classList.remove('report-ads-modal-open');document.querySelector('#reportAdsModalBody').innerHTML='';reportAdsModalTrigger?.focus?.();reportAdsModalTrigger=null;
}
function reportAdDownloadName(ad){return `${safeReportFileName(ad.campaign_name||'Campanha')} - ${safeReportFileName(ad.ad_name||'AnÃºncio')}.png`}
let html2CanvasLoader=null;
function loadHtml2Canvas(){
  if(window.html2canvas)return Promise.resolve(window.html2canvas);
  if(html2CanvasLoader)return html2CanvasLoader;
  html2CanvasLoader=new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js';script.async=true;script.onload=()=>window.html2canvas?resolve(window.html2canvas):reject(new Error('Biblioteca de imagem indisponÃ­vel.'));script.onerror=()=>reject(new Error('NÃ£o foi possÃ­vel carregar o gerador de imagem.'));document.head.appendChild(script)});
...
async function downloadReportAdCard(card){
  ...
  try{const render=await loadHtml2Canvas(),canvas=await render(card,{scale:2,useCORS:true,backgroundColor:'#ffffff',logging:false});canvas.toBlob(blob=>blob&&downloadBlob(blob,reportAdDownloadName(ad)),'image/png')}catch(error){alert(`NÃ£o foi possÃ­vel baixar este anÃºncio: ${error.message}`)}finally{button.disabled=false;button.textContent='Baixar este anÃºncio'}
}
...
function closeReportAdPreview(){document.querySelector('.report-ad-lightbox')?.remove();document.body.classList.remove('report-preview-open')}
document.addEventListener('click',event=>{const trigger=event.target.closest('[data-open-ad-preview]');if(!trigger)return;const card=trigger.closest('[data-report-ad]'),accountId=card?.dataset.accountId,ad=reportAdsForAccount(accountId)[Number(card?.dataset.adIndex)],name=ad?.ad_name||'AnÃºncio';closeReportAdPreview();const lightbox=document.createElement('div');lightbox.className='report-ad-lightbox';lightbox.setAttribute('role','dialog');lightbox.setAttribute('aria-modal','true');lightbox.setAttribute('aria-label',`PrÃ©via ampliada de ${name}`);lightbox.innerHTML=`<div class="report-ad-lightbox-panel"><div class="report-ad-lightbox-head"><div><span>PRÃ‰VIA DO CRIATIVO</span><strong>${escapeHtml(name)}</strong><small>Clique na imagem para visualizar todos os detalhes da capa.</small></div><button type="button" data-close-ad-preview aria-label="Fechar prÃ©via">Ã—</button></div><div class="report-ad-lightbox-stage"><img src="${escapeHtml(trigger.dataset.openAdPreview)}" alt="Capa completa do anÃºncio ${escapeHtml(name)}" referrerpolicy="no-referrer"></div></div>`;document.body.appendChild(lightbox);document.body.classList.add('report-preview-open');lightbox.querySelector('[data-close-ad-preview]').focus()});
document.addEventListener('click',event=>{if(event.target.matches('[data-close-ad-preview],.report-ad-lightbox'))closeReportAdPreview()});
document.addEventListener('click',event=>{const trigger=event.target.closest('[data-view-report-ads]');if(trigger)openReportAdsModal(trigger.dataset.viewReportAds,trigger)});
...
async function persistAccountProfiles(){
  const response=await fetch('/api/account-profiles',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(accountProfiles)});
  const result=await response.json();if(!response.ok)throw new Error(result.error||'NÃ£o foi possÃ­vel salvar o perfil.');
  cacheAccountProfiles();
}
async function loadAccountProfiles(){
  accountProfiles=readAccountProfiles();
  try{const response=await fetch('/api/account-profiles');if(response.ok){const remote=normalizeAccountProfiles(await response.json());accountProfiles=remote}}catch{}
...
function applyActiveAccountProfile(sync=true){
  ...
  if(!profile||!accountCatalogReady)return;
...
function installAccountProfilesUI(){
  ...
  #accountProfileControl{position:relative;z-index:900;display:flex;align-items:center;gap:.45rem;min-height:3.15rem;padding:.3rem .4rem .3rem .7rem;background:var(--profile-bg);border:1px solid var(--profile-line);border-radius:var(--profile-radius);color:var(--profile-text)}
  .profile-select-wrap{display:grid;gap:0;line-height:1}.profile-select-wrap label{font-size:.55rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--profile-muted)}.profile-select-wrap select{min-width:8.5rem;max-width:11rem;border:0;background:transparent;color:inherit;font:inherit;font-size:.82rem;font-weight:700;line-height:1.25;outline:none}.profile-select-wrap small{display:none}.profile-icon-button svg{display:block;margin:auto}
  ...
  document.querySelector('#saveAccountProfile').onclick=async event=>{const name=document.querySelector('#profileName').value.trim(),ids=[...document.querySelectorAll('#profileAccountList input:checked')].map(input=>input.value),status=document.querySelector('#profileStatus');if(!name){status.textContent='Informe um nome para o perfil.';document.querySelector('#profileName').focus();return}if(!ids.length){status.textContent='Selecione pelo menos uma conta de anÃºncio.';return}let profile=creatingAccountProfile?null:activeAccountProfile();if(profile){profile.name=name;profile.accountIds=ids}else{profile={id:`profile_${Date.now().toString(36)}`,name,accountIds:ids};accountProfiles.items.push(profile);accountProfiles.activeId=profile.id}creatingAccountProfile=false;status.textContent='Salvando perfil...';try{await persistAccountProfiles()}catch(error){status.textContent=error.message;return}renderProfileSelector();applyActiveAccountProfile();status.textContent='Perfil salvo.';setTimeout(closeProfileManager,450)};
  document.querySelector('#deleteAccountProfile').onclick=async()=>{const profile=activeAccountProfile();if(!profile||!confirm(`Excluir o perfil â€œ${profile.name}â€? As contas de anÃºncio nÃ£o serÃ£o excluÃ­das.`))return;accountProfiles.items=accountProfiles.items.filter(item=>item.id!==profile.id);accountProfiles.activeId=accountProfiles.items[0]?.id||null;try{await persistAccountProfiles()}catch(error){document.querySelector('#profileStatus').textContent=error.message;return}renderProfileSelector();if(activeAccountProfile())applyActiveAccountProfile();else{selectedAccountIds=new Set(accounts.map(account=>account.id));renderSummary();renderAccounts(document.querySelector('#searchInput')?.value||'')}closeProfileManager()};
  document.querySelector('#refreshProfileAccounts').onclick=async event=>{const status=document.querySelector('#profileStatus');status.textContent='Buscando novas contas na Meta...';setButtonLoading(event.currentTarget,true,'Atualizando contas...');const result=await findMetaAccounts(false);renderProfileManager();status.textContent=result?'Lista de contas atualizada.':document.querySelector('#accountSearchStatus').textContent;setButtonLoading(event.currentTarget,false)};
...
let commentsInitialized=false,commentsData=[],commentsPendingDelete=[];
const commentsSelectedIds=()=>[...document.querySelectorAll('[data-comment-select]:checked')].map(input=>input.value);
function updateCommentsSelection(){const selected=commentsSelectedIds(),all=[...document.querySelectorAll('[data-comment-select]')],toggle=document.querySelector('#commentsSelectAll');document.querySelector('#commentsSelectedCount').textContent=`${selected.length} ${selected.length===1?'selecionado':'selecionados'}`;document.querySelector('#commentsBulkBar').hidden=!all.length;toggle.checked=all.length>0&&selected.length===all.length;toggle.indeterminate=selected.length>0&&selected.length<all.length}
function commentsInitials(name){return String(name||'?').trim().split(/\s+/).slice(0,2).map(part=>part[0]).join('').toUpperCase()||'?'}
function visibleCommentsData(){const visibility=document.querySelector('#commentsVisibility').value,query=document.querySelector('#commentsSearch').value.trim().toLocaleLowerCase('pt-BR');return commentsData.map(item=>({...item,comments:item.comments.filter(comment=>{const hidden=comment.is_hidden===true;if(visibility==='visible'&&hidden)return false;if(visibility==='hidden'&&!hidden)return false;return !query||[item.ad.name,item.account_name,item.ad.campaign_name,item.ad.adset_name,comment.message,comment.from?.name].some(value=>String(value||'').toLocaleLowerCase('pt-BR').includes(query))})})).filter(item=>item.comments.length||(!query&&item.deleted_count>0))}
let commentsOpenAd=null,commentsReturnFocus=null,commentsModerating=false,commentsDialogTab='current',commentsArchiveRequest=0;
const commentAdKey=item=>item.account_id+':'+item.ad.id+':'+(item.platform||'facebook');
const commentDate=ad=>ad.published_time?'Publicado em '+new Date(ad.published_time).toLocaleDateString('pt-BR'):ad.created_time?'AnÃºncio criado em '+new Date(ad.created_time).toLocaleDateString('pt-BR'):'Data de publicaÃ§Ã£o indisponÃ­vel';
function renderComments(){
 const rows=visibleCommentsData(),root=document.querySelector('#commentsResults'),count=rows.reduce((sum,item)=>sum+item.comments.length,0);
 document.querySelector('#commentsResultsSummary').textContent=`${count} comentÃ¡rio(s) em ${rows.length} anÃºncio(s), conforme os filtros locais.`;
 root.innerHTML=rows.length?rows.map(item=>`<button type="button" class="comments-ad-card comments-ad-open" data-comment-ad="${escapeHtml(commentAdKey(item))}" aria-haspopup="dialog"><span class="comments-ad-head">${item.ad.thumbnail_url?`<img class="comments-ad-thumb" src="${escapeHtml(item.ad.thumbnail_url)}" alt="" referrerpolicy="no-referrer">`:'<span class="comments-ad-thumb"></span>'}<span class="comments-ad-copy"><strong>${escapeHtml(item.ad.name)}</strong><span>${escapeHtml(item.account_name)} Â· ${escapeHtml(item.ad.campaign_name)}</span><small>${escapeHtml(commentDate(item.ad))}</small></span><span class="comments-ad-meta"><span class="comments-status ${item.ad.effective_status==='ACTIVE'?'active':'inactive'}">${item.ad.effective_status==='ACTIVE'?'Ativo':item.ad.effective_status==='PAUSED'?'Pausado':'Inativo'}</span><span class="comments-count">${item.comments.length} comentÃ¡rio(s)${item.deleted_count?' Â· '+item.deleted_count+' apagado(s)':''}</span><span class="comment-platform ${item.platform==='instagram'?'comment-platform-instagram':''}">${item.platform==='instagram'?'Instagram':item.platform==='facebook'?'Facebook Â· PÃ¡gina':'Origem nÃ£o informada'}</span><span class="comments-open-label">Ver comentÃ¡rios â†’</span></span></span></button>`).join(''):'<div class="comments-empty"><strong>Nenhum comentÃ¡rio encontrado</strong><span>Selecione uma pÃ¡gina ou altere os filtros.</span></div>';
 renderCommentsDialog();
}
function renderCommentsDialog(){
 const item=visibleCommentsData().find(item=>commentAdKey(item)===commentsOpenAd),list=document.querySelector('#commentsDialogList');
 const original=commentsData.find(item=>commentAdKey(item)===commentsOpenAd);
 if(commentsDialogTab==='deleted'){void renderDeletedComments(original);return}
 document.querySelector('#commentsDialogTitle').textContent=original?.ad.name||'ComentÃ¡rios do anÃºncio';
 document.querySelector('#commentsDialogMeta').textContent=original?commentDate(original.ad)+' Â· '+(original.ad.effective_status==='ACTIVE'?'Ativo':'Inativo'):'';
 list.innerHTML=item?item.comments.map(comment=>`<div class="comment-row" data-comment-row="${escapeHtml(comment.id)}"><label class="comment-check"><input type="checkbox" value="${escapeHtml(comment.id)}" data-comment-select aria-label="Selecionar comentÃ¡rio de ${escapeHtml(comment.from?.name||'autor nÃ£o identificado')}"></label>${commentAvatar(comment)}<div class="comment-copy"><strong>${escapeHtml(comment.from?.name||'Nome nÃ£o disponibilizado pela Meta')}${comment.is_hidden?'<span class="comment-hidden-label">Oculto</span>':''}</strong><span class="comment-platform ${comment.platform==='instagram'?'comment-platform-instagram':''}">${comment.platform==='instagram'?'Instagram':comment.platform==='facebook'?'Facebook Â· PÃ¡gina':'Origem nÃ£o informada'}</span><p>${escapeHtml(comment.message||'ComentÃ¡rio sem texto')}</p><small>${comment.created_time?new Date(comment.created_time).toLocaleString('pt-BR'):'Data indisponÃ­vel'} â€¢ ${num(comment.like_count)} curtidas â€¢ ${num(comment.comment_count)} respostas</small></div><div class="comment-actions">${comment.permalink_url?`<a class="comment-action" href="${escapeHtml(comment.permalink_url)}" target="_blank" rel="noopener noreferrer">Abrir</a>`:''}<button class="comment-action" type="button" data-comment-action="${comment.is_hidden?'unhide':'hide'}" data-comment-id="${escapeHtml(comment.id)}">${comment.is_hidden?'Reexibir':'Ocultar'}</button><button class="comment-action danger" type="button" data-comment-action="delete" data-comment-id="${escapeHtml(comment.id)}">Excluir</button></div></div>`).join(''):'<p class="comments-dialog-empty">Nenhum comentÃ¡rio para exibir neste anÃºncio com os filtros atuais.</p>';
 updateCommentsSelection();bindCommentAvatarFallback();
 document.querySelectorAll('[data-comment-action],[data-comments-action]').forEach(button=>button.disabled=commentsModerating);
}
function commentAvatar(comment){const raw=comment.from?.picture?.data?.url;let url='';try{const parsed=new URL(raw);if(parsed.protocol==='https:')url=parsed.href}catch{}return `<span class="comment-avatar" aria-hidden="true">${url?`<img src="${escapeHtml(url)}" alt="" referrerpolicy="no-referrer" loading="lazy">`:escapeHtml(commentsInitials(comment.from?.name))}</span>`}
function bindCommentAvatarFallback(){document.querySelectorAll('#commentsDialogList .comment-avatar img').forEach(img=>img.onerror=()=>{img.parentElement.textContent='?'})}
function setCommentsTabs(){for(const [id,tab] of [['commentsCurrentTab','current'],['commentsDeletedTab','deleted']]){const button=document.getElementById(id);button.setAttribute('aria-selected',String(commentsDialogTab===tab));button.tabIndex=commentsDialogTab===tab?0:-1}document.querySelector('#commentsDialogPanel').setAttribute('aria-labelledby',commentsDialogTab==='current'?'commentsCurrentTab':'commentsDeletedTab');document.querySelector('#commentsDialogBulk').hidden=commentsDialogTab==='deleted'}
async function renderDeletedComments(item){const request=++commentsArchiveRequest,list=document.querySelector('#commentsDialogList');list.innerHTML='<p class="comments-dialog-empty" role="status">Carregando histÃ³rico de exclusÃµes...</p>';updateCommentsSelection();if(!item)return;
 try{const response=await fetch('/api/meta-comment-archive?'+new URLSearchParams({account:item.account_id,post:item.ad.post_id})),data=await response.json();if(!response.ok)throw new Error(data.error||'NÃ£o foi possÃ­vel carregar o histÃ³rico.');if(request!==commentsArchiveRequest||commentsDialogTab!=='deleted')return;
 list.innerHTML='<p class="comments-archive-note">ExclusÃµes confirmadas pelo Traffic pocket a partir desta atualizaÃ§Ã£o. ExclusÃµes anteriores ou feitas fora do app nÃ£o tÃªm conteÃºdo recuperÃ¡vel aqui. HistÃ³rico limitado aos 2.000 registros mais recentes do seu espaÃ§o.</p>'+(data.comments.length?data.comments.map(record=>{const comment=record.comment;return `<article class="comment-row comment-deleted">${commentAvatar(comment)}<div class="comment-copy"><strong>${escapeHtml(comment.from?.name||'Nome nÃ£o disponibilizado pela Meta')}</strong><span class="comment-platform ${comment.platform==='instagram'?'comment-platform-instagram':''}">${comment.platform==='instagram'?'Instagram':'Facebook Â· PÃ¡gina'}</span><p>${escapeHtml(comment.message||'ComentÃ¡rio sem texto')}</p><small>ExcluÃ­do em ${new Date(record.deleted_at).toLocaleString('pt-BR')} Â· ExclusÃ£o confirmada pela Meta</small></div><span class="comment-deleted-label">Apagado</span></article>`}).join(''):'<p class="comments-dialog-empty">Nenhuma exclusÃ£o registrada para esta publicaÃ§Ã£o.</p>');bindCommentAvatarFallback();
 }catch(error){if(request===commentsArchiveRequest&&commentsDialogTab==='deleted')list.innerHTML=`<p class="comments-dialog-empty" role="alert">${escapeHtml(error.message)} <button type="button" id="commentsArchiveRetry">Tentar novamente</button></p>`;document.querySelector('#commentsArchiveRetry')?.addEventListener('click',()=>renderDeletedComments(item))}
}
function openCommentsDialog(key,trigger){commentsDialogTab='current';setCommentsTabs();commentsOpenAd=key;commentsReturnFocus=trigger;document.querySelector('#commentsDialogStatus').textContent='';document.querySelector('#commentsDialogStatus').classList.remove('error');renderCommentsDialog();document.querySelector('#commentsAdDialog').showModal();document.querySelector('#commentsDialogClose').focus()}
function closeCommentsDialog(){commentsArchiveRequest++;if(!document.querySelector('#commentsConfirmModal').hidden){closeCommentsConfirm();return}document.querySelector('#commentsAdDialog').close();commentsOpenAd=null;document.querySelector('#commentsDialogList').innerHTML='';updateCommentsSelection();if(commentsReturnFocus?.isConnected)commentsReturnFocus.focus();else document.querySelector('#commentsPageSelect').focus()}
let commentsInstagramRequest=0;
function showCommentsInstagram(data){const el=document.querySelector('#commentsInstagramStatus');el.textContent=data.message;el.dataset.state=data.state;document.querySelector('#commentsInstagramAuthorize').hidden=data.state!=='permissions'}
async function loadCommentsInstagram(){const page=commentsPage,request=++commentsInstagramRequest;showCommentsInstagram({state:'loading',message:page?'Verificando Instagram vinculado...':'Selecione uma pÃ¡gina para verificar o Instagram.'});if(!page)return;try{const response=await fetch('/api/meta-comment-instagram?'+new URLSearchParams({page:page.id})),data=await response.json();if(!response.ok)throw new Error(data.error||'Falha ao verificar Instagram.');if(request===commentsInstagramRequest&&commentsPage?.id===page.id)showCommentsInstagram(data)}catch{if(request===commentsInstagramRequest)showCommentsInstagram({state:'error',message:'NÃ£o foi possÃ­vel verificar o Instagram. Atualize a consulta para tentar novamente.'})}}
let commentsPage=null,commentsPages=[],commentsPageRequest=null,commentsPagesLoadedAt=0;
function renderCommentsPageList(){const list=document.querySelector('#commentsPageList'),query=document.querySelector('#commentsPageSearch').value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase(),pages=commentsPages.filter(page=>page.name.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().includes(query));list.innerHTML=pages.length?pages.map(page=>`<button type="button" class="comments-page-option ${commentsPage?.id===page.id?'selected':''}" data-comments-page="${escapeHtml(page.id)}" role="option" aria-selected="${commentsPage?.id===page.id}"><span class="comments-page-initial">${escapeHtml(page.name.trim().slice(0,1).toUpperCase())}</span><strong>${escapeHtml(page.name)}</strong>${commentsPage?.id===page.id?'<b aria-hidden="true">âœ“</b>':''}</button>`).join(''):'<p class="comments-page-empty">Nenhuma pÃ¡gina encontrada.</p>'}
function renderCommentsPages(){const select=document.querySelector('#commentsPageSelect'),trigger=document.querySelector('#commentsPageTrigger');select.innerHTML='<option value="">Selecione uma pÃ¡gina</option>'+commentsPages.map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.name)}</option>`).join('');if(commentsPage&&commentsPages.some(p=>p.id===commentsPage.id))select.value=commentsPage.id;else{commentsPage=null;commentsData=[];renderComments()}select.disabled=!commentsPages.length;trigger.disabled=!commentsPages.length;trigger.querySelector('span').textContent=commentsPage?.name||'Selecione uma pÃ¡gina';renderCommentsPageList()}
function fetchCommentsPages(){if(commentsPageRequest)return commentsPageRequest;const select=document.querySelector('#commentsPageSelect'),status=document.querySelector('#commentsPagesStatus');select.disabled=true;status.textContent='Atualizando suas pÃ¡ginas autorizadas...';commentsPageRequest=(async()=>{try{const response=await fetch('/api/meta-comment-pages'),data=await response.json();if(!response.ok)throw new Error(data.error||'NÃ£o foi possÃ­vel listar as pÃ¡ginas.');commentsPages=data.pages||[];const previous=commentsPage?.id,saved=userStorage.getItem('hurtz-comments-page');if(!commentsPage&&saved)commentsPage=commentsPages.find(p=>p.id===saved)||null;renderCommentsPages();commentsPagesLoadedAt=Date.now();void loadCommentsInstagram();const labels={pages_show_list:'listar pÃ¡ginas',pages_read_engagement:'ler publicaÃ§Ãµes',pages_read_user_content:'ler comentÃ¡rios',pages_manage_engagement:'moderar comentÃ¡rios'},missing=data.missingPermissions||[];status.textContent=missing.length?'Falta autorizar: '+missing.map(p=>labels[p]||p).join(', ')+'. Se vocÃª jÃ¡ confirmou no Facebook, essa conexÃ£o nÃ£o recebeu as permissÃµes. O administrador do Tryv CRM deve verificar o acesso delas para usuÃ¡rios em produÃ§Ã£o.':data.warnings?.length?'Algumas pÃ¡ginas nÃ£o puderam ser consultadas. Clique em Atualizar pÃ¡ginas para tentar novamente.':commentsPages.length?commentsPages.length+' pÃ¡ginas disponÃ­veis. A lista Ã© atualizada automaticamente.':'Nenhuma pÃ¡gina disponÃ­vel. Clique em Autorizar pÃ¡ginas e revise as pÃ¡ginas e empresas liberadas para o Tryv CRM.';document.querySelector('#commentsPagesAuthorize').textContent=missing.length?'Autorizar pÃ¡ginas':'Revisar autorizaÃ§Ã£o';if(commentsPage&&!previous&&!missing.includes('pages_read_user_content')&&!missing.includes('pages_read_engagement'))void loadAdComments();return data}catch(e){status.textContent=e.message;select.disabled=!commentsPages.length}finally{commentsPageRequest=null}})();return commentsPageRequest}
function refreshCommentsPagesIfNeeded(){if(commentsInitialized&&!document.hidden&&!document.querySelector('#comments').hidden&&Date.now()-commentsPagesLoadedAt>60000)void fetchCommentsPages()}
document.addEventListener('visibilitychange',refreshCommentsPagesIfNeeded);
window.addEventListener('focus',refreshCommentsPagesIfNeeded);
setInterval(refreshCommentsPagesIfNeeded,300000);
async function loadCommentsHistory(){const root=document.querySelector('#commentsHistoryList');try{const response=await fetch('/api/meta-comment-history'),payload=await response.json();if(!response.ok)throw new Error();const labels={hide:'Ocultou',unhide:'Reexibiu',delete:'Excluiu'},events=payload.events||[];root.innerHTML=events.length?events.slice(0,50).map(event=>`<div class="comments-history-row"><strong>${new Date(event.created_at).toLocaleString('pt-BR')}</strong><span>${labels[event.action]||event.action} ${event.comment_ids?.length||0} comentÃ¡rio(s)</span><span>${event.success||0} concluÃ­do(s)</span><span class="${event.failed?'failed':''}">${event.failed||0} falha(s)</span></div>`).join(''):'<div class="comments-history-empty">Nenhuma aÃ§Ã£o registrada.</div>'}catch{root.innerHTML='<div class="comments-history-empty">HistÃ³rico indisponÃ­vel neste momento.</div>'}}
async function loadAdComments(){const page=commentsPage,button=document.querySelector('#commentsFilterForm [type="submit"]'),status=document.querySelector('#commentsRequestStatus'),view=document.querySelector('#comments');if(!page){status.textContent='Selecione uma pÃ¡gina para buscar os comentÃ¡rios.';document.querySelector('#commentsPageSelect').focus();return}button.disabled=true;view.classList.add('is-loading');status.classList.remove('error');status.textContent='Consultando anÃºncios, publicaÃ§Ãµes e comentÃ¡rios na Meta...';try{const query=new URLSearchParams({page:page.id,status:document.querySelector('#commentsAdStatus').value,days:document.querySelector('#commentsPeriod').value}),response=await fetch(`/api/meta-comments?${query}`),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao consultar comentÃ¡rios.');if(commentsPage?.id!==page.id)return;commentsData=payload.ads||[];if(payload.instagram){commentsInstagramRequest++;showCommentsInstagram(payload.instagram)}renderComments();status.textContent=`Consulta concluÃ­da: ${payload.comments||0} comentÃ¡rio(s) em ${commentsData.length} anÃºncio(s).${payload.warnings?.length?` ${payload.warnings.length} consulta(s) de publicaÃ§Ãµes nÃ£o concluÃ­da(s).`:''}`}catch(error){status.textContent=error.message;status.classList.add('error');commentsData=[];renderComments()}finally{button.disabled=false;view.classList.remove('is-loading')}}
async function moderateComments(action,ids){
 if(!ids.length||commentsModerating)return;commentsModerating=true;
 const announce=(message,error=false)=>{for(const id of ['commentsRequestStatus','commentsDialogStatus']){const el=document.getElementById(id);el.textContent=message;el.classList.toggle('error',error)}};
 announce(`${action==='delete'?'Excluindo':action==='hide'?'Ocultando':'Reexibindo'} ${ids.length} comentÃ¡rio(s)...`);
 document.querySelectorAll('[data-comment-action],[data-comments-action]').forEach(button=>button.disabled=true);
 try{const response=await fetch('/api/meta-comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,comment_ids:ids})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'A moderaÃ§Ã£o nÃ£o foi concluÃ­da.');
 const succeeded=new Set((payload.results||[]).filter(row=>row.ok===true&&ids.includes(row.id)).map(row=>row.id));
 commentsData=commentsData.map(item=>({...item,deleted_count:(item.deleted_count||0)+(action==='delete'?item.comments.filter(c=>succeeded.has(c.id)).length:0),comments:item.comments.filter(comment=>action!=='delete'||!succeeded.has(comment.id)).map(comment=>succeeded.has(comment.id)&&action!=='delete'?{...comment,is_hidden:action==='hide'}:comment)}));
 renderComments();void loadCommentsHistory();const failed=ids.length-succeeded.size;
 const successMessage=succeeded.size?`${succeeded.size} comentÃ¡rio(s) ${action==='delete'?'excluÃ­do(s). ExclusÃ£o confirmada pela Meta; removido(s) desta lista':action==='hide'?'ocultado(s) com confirmaÃ§Ã£o da Meta':'reexibido(s) com confirmaÃ§Ã£o da Meta'}.` : '';
 announce(successMessage+(failed?` ${failed} comentÃ¡rio(s) nÃ£o alterado(s). `+((payload.results||[]).find(row=>!row.ok)?.error||'A Meta nÃ£o confirmou a operaÃ§Ã£o. Tente novamente.') : ''),failed>0);
 }catch(error){announce(error.message,true)}finally{commentsModerating=false;document.querySelectorAll('[data-comment-action],[data-comments-action]').forEach(button=>button.disabled=false);document.querySelector('#commentsDialogClose').focus()}
}
function closeCommentsConfirm(){const modal=document.querySelector('#commentsConfirmModal');modal.hidden=true;document.querySelector('#commentsConfirmInput').value='';document.querySelector('#commentsConfirmDelete').disabled=true;commentsPendingDelete=[];document.querySelector('[data-comments-action="delete"]')?.focus()}
function requestCommentsDelete(ids){if(!ids.length)return;commentsPendingDelete=ids;const modal=document.querySelector('#commentsConfirmModal');document.querySelector('#commentsConfirmDescription').textContent=`VocÃª estÃ¡ prestes a excluir permanentemente ${ids.length} comentÃ¡rio(s). Esta aÃ§Ã£o nÃ£o poderÃ¡ ser desfeita pelo dashboard.`;modal.hidden=false;document.querySelector('#commentsConfirmInput').focus()}
...
// Personal Facebook connection, backed by the authenticated Traffic pocket user.
...
async function personalRequest(route,options={}){
  ...
  if(!response.ok)throw Object.assign(new Error(payload.error||'NÃ£o foi possÃ­vel concluir a conexÃ£o.'),{status:response.status});
  return payload;
}
...
async function prepareFacebookLogin(){
  ...
  facebookNonce=result.nonce;facebookNonceAt=Date.now();facebookConnect.dataset.loginReady='true';
  facebookConnect.disabled=false;renderFacebookConnection();
}
function loadFacebookSdk(settings){
  return new Promise((resolve,reject)=>{
    if(window.FB){resolve();return}
    const timeout=setTimeout(()=>reject(new Error('O login do Facebook nÃ£o carregou. Verifique sua conexÃ£o e atualize a pÃ¡gina.')),20000);
    ...
    script.onerror=()=>{clearTimeout(timeout);reject(new Error('NÃ£o foi possÃ­vel carregar o login do Facebook.'))};
...
facebookConnect.onclick=()=>{
  ...
  try{window.FB.login(response=>{void(async()=>{
    if(!response.authResponse?.accessToken){facebookLoginBusy=false;renderFacebookConnection();facebookStatus.textContent=facebookSettings?.connected?'AutorizaÃ§Ã£o cancelada ou recusada. Sua conexÃ£o salva foi mantida.':'ConexÃ£o cancelada ou recusada. VocÃª pode tentar novamente.';try{await prepareFacebookLogin()}catch(error){facebookStatus.textContent=error.message}return}
    facebookStatus.textContent='Conectando suas contas de anÃºncio...';
    try{
      ...
      if(requestedAds){facebookStatus.textContent='ConexÃ£o atualizada. Verificando o gerenciamento de anÃºncios...';await prepareFacebookLogin();window.dispatchEvent(new Event('hurtz-ads-authorized'));return}
...
analyticsLogout.onclick=async()=>{if(window.HURTZ_LOCAL)originalStorage.setItem('hurtz-local-signed-out','1');
...
if(personalIdentity?.personal){
  ...
  void (async()=>{try{
    ...
    facebookStatus.textContent=facebookSettings.connected?`Conectado como ${facebookSettings.name}. Sua conexÃ£o estÃ¡ salva para os prÃ³ximos acessos.`:facebookSettings.rejected?'A Meta recusou a autorizaÃ§Ã£o salva para este aplicativo. Conecte novamente para atualizÃ¡-la.':facebookSettings.expired?'Sua autorizaÃ§Ã£o expirou. Conecte o Facebook novamente.':'Conecte seu Facebook e autorize a leitura das contas que deseja acompanhar.';
...
const campaignModule=await import('./campaign-manager-ui.js?v=20260916-review-fix');
...
modal.className='account-detail-page';modal.hidden=true;modal.removeAttribute('aria-hidden');document.querySelector('main').append(modal);
const accountContent=modal.querySelector('.modal');accountContent.className='account-detail-content';accountContent.removeAttribute('role');accountContent.removeAttribute('aria-modal');
const backAccount=document.querySelector('#closeModal');backAccount.textContent='â† Voltar Ã s contas';backAccount.setAttribute('aria-label','Voltar Ã s contas');backAccount.className='account-back';document.querySelector('#modalTitle').tabIndex=-1;
let accountReturnFocus=null,accountReturnScroll=0,accountPageSnapshot=new Map(),routingAccount=false;
const hideAccountPage=()=>{if(campaignManagerUI.busy())return false;campaignManagerUI.close();document.body.classList.remove('account-workspace');if(!modal.hidden)for(const [element,hidden] of accountPageSnapshot)element.hidden=hidden;modal.hidden=true;modal.classList.remove('open');document.body.style.overflow='';return true};
const previousDashboardView=showDashboardView;showDashboardView=function(view){if(view==='account'){routeAccount();return}if(!hideAccountPage())return;previousDashboardView(view)};
openAccount=function(id,plan=false,fromHistory=false){const next=accounts.find(a=>a.id===id);if(!next||campaignManagerUI.busy())return;if(modal.hidden){accountReturnFocus=document.activeElement;accountReturnScroll=window.scrollY}else hideAccountPage();routingAccount=true;previousDashboardView('accounts');accountPageSnapshot=new Map([...document.querySelector('main').children].filter(x=>x!==modal).map(x=>[x,x.hidden]));for(const element of document.querySelector('main').children)if(element!==modal)element.hidden=true;modal.hidden=false;document.body.classList.add('account-workspace');modal.classList.add('open');selectedAccount=next;closeCampaignGoal();document.querySelector('#modalTitle').textContent=next.name;document.querySelector('#modalSubtitle').textContent=next.id+' Â· Conta de anÃºncio';fillPlan();syncModalDates();togglePlan(plan);setAccountTab(plan?'campaigns':'manage');if(plan)renderModal();document.body.style.overflow='';if(!fromHistory)history.pushState({account:id},'','?view=account&account='+encodeURIComponent(id));document.querySelectorAll('.sidebar nav a').forEach(a=>a.classList.toggle('active',a.getAttribute('href')?.endsWith('#accounts')));window.scrollTo({top:0,behavior:'instant'});document.querySelector('#modalTitle').focus({preventScroll:true});routingAccount=false};
...
})().catch(error=>{console.error("Falha ao iniciar o Traffic pocket");const status=document.querySelector("#accountSearchStatus");if(status)status.textContent="NÃ£o foi possÃ­vel iniciar. Atualize a pÃ¡gina."});
