...
function fetchCommentsPages(){if(commentsPageRequest)return commentsPageRequest;const select=document.querySelector('#commentsPageSelect'),status=document.querySelector('#commentsPagesStatus');select.disabled=true;status.textContent='Carregando suas páginas...';commentsPageRequest=(async()=>{try{const response=await fetch('/api/meta-comment-pages'),data=await response.json();if(!response.ok)throw new Error(data.error||'Não foi possível listar as páginas.');commentsPages=data.pages||[];renderCommentsPages();status.textContent=commentsPages.length?'':'Nenhuma página disponível. Autorize as páginas no Facebook.'}catch(e){status.textContent=e.message;select.disabled=!commentsPages.length}finally{commentsPageRequest=null}})();return commentsPageRequest}
