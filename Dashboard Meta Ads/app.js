...
const startOfWeeklyCycle=(date,startDay=1)=>{const d=new Date(date);d.setHours(12,0,0,0);d.setDate(d.getDate()-((d.getDay()-Number(startDay)+7)%7));return d};
...
async function loadCreditAccountPeriods(accountId){
  const account=accounts.find(item=>item.id===accountId),today=parseDate(iso(NOW)),weekStart=startOfWeeklyCycle(NOW,account?.plan?.weekStartDay),periods=[[today,today],[weekStart,today]];
...
function renderModal(){
  ...
  const credit=m.credit,today=parseDate(iso(NOW)),weekStart=startOfWeeklyCycle(NOW,a.plan.weekStartDay),todaySpend=performanceForPeriod(a,today,today),weekSpend=performanceForPeriod(a,weekStart,today),dailyUsed=todaySpend.complete?todaySpend.spend:null,weeklyUsed=weekSpend.complete?weekSpend.spend:null,dailyRemaining=dailyUsed==null?null:Math.max(0,a.plan.dailyLimit-dailyUsed),weeklyRemaining=weeklyUsed==null?null:Math.max(0,a.plan.weeklyLimit-weeklyUsed),dailyRatio=dailyUsed!=null&&a.plan.dailyLimit>0?dailyUsed/a.plan.dailyLimit:null,weeklyRatio=weeklyUsed!=null&&a.plan.weeklyLimit>0?weeklyUsed/a.plan.weeklyLimit:null,cycleLabel=Number(a.plan.weekStartDay)===0?'Domingo a domingo':'Segunda a domingo';
  ...
  document.querySelector('#modalSummary').innerHTML=(credit?[
    ...
    ['Gasto no ciclo',brlExact(weeklyUsed),`Soma desde o início do ciclo ${cycleLabel.toLowerCase()}; provisória enquanto o dia atual estiver aberto.`],
  ...
  document.querySelector('#planStrip').innerHTML=credit?`
    ...
    <div>${metricLabel('Ciclo semanal','Período de sete dias escolhido para reiniciar o acompanhamento.')}<strong>${cycleLabel}</strong></div>`:`
...
function renderNetBudgetPreview(){const credit=document.querySelector('#paymentType').value==='credit',deposit=Number(document.querySelector('#depositAmount').value)||0,daily=Number(document.querySelector('#dailyLimit').value)||0,weekly=Number(document.querySelector('#weeklyLimit').value)||0,cycle=Number(document.querySelector('#weekStartDay').value)===0?'domingo a domingo':'segunda a domingo';document.querySelector('#netBudgetPreview').innerHTML=credit?`<span>Limite diário: <b>${brl(daily)}</b></span><strong>Disponível no ciclo ${cycle}: ${brl(weekly)}</strong>`:`<span>Taxa de 12,15%: <b>${brl(feeAmount(deposit))}</b></span><strong>Disponível para anúncios: ${brl(netBudget(deposit))}</strong>`}
