...
function updateCommentsAccountSummary(){const checked=document.querySelectorAll('#commentsAccountOptions input:checked').length,total=accounts.length;document.querySelector('#commentsAccountSummary').textContent=`${checked} de ${total} contas selecionadas`;document.querySelector('#commentsToggleAccounts').textContent=checked===total?'Limpar seleção':'Selecionar todas'}
function renderCommentsAccounts(){let stored=[];try{stored=JSON.parse(userStorage.getItem('hurtz-comments-accounts')||'[]')}catch{}const saved=new Set(stored),selected=saved.size?saved:new Set([...selectedAccountIds]);document.querySelector('#commentsAccountOptions').innerHTML=accounts.map(account=>`<label><input type="checkbox" value="${account.id}" ${selected.has(account.id)?'checked':''}><span title="${escapeHtml(account.name)}">${escapeHtml(account.name)}</span></label>`).join('');updateCommentsAccountSummary()}
...
async function loadAdComments(){const accountIds=[...document.querySelectorAll('#commentsAccountOptions input:checked')].map(input=>input.value),button=document.querySelector('#commentsFilterForm [type="submit"]'),status=document.querySelector('#commentsRequestStatus'),view=document.querySelector('#comments');if(!accountIds.length){status.textContent='Selecione pelo menos uma conta de anúncio.';status.classList.add('error');return}userStorage.setItem('hurtz-comments-accounts',JSON.stringify(accountIds));button.disabled=true;view.classList.add('is-loading');status.classList.remove('error');status.textContent='Consultando anúncios, publicações e comentários na Meta...';try{const query=new URLSearchParams({accounts:accountIds.join(','),status:document.querySelector('#commentsAdStatus').value,days:document.querySelector('#commentsPeriod').value}),response=await fetch(`/api/meta-comments?${query}`),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao consultar comentários.');commentsData=payload.ads||[];renderComments();status.textContent=`Consulta concluída: ${payload.comments||0} comentário(s) em ${commentsData.length} anúncio(s).${payload.warnings?.length?` ${payload.warnings.length} publicação(ões) sem permissão de leitura.`:''}`}catch(error){status.textContent=error.message;status.classList.add('error');commentsData=[];renderComments()}finally{button.disabled=false;view.classList.remove('is-loading')}}
