...
function renderNativeTasks(){
  ...
  const completedColumn=nativeTasks.columns.find(column=>/conclu/i.test(column.title)),completed=nativeTasks.tasks.filter(task=>task.column_id===completedColumn?.id).length,overdue=nativeTasks.tasks.filter(task=>task.due_date&&task.column_id!==completedColumn?.id&&new Date(`${task.due_date}T23:59:59`)<NOW).length;
  ...
  board.innerHTML=nativeTasks.columns.map(column=>{
    ...
    return `<section class="task-column" data-task-column="${column.id}"><header class="task-column-header"><div class="task-column-title"><button class="task-column-name" type="button" data-rename-column="${column.id}" title="Clique para renomear a etapa">${escapeHtml(column.title)}</button><span title="${total} tarefas nesta etapa">${total}</span></div><button class="task-column-menu" type="button" data-delete-column="${column.id}" title="Excluir etapa vazia">×</button></header><div class="task-list">${items.map(task=>taskCardMarkup(task,completedColumn)).join('')}${inlineNewColumnId===column.id?`<article class="task-card task-card-editing task-new-card">${taskInlineForm(null,column.id)}</article>`:''}</div><button class="task-add-card" type="button" data-quick-task="${column.id}" ${inlineNewColumnId===column.id?'disabled':''}><span>+</span> Adicionar Cartão</button></section>`;
  ...
  board.querySelectorAll('[data-rename-column]').forEach(button=>button.onclick=async()=>{const column=nativeTasks.columns.find(item=>item.id===button.dataset.renameColumn),title=prompt('Nome da etapa:',column?.title||'');if(!title?.trim()||title.trim()===column?.title)return;await taskApi('/api/task-columns',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:column.id,title:title.trim(),position:column.position})});await loadNativeTasks()});
...
function initializeTaskStructures(){
  ...
  document.querySelectorAll('[data-create-structure]').forEach(button=>button.onclick=async()=>{const type=button.dataset.createStructure;let url,payload;if(type==='column'){url='/api/task-columns';payload={title:document.querySelector('#newTaskColumnTitle').value,position:nativeTasks.columns.length}}else if(type==='project'){url='/api/task-projects';payload={title:document.querySelector('#newTaskProjectTitle').value}}else if(type==='module'){url='/api/task-modules';payload={project_id:document.querySelector('#newTaskModuleProject').value,title:document.querySelector('#newTaskModuleTitle').value}}else{url='/api/task-cycles';payload={project_id:document.querySelector('#newTaskCycleProject').value,title:document.querySelector('#newTaskCycleTitle').value,starts_on:document.querySelector('#newTaskCycleStart').value,ends_on:document.querySelector('#newTaskCycleEnd').value}}status.textContent='Salvando...';try{await taskApi(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});status.textContent='Criado com sucesso.';await loadNativeTasks();open()}catch(error){status.textContent=error.message}});
}
