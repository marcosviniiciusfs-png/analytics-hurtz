...
function hydrateAnalysisLoaders(){clearAnalysisLoaders();if(!window.lottie)return;document.querySelectorAll('#analysis .analysis-lottie-animation').forEach(container=>analysisLoaderAnimations.push(window.lottie.loadAnimation({container,renderer:'svg',loop:true,autoplay:true,path:'toaster.json'})))}
