...
let nativeTasks={columns:[],tasks:[],projects:[],modules:[],cycles:[],subtasks:[],comments:[],attachments:[],activities:[]},tasksInitialized=false,draggedTaskId=null,taskView='board',activeTaskDetail='checklist';
...
function renderNativeTasks(){
  ...
  board.innerHTML=nativeTasks.columns.map(column=>{
    ...
    return `<section class="task-column" data-task-column="${column.id}"><header class="task-column-header"><div class="task-column-title"><i class="task-column-dot"></i><h3>${escapeHtml(column.title)}</h3><span>${total}</span></div><button class="task-column-quick-add" type="button" data-quick-task="${column.id}" title="Adicionar tarefa nesta etapa">+</button></header><div class="task-list">${items.length?items.map(task=>{const isOverdue=task.due_date&&task.column_id!==completedColumn?.id&&new Date(`${task.due_date}T23:59:59`)<NOW,subtasks=nativeTasks.subtasks.filter(item=>item.task_id===task.id),progress=taskProgress(task);return `<article class="task-card" draggable="true" data-task-id="${task.id}"><div class="task-card-top"><span class="task-priority ${task.priority}">${task.priority==='high'?'Alta':task.priority==='low'?'Baixa':'Média'}</span><button class="task-edit" type="button" aria-label="Editar tarefa" data-edit-task="${task.id}">•••</button></div><h4>${escapeHtml(task.title)}</h4>${task.description?`<p>${escapeHtml(task.description)}</p>`:''}${task.labels?.length?`<div class="task-labels">${task.labels.map(label=>`<span class="task-label">${escapeHtml(label)}</span>`).join('')}</div>`:''}<div class="task-meta">${task.project_id?`<span>▦ ${escapeHtml(taskPropertyTitle(nativeTasks.projects,task.project_id))}</span>`:''}${task.assignee?`<span>👤 ${escapeHtml(task.assignee)}</span>`:''}${task.due_date?`<span class="${isOverdue?'task-overdue':''}">◷ ${taskDateLabel(task.due_date)}</span>`:''}${subtasks.length?`<span>✓ ${progress}%</span>`:''}</div></article>`}).join(''):'<div class="task-empty">Solte uma tarefa aqui</div>'}</div></section>`;
  }).join('');
...
async function loadNativeTasks(){
  ...
  try{nativeTasks=await taskApi('/api/tasks');const select=document.querySelector('#taskAssigneeFilter'),selected=select.value,assignees=[...new Set(nativeTasks.tasks.map(task=>task.assignee).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR'));select.innerHTML='<option value="">Todos os responsáveis</option>'+assignees.map(name=>`<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('');if(assignees.includes(selected))select.value=selected;
    ...
    fill('#taskProjectFilter','Todos os projetos',nativeTasks.projects||[]);fill('#taskModuleFilter','Todos os módulos',nativeTasks.modules||[]);fill('#taskCycleFilter','Todos os ciclos',nativeTasks.cycles||[]);renderNativeTasks()
...
async function initializeNativeTasks(){
  ...
  document.querySelector('#taskForm').onsubmit=async event=>{
    ...
    button.disabled=true;status.textContent='Salvando...';try{await taskApi('/api/tasks',{method:id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeTaskModal();await loadNativeTasks()}catch(error){status.textContent=error.message}finally{button.disabled=false}
  };
...
function initializeTaskStructures(){
  ...
  document.querySelectorAll('[data-create-structure]').forEach(button=>button.onclick=async()=>{const type=button.dataset.createStructure;let url,payload;if(type==='project'){url='/api/task-projects';payload={title:document.querySelector('#newTaskProjectTitle').value}}else if(type==='module'){url='/api/task-modules';payload={project_id:document.querySelector('#newTaskModuleProject').value,title:document.querySelector('#newTaskModuleTitle').value}}else{url='/api/task-cycles';payload={project_id:document.querySelector('#newTaskCycleProject').value,title:document.querySelector('#newTaskCycleTitle').value,starts_on:document.querySelector('#newTaskCycleStart').value,ends_on:document.querySelector('#newTaskCycleEnd').value}}status.textContent='Salvando...';try{await taskApi(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});status.textContent='Criado com sucesso.';await loadNativeTasks();open()}catch(error){status.textContent=error.message}});
}
function renderTaskStructures(){
  const target=document.querySelector('#taskStructureExisting'),groups=[['Projetos','project',nativeTasks.projects],['Módulos','module',nativeTasks.modules],['Ciclos','cycle',nativeTasks.cycles]];
  ...
  target.querySelectorAll('[data-edit-structure]').forEach(button=>button.onclick=async()=>{const title=prompt('Novo nome:',button.dataset.title);if(!title?.trim())return;await taskApi(`/api/task-${button.dataset.editStructure}s`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:button.dataset.id,title})});await loadNativeTasks();renderTaskStructures()});
  target.querySelectorAll('[data-delete-structure]').forEach(button=>button.onclick=async()=>{if(!confirm('Excluir este item? As tarefas serão preservadas sem este vínculo.'))return;await taskApi(`/api/task-${button.dataset.deleteStructure}s?id=${button.dataset.id}`,{method:'DELETE'});await loadNativeTasks();renderTaskStructures()});
}
