...
async function initializeReports(){if(reportInitialized)return;reportInitialized=true;initializeReportAnimation();const yesterday=new Date(NOW);yesterday.setDate(yesterday.getDate()-1);document.querySelector('#reportDateFrom').value=iso(yesterday);document.querySelector('#reportDateTo').value=iso(yesterday);try{const response=await fetch('/api/meta-monitor-config'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao carregar contas');if(!accounts.length&&payload.accounts?.length)setReportMemberAccounts(payload.accounts)}catch(error){reportInitialized=false;document.querySelector('#reportStatus').textContent=error.message||'NÃ£o foi possÃ­vel carregar as contas monitoradas.'}}
...
function createPngReportEdit(accountId,displayName,row,groups,insights,context){
  const products=[...new Set(groups.map(group=>group.name.split(' - ')[0]).filter(name=>name!=='NÃƒO IDENTIFICADO'))].join(' â€¢ ')||'NÃƒO IDENTIFICADO';
  return {reportTitle:'RelatÃ³rio de Performance',accountName:displayName,period:`${context.from.toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})} a ${context.to.toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})}`,campaignsLabel:'Campanhas:',products,leadsLabel:'RESULTADOS',leadsValue:num(insights.totalResults),cplLabel:'CUSTO POR RESULTADO',cplValue:insights.overallCpl==null?'â€”':brl(insights.overallCpl),spendLabel:'VALOR GASTO',spendValue:brl(row.spend),groupTitle:'DESEMPENHO POR GRUPO DE CAMPANHA',confirmedLabel:'TOTAL CONFIRMADO',confirmedValue:num(insights.totalResults),confirmedUnit:'resultados',investmentText:`Investimento total de ${brl(row.spend)} no perÃ­odo analisado.`,groups:groups.map(group=>({name:group.name,results:group.complete?num(group.results):'â€”',cpl:group.complete&&group.results?brl(group.spend/group.results):'â€”'})),summaryTitle:'Resumo do perÃ­odo',summaryText:`${displayName} gerou ${num(insights.totalResults)} resultados confirmados, com investimento total de ${brl(row.spend)} entre ${context.from.toLocaleDateString('pt-BR')} e ${context.to.toLocaleDateString('pt-BR')}.`,goodTitle:'O que estÃ¡ bom',goodText:insights.good,improveTitle:'O que pode melhorar',improveText:insights.improve};
}
...
function renderPngReportFields(accountId=currentPngAccountId){
  ...
  root.innerHTML=`
    ...
    <fieldset><legend>MÃ©tricas principais</legend><div class="png-edit-grid metrics">
      ${pngEditorControl('Nome da mÃ©trica 1','leadsLabel',edit.leadsLabel)}${pngEditorControl('Valor da mÃ©trica 1','leadsValue',edit.leadsValue)}
      ${pngEditorControl('Nome da mÃ©trica 2','cplLabel',edit.cplLabel)}${pngEditorControl('Valor da mÃ©trica 2','cplValue',edit.cplValue)}
      ${pngEditorControl('Nome da mÃ©trica 3','spendLabel',edit.spendLabel)}${pngEditorControl('Valor da mÃ©trica 3','spendValue',edit.spendValue)}
    </div></fieldset>
    <fieldset><legend>Desempenho por grupo de campanha</legend><div class="png-edit-grid">
      ...
      ${pngEditorControl('RÃ³tulo do total','confirmedLabel',edit.confirmedLabel)}${pngEditorControl('Total confirmado','confirmedValue',edit.confirmedValue)}
    ...
    <fieldset><legend>Textos de anÃ¡lise</legend><div class="png-edit-grid">
      ${pngEditorControl('TÃ­tulo do resumo','summaryTitle',edit.summaryTitle)}${pngEditorControl('Resumo','summaryText',edit.summaryText,{wide:true,multiline:true})}
      ${pngEditorControl('TÃ­tulo â€” ponto positivo','goodTitle',edit.goodTitle)}${pngEditorControl('O que estÃ¡ bom','goodText',edit.goodText,{wide:true,multiline:true})}
      ${pngEditorControl('TÃ­tulo â€” melhoria','improveTitle',edit.improveTitle)}${pngEditorControl('O que pode melhorar','improveText',edit.improveText,{wide:true,multiline:true})}
    </div></fieldset>`;
}
...
function taskCardMarkup(task,completedColumn){
  ...
  return `<article class="task-card" draggable="true" data-task-id="${task.id}"><div class="task-card-heading"><span class="task-drag-handle" title="Arraste para mover">â ¿</span><button class="task-card-title" type="button" data-inline-edit-task="${task.id}">${escapeHtml(task.title)}</button><button class="task-card-settings ${menuOpen?'active':''}" type="button" data-task-card-menu="${task.id}" aria-label="OpÃ§Ãµes do cartÃ£o">âš™</button></div>${task.description?`<p>${escapeHtml(task.description)}</p>`:''}${task.labels?.length?`<div class="task-labels">${task.labels.map(label=>`<span class="task-label">${escapeHtml(label)}</span>`).join('')}</div>`:''}<div class="task-meta">${task.assignee?`<span>ðŸ‘¤ ${escapeHtml(task.assignee)}</span>`:''}${task.due_date?`<span class="${isOverdue?'task-overdue':''}">â–£ ${taskDateLabel(task.due_date)}</span>`:''}${task.estimate_minutes?`<span>â—· ${taskEstimateLabel(task.estimate_minutes)}</span>`:''}${subtasks.length?`<span>âœ“ ${progress}%</span>`:''}</div>${menuOpen?`<div class="task-card-popover"><strong>OpÃ§Ãµes do cartÃ£o</strong><button type="button" data-inline-edit-task="${task.id}">âš™ Editar</button><button type="button" data-edit-task="${task.id}">â‹¯ Mais detalhes</button><button class="danger" type="button" data-delete-card="${task.id}">Ã— Excluir</button></div>`:''}</article>`;
}
function renderNativeTasks(){
  ...
  board.innerHTML=nativeTasks.columns.map(column=>{
    ...
    return `<section class="task-column task-column-${role}" data-task-column="${column.id}" data-column-role="${role}"><header class="task-column-header"><div class="task-column-title"><span class="task-column-name">${escapeHtml(column.title)}</span><small>${taskColumnRoleLabel(role)}</small><span title="${total} tarefas nesta etapa">${total}</span></div><button class="task-column-settings-button" type="button" data-configure-column="${column.id}" title="Configurar etapa" aria-label="Configurar etapa ${escapeHtml(column.title)}">âš™</button></header><div class="task-list">${items.map(task=>taskCardMarkup(task,completedColumn)).join('')}${inlineNewColumnId===column.id?`<article class="task-card task-card-editing task-new-card">${taskInlineForm(null,column.id)}</article>`:''}</div><button class="task-add-card" type="button" data-quick-task="${column.id}" ${inlineNewColumnId===column.id?'disabled':''}><span>+</span> Adicionar CartÃ£o</button></section>`;
...
function renderTaskStructures(){
  const target=document.querySelector('#taskStructureExisting'),groups=[['Etapas','column',nativeTasks.columns],['Projetos','project',nativeTasks.projects],['MÃ³dulos','module',nativeTasks.modules],['Ciclos','cycle',nativeTasks.cycles]];
...
async function personalRequest(route,options={}){
  ...
  if(!response.ok)throw new Error(payload.error||'NÃ£o foi possÃ­vel concluir a conexÃ£o.');
  return payload;
}
...
showDashboardView=function(view){
  ...
  title.textContent=accountsView?'Contas de anÃºncio':'VisÃ£o geral';
  subtitle.textContent=accountsView?'Consulte, filtre e abra cada conta de anÃºncio autorizada.':'Acompanhe os indicadores consolidados das contas selecionadas.';
};
