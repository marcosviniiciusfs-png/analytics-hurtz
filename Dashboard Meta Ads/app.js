...
renderCreativeLibrary=function(){
  ...
  root.innerHTML=creativeVideos.map((video,index)=>`<article class="creative-video-card" data-creative-index="${index}"><label class="creative-select"><input type="checkbox" checked data-select-creative="${index}"><span>Selecionar</span></label><button class="creative-thumb" type="button" data-preview-creative="${index}">${video.thumbnail_url?`<img src="${escapeHtml(video.thumbnail_url)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:'<span>▶</span>'}<b>Assistir preview</b></button><div class="creative-video-body"><div class="creative-video-tags"><span>${escapeHtml(video.search_term||'Busca relacionada')}</span><span>${video.duration?`${video.duration}s`:'Vídeo'}</span>${video.visual_verified?`<span class="verified">✓ ${video.detected_type==='car'?'Carro confirmado':'Imóvel confirmado'}</span>`:''}</div><h4>${escapeHtml(video.title||'Vídeo do TikTok')}</h4><a href="${escapeHtml(video.creator_url||video.video_url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(video.creator_name||'Criador não identificado')}</a><div class="creative-video-metrics"><span>Visualizações <b>${compactMetric(video.view_count)}</b></span><span>Curtidas <b>${compactMetric(video.like_count)}</b></span><span>Comentários <b>${compactMetric(video.comment_count)}</b></span></div><div class="creative-video-actions"><a href="${escapeHtml(video.video_url)}" target="_blank" rel="noopener noreferrer">Abrir no TikTok</a>${video.download_url?`<a class="primary" href="${escapeHtml(video.download_url)}" target="_blank" rel="noopener noreferrer" download="tiktok-${escapeHtml(video.id)}.mp4">Baixar vídeo</a>`:'<span>Download indisponível</span>'}</div></div></article>`).join('')||'<div class="creative-library-empty"><strong>Nenhum vídeo visualmente compatível</strong><span>Tente termos mais específicos ou escolha corretamente o conteúdo esperado.</span></div>';
};
initializeCreativeLibrary=function(){
  ...
  form.onsubmit=async event=>{event.preventDefault();const terms=[...new Set(document.querySelector('#creativeSearchTerms').value.split(/\r?\n|,/).map(value=>value.trim()).filter(Boolean))],limit=Math.min(30,Math.max(1,Number(document.querySelector('#creativeResultLimit').value)||10));if(!terms.length)return alert('Informe pelo menos um termo, usando uma linha para cada pesquisa.');if(terms.length*limit>150)return alert('O máximo por execução é 150 vídeos. Reduza os termos ou a quantidade por termo.');const button=form.querySelector('[type="submit"]');button.disabled=true;progress.hidden=false;document.querySelector('#creativeLibraryStatus').textContent=`Pesquisando e auditando ${terms.length} termo(s)...`;try{const response=await fetch('/api/creative-search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({terms,limit,content_type:document.querySelector('#creativeContentType').value,sorting:document.querySelector('#creativeSearchSorting').value,period:document.querySelector('#creativeSearchPeriod').value})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'A pesquisa não foi concluída.');creativeVideos=payload.videos||[];renderCreativeLibrary();document.querySelector('#creativeLibraryStatus').textContent=`${creativeVideos.length} vídeo(s) aprovados • ${payload.rejected||0} sem relação removido(s) • nada foi armazenado`;clear.hidden=false;downloadSelected.hidden=!creativeVideos.some(video=>video.download_url)}catch(error){document.querySelector('#creativeLibraryStatus').textContent=error.message;document.querySelector('#creativeLibrary').innerHTML=`<div class="creative-library-empty"><strong>Não foi possível concluir</strong><span>${escapeHtml(error.message)}</span></div>`}finally{progress.hidden=true;button.disabled=false}};
  clear.onclick=()=>{creativeVideos=[];renderCreativeLibrary();clear.hidden=true;downloadSelected.hidden=true};
  document.querySelector('#creativeLibrary').onclick=event=>{const preview=event.target.closest('[data-preview-creative]');if(preview){const video=creativeVideos[Number(preview.dataset.previewCreative)];preview.outerHTML=`<div class="creative-thumb creative-embed"><iframe src="${escapeHtml(video.embed_url)}" title="Preview do TikTok" allow="encrypted-media; picture-in-picture" allowfullscreen loading="lazy"></iframe></div>`}};
  downloadSelected.onclick=()=>{const selected=[...document.querySelectorAll('[data-select-creative]:checked')].map(input=>creativeVideos[Number(input.dataset.selectCreative)]).filter(video=>video?.download_url);if(!selected.length)return alert('Selecione pelo menos um vídeo com download disponível.');selected.forEach((video,index)=>setTimeout(()=>{const link=document.createElement('a');link.href=video.download_url;link.target='_blank';link.rel='noopener';link.download=`tiktok-${video.id}.mp4`;link.click()},index*700))};
};
