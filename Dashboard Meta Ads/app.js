...
async function initializeReports(){if(reportInitialized)return;reportInitialized=true;initializeReportAnimation();const yesterday=new Date(NOW);yesterday.setDate(yesterday.getDate()-1);document.querySelector('#reportDateFrom').value=iso(yesterday);document.querySelector('#reportDateTo').value=iso(yesterday);try{const response=await fetch('/api/meta-monitor-config'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao carregar contas');document.querySelector('#reportAccountOptions').innerHTML=(payload.accounts||[]).map(account=>`<label><input type="checkbox" value="${account.id}"><span>${escapeHtml(account.name)}</span></label>`).join('')}catch(error){reportInitialized=false;document.querySelector('#reportStatus').textContent=error.message||'Não foi possível carregar as contas monitoradas.'}}
