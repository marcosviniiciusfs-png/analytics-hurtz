...
function renderNativeTasks(){
  ...
  board.innerHTML=nativeTasks.columns.map(column=>{
    ...
    return `<section class="task-column" data-task-column="${column.id}"><header class="task-column-header"><div class="task-column-title"><i class="task-column-dot"></i><button class="task-column-name" type="button" data-rename-column="${column.id}" title="Renomear etapa">${escapeHtml(column.title)}</button><span>${total}</span></div><div class="task-column-actions"><button class="task-column-quick-add" type="button" data-quick-task="${column.id}" title="Adicionar tarefa nesta etapa">+</button><button class="task-column-menu" type="button" data-delete-column="${column.id}" title="Excluir etapa">×</button></div></header><div class="task-list">${items.length?items.map(task=>{const isOverdue=task.due_date&&task.column_id!==completedColumn?.id&&new Date(`${task.due_date}T23:59:59`)<NOW,subtasks=nativeTasks.subtasks.filter(item=>item.task_id===task.id),progress=taskProgress(task);return `<article class="task-card" draggable="true" data-task-id="${task.id}"><div class="task-card-top"><span class="task-priority ${task.priority}">${task.priority==='high'?'Alta':task.priority==='low'?'Baixa':'Média'}</span><button class="task-edit" type="button" aria-label="Editar tarefa" data-edit-task="${task.id}">•••</button></div><h4>${escapeHtml(task.title)}</h4>${task.description?`<p>${escapeHtml(task.description)}</p>`:''}${task.labels?.length?`<div class="task-labels">${task.labels.map(label=>`<span class="task-label">${escapeHtml(label)}</span>`).join('')}</div>`:''}<div class="task-meta">${task.project_id?`<span>▦ ${escapeHtml(taskPropertyTitle(nativeTasks.projects,task.project_id))}</span>`:''}${task.assignee?`<span>👤 ${escapeHtml(task.assignee)}</span>`:''}${task.due_date?`<span class="${isOverdue?'task-overdue':''}">◷ ${taskDateLabel(task.due_date)}</span>`:''}${task.estimate_minutes?`<span>◴ ${taskEstimateLabel(task.estimate_minutes)}</span>`:''}${subtasks.length?`<span>✓ ${progress}%</span>`:''}<span class="task-expiry">⌛ ${taskExpiryLabel(task.expires_at)}</span></div></article>`}).join(''):'<div class="task-empty">Solte uma tarefa aqui</div>'}</div></section>`;
  }).join('');
