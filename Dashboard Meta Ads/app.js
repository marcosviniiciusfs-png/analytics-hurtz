...
function openReportAdsModal(accountId,trigger){
  ...
  document.querySelector('#reportAdsModalMeta').textContent=`${context.from.toLocaleDateString('pt-BR')} a ${context.to.toLocaleDateString('pt-BR')} • ${ads.length} ${ads.length===1?'anúncio':'anúncios'} com gasto`;
  document.querySelector('#reportAdsModalBody').innerHTML=`<section class="report-ads-detail"><div class="report-ads-title"><div><span>DETALHAMENTO POR ANÚNCIO</span><h4>Campanhas e peças veiculadas</h4><p>Somente anúncios com gasto no período auditado desta conta.</p></div><b>${ads.length} ${ads.length===1?'anúncio':'anúncios'}</b></div><div class="report-ad-list">${ads.map((ad,index)=>reportAdCard(accountId,ad,index)).join('')||'<p class="report-ads-empty">Nenhum anúncio teve veiculação no período selecionado.</p>'}</div></section>`;
