...
async function loadNativeTasks(){
  ...
  try{nativeTasks=await taskApi('/api/tasks');const select=document.querySelector('#taskAssigneeFilter'),selected=select.value,assignees=[...new Set(nativeTasks.tasks.map(task=>task.assignee).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR'));select.innerHTML='<option value="">Todos os responsáveis</option>'+assignees.map(name=>`<option value="${escapeHtml(name)}">${escapeHtml(name)}</option>`).join('');if(assignees.includes(selected))select.value=selected;renderNativeTasks()}catch(error){board.innerHTML=`<div class="task-error">${escapeHtml(error.message)}</div>`}
}
...
async function initializeNativeTasks(){if(tasksInitialized)return;tasksInitialized=true;document.querySelector('#newTaskButton').onclick=()=>openTaskModal();document.querySelector('#refreshTasks').onclick=loadNativeTasks;document.querySelector('#taskSearch').oninput=renderNativeTasks;document.querySelector('#taskPriorityFilter').onchange=renderNativeTasks;document.querySelector('#taskAssigneeFilter').onchange=renderNativeTasks;document.querySelector('#closeTaskModal').onclick=closeTaskModal;document.querySelector('#taskModal').onclick=event=>{if(event.target.id==='taskModal')closeTaskModal()};document.querySelector('#taskForm').onsubmit=async event=>{event.preventDefault();const id=document.querySelector('#taskId').value,status=document.querySelector('#taskFormStatus'),button=event.submitter,payload={id,title:document.querySelector('#taskTitle').value,description:document.querySelector('#taskDescription').value,assignee:document.querySelector('#taskAssignee').value,priority:document.querySelector('#taskPriority').value,due_date:document.querySelector('#taskDueDate').value||null,labels:document.querySelector('#taskLabels').value.split(',').map(item=>item.trim()).filter(Boolean),column_id:document.querySelector('#taskColumn').value,position:nativeTasks.tasks.filter(task=>task.column_id===document.querySelector('#taskColumn').value).length};button.disabled=true;status.textContent='Salvando...';try{await taskApi('/api/tasks',{method:id?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeTaskModal();await loadNativeTasks()}catch(error){status.textContent=error.message}finally{button.disabled=false}};document.querySelector('#deleteTask').onclick=async()=>{const id=document.querySelector('#taskId').value;if(!id||!confirm('Excluir esta tarefa?'))return;const status=document.querySelector('#taskFormStatus');status.textContent='Excluindo...';try{await taskApi(`/api/tasks?id=${encodeURIComponent(id)}`,{method:'DELETE'});closeTaskModal();await loadNativeTasks()}catch(error){status.textContent=error.message}};await loadNativeTasks()}
