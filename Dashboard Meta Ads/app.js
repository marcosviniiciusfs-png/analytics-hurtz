...
function renderAccounts(filter=''){
  ...
  accountsBody.innerHTML=list.map(a=>{const m=metrics(a),p=performanceForPeriod(a),status=metaAccountStatus(a),spendState=spendHeat(a,p),balanceRatio=m.availableBudget?m.balance/m.availableBudget:0,balanceState=m.credit?{tone:'neutral',reason:'Conta no cartão: não existe saldo pré-pago para estimar.'}:m.balance<=0?{tone:'danger',reason:'Vermelho: o saldo estimado do planejamento chegou a zero.'}:balanceRatio<=.3?{tone:'warning',reason:`Amarelo: resta ${Math.round(balanceRatio*100)}% da verba líquida planejada.`}:{tone:'good',reason:`Verde: resta ${Math.round(balanceRatio*100)}% da verba líquida planejada.`},forecastState=m.credit?{tone:'neutral',reason:`Cartão: acompanhamento pelo teto diário de ${brl(a.plan.dailyLimit)} e semanal de ${brl(a.plan.weeklyLimit)}.`}:m.calendarDaysRemaining<=0?{tone:'danger',reason:'Vermelho: a data planejada de término já chegou.'}:m.calendarDaysRemaining<=3?{tone:'warning',reason:`Amarelo: faltam ${m.calendarDaysRemaining} dias para o fim planejado.`}:{tone:'good',reason:`Verde: faltam ${m.calendarDaysRemaining} dias para o fim planejado.`};return `<tr>
    ...
    ${m.credit?heatCell(`<strong>${brl(a.plan.dailyLimit)}/dia</strong><small class="table-sub">${brl(a.plan.weeklyLimit)} disponíveis na semana</small>`,'neutral',`Cartão: ciclo ${Number(a.plan.weekStartDay)===0?'domingo a domingo':'segunda a domingo'}.`,'number'):heatCell(`<strong>${brl(m.spendInCycle)}</strong><small class="table-sub">Projeção pelo teto diário</small>`,'neutral','Neutro: projeção calculada pelo limite diário e tempo transcorrido; não é gasto real da Meta.','number')}
...
initializeCreativeLibrary=function(){
  ...
  const presetSelect=document.querySelector('#creativePresetSelect'),presetName=document.querySelector('#creativePresetName'),presetStatus=document.querySelector('#creativePresetStatus'),savePreset=document.querySelector('#saveCreativePreset'),deletePreset=document.querySelector('#deleteCreativePreset'),termsField=document.querySelector('#creativeSearchTerms');let creativePresets=[];
  const showPresetStatus=(message,type='')=>{presetStatus.textContent=message;presetStatus.className=type};
  const renderCreativePresets=(selectedId='')=>{presetSelect.innerHTML='<option value="">Selecione uma predefinição</option>'+creativePresets.map(preset=>`<option value="${escapeHtml(preset.id)}">${escapeHtml(preset.name)}</option>`).join('');presetSelect.value=selectedId;deletePreset.disabled=!selectedId};
  ...
  presetSelect.onchange=()=>{const preset=creativePresets.find(item=>item.id===presetSelect.value);deletePreset.disabled=!preset;if(!preset)return;termsField.value=(preset.terms||[]).join('\n');presetName.value=preset.name;showPresetStatus(`Predefinição “${preset.name}” carregada.`,'success')};
  savePreset.onclick=async()=>{const name=presetName.value.trim(),terms=[...new Set(termsField.value.split(/\r?\n|,/).map(value=>value.trim()).filter(Boolean))];if(!name)return showPresetStatus('Digite um nome para a predefinição.','error');if(!terms.length)return showPresetStatus('Informe pelo menos um termo para salvar.','error');savePreset.disabled=true;showPresetStatus('Salvando...');try{const response=await fetch('/api/creative-search-settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,terms})}),preset=await response.json();if(!response.ok)throw new Error(preset.error||'Não foi possível salvar.');creativePresets=creativePresets.filter(item=>item.name.toLowerCase()!==preset.name.toLowerCase());creativePresets.push(preset);renderCreativePresets(preset.id);showPresetStatus('Termos salvos com sucesso.','success')}catch(error){showPresetStatus(error.message,'error')}finally{savePreset.disabled=false}};
