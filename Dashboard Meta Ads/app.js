...
function performanceForPeriod(a,from=globalPeriod.from,to=globalPeriod.to){
  ...
  if(live){
    if(!live.reconciled)return {complete:false,spend:null,leads:null,cpl:null,campaigns:null,source:live.error||'Conta e campanhas não reconciliaram'};
...
function forecastCell(a){
  const m=metrics(a);
  if(m.credit)return `<span class="forecast safe">Cartão de crédito</span><small class="forecast-date">Verba semanal ${brl(a.plan.weeklyLimit)}</small>`;
...
function openComparisonModal(index){const pair=analysisComparisons[index];if(!pair)return;const {a,b,winner}=pair,modal=document.querySelector('#comparisonModal');document.querySelector('#comparisonModalTitle').textContent=`${a.name} × ${b.name}`;document.querySelector('#comparisonModalPeriod').textContent=`${analysisFrom.toLocaleDateString('pt-BR')} até ${analysisTo.toLocaleDateString('pt-BR')} • ${analysisObjectiveName(a.objective)} • ${a.result_type}`;document.querySelector('#comparisonModalBody').innerHTML=`<div class="comparison-detail-verdict"><span>Melhor equilíbrio observado</span><strong>${escapeHtml(winner.name)}</strong><p>${escapeHtml(pair.hypothesis)}</p><small>${escapeHtml(pair.criteriaSummary)}. Associação observada, não prova de causalidade. Vagas de emprego não participam da comparação.</small></div><div class="comparison-detail-grid">${[a,b].map(row=>`<article><h3>${escapeHtml(row.name)}</h3><dl><div><dt>Resultados</dt><dd>${num(row.results)}</dd></div><div><dt>Valor usado</dt><dd>${brl(row.spend)}</dd></div><div><dt>Custo/resultado</dt><dd>${row.cpl==null?'—':brl(row.cpl)}</dd></div><div><dt>Resultados/R$ 100</dt><dd>${row.resultsPer100.toLocaleString('pt-BR',{maximumFractionDigits:2})}</dd></div><div><dt>CTR</dt><dd>${row.ctr.toLocaleString('pt-BR',{maximumFractionDigits:2})}%</dd></div><div><dt>Produto predominante</dt><dd>${escapeHtml(row.topProduct)} (${row.productShare.toFixed(0)}%)</dd></div><div><dt>Formato predominante</dt><dd>${escapeHtml(row.topFormat)} (${row.formatShare.toFixed(0)}%)</dd></div></dl></article>`).join('')}</div><div class="comparison-evidence"><h3>Distribuição observada</h3>${miniMetric('Resultados',a.results,b.results)}${miniMetric('Custo/resultado',a.cpl||0,b.cpl||0,true)}${miniMetric('Resultados por R$ 100',a.resultsPer100,b.resultsPer100)}</div>`;modal.classList.add('open');modal.setAttribute('aria-hidden','false')}
...
function renderObjectiveComparison(data){clearComparisonArrows();const target=document.querySelector('#analysisObjectiveComparison');if(analysisSelectedAccount!=='all'){target.innerHTML=emptyChart('Selecione “Comparar todas as contas” para visualizar comparações entre contas.');return}analysisComparisons=buildComparisons(data);target.innerHTML=analysisComparisons.length?`<div class="comparison-list">${analysisComparisons.map((pair,index)=>{const {a,b,winner}=pair,maxResults=Math.max(a.results,b.results,1);return `<button type="button" class="comparison-card" data-comparison-index="${index}"><div class="comparison-card-head"><span>${escapeHtml(analysisObjectiveName(a.objective))} • ${escapeHtml(a.result_type)}</span><b>Ver detalhes →</b></div><div class="comparison-sides"><div><strong>${escapeHtml(a.name)}</strong><span>${num(a.results)} resultados • ${a.cpl==null?'—':brl(a.cpl)}</span><i><em style="width:${a.results/maxResults*100}%"></em></i></div><span class="comparison-arrow" role="img" aria-label="Da conta com pior equilíbrio para a conta com melhor equilíbrio"><span class="comparison-arrow-animation" aria-hidden="true"></span></span><div class="winner"><strong>${escapeHtml(b.name)}</strong><span>${num(b.results)} resultados • ${b.cpl==null?'—':brl(b.cpl)}</span><i><em style="width:${b.results/maxResults*100}%"></em></i></div></div><p><strong>${escapeHtml(winner.name)}</strong> apresentou o melhor equilíbrio nos critérios observados. ${escapeHtml(pair.hypothesis)}</p><small>${escapeHtml(pair.criteriaSummary)} • vagas de emprego excluídas • métricas oficiais da Meta</small></button>`}).join('')}</div>`:emptyChart('Não há duas contas comparáveis com o mesmo objetivo e tipo de resultado auditado neste período. Campanhas de vagas de emprego são excluídas.');target.querySelectorAll('[data-comparison-index]').forEach(button=>button.onclick=()=>openComparisonModal(Number(button.dataset.comparisonIndex)));requestAnimationFrame(hydrateComparisonArrows)}
function renderDetailedAnalysis(data){clearAnalysisLoaders();const chip=document.querySelector('#analysisAuditChip'),name=analysisSelectedAccount==='all'?'Visão consolidada':(monitoredAnalysisAccounts.find(item=>item.id===analysisSelectedAccount)?.name||'Conta');document.querySelector('#analysisPeriod').textContent=`${name} • ${analysisFrom.toLocaleDateString('pt-BR')} até ${analysisTo.toLocaleDateString('pt-BR')}`;if(!data){chip.textContent=analysisLoading?'Consultando a Meta...':'Dados indisponíveis';chip.classList.add('pending');document.querySelector('#analysisKpis').innerHTML='';document.querySelector('#analysisGeographyHighlight').innerHTML='';['analysisObjectiveComparison','analysisAgeChart','analysisFormatChart','analysisGeographyChart','analysisPlacementChart','analysisCtrChart','analysisCplChart','analysisAdsRanking'].forEach(id=>document.querySelector(`#${id}`).innerHTML=analysisLoading?analysisLoadingChart():emptyChart('Não foi possível carregar esta análise.'));if(analysisLoading)requestAnimationFrame(hydrateAnalysisLoaders);return}chip.textContent=data.partial?'Análise parcial: há contas sem auditoria':data.reconciled?'Gasto reconciliado com a Meta':'Auditoria pendente';chip.classList.toggle('pending',!data.reconciled);const m=data.account;document.querySelector('#analysisKpis').innerHTML=analysisKpiMarkup([
...
function reportMemberName(name){return String(name).replace(/^(?:\[CA\]|CA(?=\s|[-–:]))\s*[-–:]?\s*/i,'').trim()||name}
...
function renderReportMembers(){const all=reportMemberAccounts(),sorted=[...all].sort((a,b)=>Number(b.selected)-Number(a.selected)),selected=all.filter(a=>a.selected);reportMemberSelection=new Set(selected.map(a=>a.id));if(all.length)try{userStorage.setItem("hurtz-report-selection-v1",JSON.stringify({scope:reportMemberScope,ids:[...reportMemberSelection]}))}catch{}
 document.querySelector('#reportMemberStrip').innerHTML=sorted.slice(0,5).map(a=>`<button type="button" class="account-member-avatar ${a.selected?'selected':''}" data-report-member="${escapeHtml(a.id)}" aria-pressed="${a.selected}" aria-label="${a.selected?'Remover':'Selecionar'} ${escapeHtml(a.name)}" title="${escapeHtml(a.name)} · ${escapeHtml(a.id)}"><span class="account-member-circle">${reportMemberAvatar(a)}<span class="account-member-mark" aria-hidden="true">${a.selected?'✓':'+'}</span></span><span class="account-member-name">${escapeHtml(reportMemberName(a.name))}</span></button>`).join('')+`<button type="button" id="reportMemberAdd" class="account-member-avatar account-member-add" aria-expanded="${reportMemberOpen}" aria-controls="reportMemberDropdown"><span class="account-member-circle" aria-hidden="true">${reportMemberOpen?'×':'+'}</span><span>Adicionar${selected.length>5?' (+'+(selected.length-5)+')':''}</span></button>`;
 document.querySelector('#reportMemberSummary').textContent=all.length?`${selected.length} de ${all.length} contas selecionadas`:'Nenhuma conta disponível. Conecte o Facebook ou revise o filtro principal.';
...
function renderReportMemberList(){const list=document.querySelector('#reportMemberList'),scrollTop=list.scrollTop;const query=document.querySelector('#reportMemberSearch').value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase(),rows=reportMemberAccounts().filter(a=>(a.name+' '+a.id).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().includes(query));list.innerHTML=rows.length?rows.map(a=>`<button type="button" class="account-member-option ${a.selected?'selected':''}" data-report-member="${escapeHtml(a.id)}" aria-pressed="${a.selected}"><span class="account-member-small" aria-hidden="true">${reportMemberAvatar(a)}</span><span class="account-member-copy"><strong>${escapeHtml(a.name)}</strong><small>${escapeHtml(a.id)}</small></span><span class="account-member-check" aria-hidden="true">${a.selected?'✓':''}</span></button>`).join(''):'<p class="account-member-empty">Nenhuma conta encontrada.</p>';list.scrollTop=scrollTop}
...
async function initializeReports(){if(reportInitialized)return;reportInitialized=true;initializeReportAnimation();const yesterday=new Date(NOW);yesterday.setDate(yesterday.getDate()-1);document.querySelector('#reportDateFrom').value=localIso(yesterday);document.querySelector('#reportDateTo').value=localIso(yesterday);try{const response=await fetch('/api/meta-monitor-config'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao carregar contas');if(!accounts.length&&payload.accounts?.length)setReportMemberAccounts(payload.accounts)}catch(error){reportInitialized=false;document.querySelector('#reportStatus').textContent=error.message||'Não foi possível carregar as contas monitoradas.'}}
...
initializeReports=async function(){
  ...
  syncReportAccountsWithDashboard();
  if(personalIdentity?.personal)await loadReportProductRules().catch(()=>{});
};
function reportGroupName(campaign){const product=inferredProduct(campaign.campaign_name||campaign.name),channel=campaign.no_delivery_in_period?'SEM VEICULAÇÃO NO PERÍODO':String(campaign.objective_label||'Resultado não identificado').toUpperCase();return `${product.toUpperCase()} - ${channel}`}
...
const reportConfiguredProduct=name=>{const text=normalizedCampaignText(name),rules=Array.isArray(globalThis.reportProductRules)?globalThis.reportProductRules:[],match=rules.map(rule=>({...rule,key:normalizedCampaignText(rule.match)})).filter(rule=>rule.key&&text.includes(rule.key)).sort((a,b)=>b.key.length-a.key.length)[0];return match?.label||inferredProduct(name)};
const reportIsUnidentified=product=>['NAO IDENTIFICADO','NA£O IDENTIFICADO'].includes(normalizedCampaignText(product));
reportGroupName=function(campaign){const original=String(campaign.campaign_name||campaign.name||'').trim(),product=reportConfiguredProduct(original),unknown=typeof reportIsUnidentified==='function'?reportIsUnidentified(product):/IDENTIFICADO/i.test(String(product));return original&&unknown?original:product.toUpperCase()};
function reportGroupsForRow(row){const groups=new Map();(row.campaigns||[]).forEach(campaign=>{const original=campaign.campaign_name||campaign.name,name=reportGroupName(campaign),product=reportConfiguredProduct(original),unknown=typeof reportIsUnidentified==='function'?reportIsUnidentified(product):/IDENTIFICADO/i.test(String(product)),group=groups.get(name)||{name,productLabel:unknown?name:product.toUpperCase(),spend:0,results:0,complete:true};group.spend+=Number(campaign.spend||0);if(campaign.results==null)group.complete=false;else group.results+=Number(campaign.results||0);groups.set(name,group)});return [...groups.values()]}
globalThis.reportProductRules=[];
const reportRulesButton=document.createElement('button'),reportRulesPanel=document.createElement('section');reportRulesButton.type='button';reportRulesButton.className='audit-chip';reportRulesButton.textContent='Regras de produtos';document.querySelector('.reports-heading')?.append(reportRulesButton);reportRulesPanel.className='report-product-rules';reportRulesPanel.hidden=true;reportRulesPanel.innerHTML='<div><strong>Regras de agrupamento por produto</strong><p>O texto é procurado no nome da campanha. O relatório exibe somente o nome do produto.</p></div><div id="reportProductRuleRows"></div><footer><button type="button" id="addReportProductRule">Adicionar regra</button><button type="button" id="saveReportProductRules">Salvar regras</button><span id="reportProductRuleStatus" role="status"></span></footer>';document.querySelector('.report-builder')?.after(reportRulesPanel);
const reportRulesStyle=document.createElement('style');reportRulesStyle.textContent='.report-product-rules{margin:0 0 18px;padding:16px 18px;border:1px solid #dbe6df;border-radius:14px;background:#fff}.report-product-rules p{margin:5px 0 14px;color:#667085;font-size:13px}.report-product-rule-row{display:grid;grid-template-columns:1fr 1fr auto;gap:8px;margin:8px 0}.report-product-rule-row input{min-width:0;padding:10px;border:1px solid #d7ddd8;border-radius:8px}.report-product-rule-row button,.report-product-rules footer button{padding:9px 12px;border:1px solid #d7ddd8;border-radius:8px;background:#fff;color:#243529;font-weight:700;cursor:pointer}.report-product-rules footer{display:flex;align-items:center;gap:8px;margin-top:12px;flex-wrap:wrap}.report-product-rules footer #saveReportProductRules{background:#3c8528;color:#fff;border-color:#3c8528}.report-product-rules footer span{font-size:12px;color:#667085}@media(max-width:700px){.report-product-rule-row{grid-template-columns:1fr}.report-product-rule-row button{justify-self:start}}';document.head.append(reportRulesStyle);
function renderReportProductRules(){const root=document.querySelector('#reportProductRuleRows'),items=globalThis.reportProductRules||[];root.innerHTML=items.length?items.map((item,index)=>`<div class="report-product-rule-row"><input value="${escapeHtml(item.match)}" data-product-rule="match" data-product-rule-index="${index}" placeholder="Ex.: NEW MEDIC" aria-label="Texto identificado na campanha"><input value="${escapeHtml(item.label)}" data-product-rule="label" data-product-rule-index="${index}" placeholder="Ex.: New Medic" aria-label="Nome exibido no relatório"><button type="button" data-remove-product-rule="${index}" aria-label="Remover regra">Remover</button></div>`).join(''):'<p>Nenhuma regra personalizada. Os produtos reconhecidos automaticamente continuam agrupados.</p>'}
async function loadReportProductRules(){const response=await fetch('/api/report-product-rules'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Não foi possível carregar as regras.');globalThis.reportProductRules=payload.items||[];renderReportProductRules()}
reportRulesButton.onclick=async()=>{reportRulesPanel.hidden=!reportRulesPanel.hidden;if(!reportRulesPanel.hidden)try{await loadReportProductRules();document.querySelector('#reportProductRuleStatus').textContent=''}catch(error){document.querySelector('#reportProductRuleStatus').textContent=error.message}};
reportRulesPanel.onclick=event=>{const remove=event.target.closest('[data-remove-product-rule]');if(remove){globalThis.reportProductRules.splice(Number(remove.dataset.removeProductRule),1);renderReportProductRules()}};document.querySelector('#reportProductRuleRows').oninput=event=>{const input=event.target,index=Number(input.dataset.productRuleIndex),field=input.dataset.productRule;if(field&&globalThis.reportProductRules[index])globalThis.reportProductRules[index][field]=input.value};
document.querySelector('#addReportProductRule').onclick=()=>{globalThis.reportProductRules.push({id:crypto.randomUUID(),match:'',label:''});renderReportProductRules();document.querySelector('#reportProductRuleRows input')?.focus()};document.querySelector('#saveReportProductRules').onclick=async event=>{const button=event.currentTarget,status=document.querySelector('#reportProductRuleStatus'),items=(globalThis.reportProductRules||[]).map(item=>({id:item.id||crypto.randomUUID(),match:String(item.match||'').trim(),label:String(item.label||'').trim()}));if(items.some(item=>item.match.length<2||item.label.length<2)){status.textContent='Preencha os dois campos de cada regra.';return}button.disabled=true;status.textContent='Salvando...';try{const response=await fetch('/api/report-product-rules',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({items})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Não foi possível salvar.');globalThis.reportProductRules=payload.items;reportPngEdits.clear();status.textContent='Regras salvas. Gere o relatório novamente para aplicar.'}catch(error){status.textContent=error.message}finally{button.disabled=false}};
...
function createPngReportEdit(accountId,displayName,row,groups,insights,context){
  const products=[...new Set(groups.map(group=>group.productLabel||group.name.split(' - ')[0]).filter(name=>name!=='NÃO IDENTIFICADO'))].join(' • ')||'NÃO IDENTIFICADO';
  ...
  const lowerCards=[
    {title:'Resumo do período',text:summaryText,icon:'document'},
    {title:'O que está bom',text:insights.good,icon:'clipboard'},
    {title:'O que pode melhorar',text:insights.improve,icon:'trend'}
  ];
  return {reportTitle:'Relatório de Performance',accountName:displayName,period:`${context.from.toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})} a ${context.to.toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})}`,campaignsLabel:'Campanhas:',products,leadsLabel:'RESULTADOS',leadsValue:num(insights.totalResults),cplLabel:'CUSTO POR RESULTADO',cplValue:insights.overallCpl==null?'—':brl(insights.overallCpl),spendLabel:'VALOR GASTO',spendValue:brl(row.spend),extraMetrics:[],lowerCards,groupWidth:55,groupTitle:'DESEMPENHO POR GRUPO DE CAMPANHA',confirmedLabel:'TOTAL CONFIRMADO',confirmedValue:num(insights.totalResults),confirmedUnit:'resultados',investmentText:`Investimento total de ${brl(row.spend)} no período analisado.`,groups:groups.map(group=>({name:group.name,results:group.complete?num(group.results):'—',cpl:group.complete&&group.results?brl(group.spend/group.results):'—'})),summaryTitle:'Resumo do período',summaryText,goodTitle:'O que está bom',goodText:insights.good,improveTitle:'O que pode melhorar',improveText:insights.improve};
}
...
function renderReport(payload,from,to,selectedIds){const rows=selectedIds.map(id=>payload.accounts?.[id]).filter(Boolean),failures=[];rows.forEach(row=>{const campaigns=row.campaigns||[],sum=campaigns.reduce((total,campaign)=>total+Number(campaign.spend||0),0);if(!row.reconciled||Math.abs(Number(row.spend||0)-sum)>=.01)failures.push(row.name||row.id)});if(rows.length!==selectedIds.length)failures.push('Conta sem retorno da API');if(failures.length)throw new Error(`Relatório bloqueado: auditoria incompleta em ${failures.join(', ')}.`);const accountHtml=rows.map(row=>{const groups=reportGroupsForRow(row);return `<section class="report-account" data-report-account="${escapeHtml(row.id)}"><div class="report-account-head"><div><span>CONTA DE ANÚNCIO</span><h3>${escapeHtml(row.name||row.id)}</h3></div><div class="report-account-head-actions"><b>${row.result_reconciled===false?'Gasto auditado · conferir atribuição de resultados':'Auditada com a Meta'}</b><button class="report-view-ads-button" type="button" data-view-report-ads="${escapeHtml(row.id)}">Visualizar anúncios</button><button type="button" data-create-png="${row.id}">Criar relatório em PNG</button></div></div><div class="report-groups">${groups.map(group=>`<article><strong>${escapeHtml(group.name)}</strong><div><span>RESULTADOS</span><b>${group.complete?num(group.results):'Sem dados de lead'}</b></div><div><span>Custo por resultado</span><b>${group.complete&&group.results?brl(group.spend/group.results):'—'}</b></div>${!group.complete?'<small>Coleta de resultado incompleta; nenhum valor foi estimado.</small>':''}</article>`).join('')||'<p>A Meta não retornou campanhas para esta conta e período.</p>'}</div><footer><span>Valor gasto</span><strong>${brl(row.spend)}</strong></footer></section>`}).join('');return `<div class="report-document"><header><div><p>HURTZ • PERFORMANCE</p><h1>Relatório Meta Ads</h1><span>Período: ${from.toLocaleDateString('pt-BR')} até ${to.toLocaleDateString('pt-BR')}</span></div><button type="button" id="printReportButton">Baixar / imprimir PDF</button></header>${accountHtml}<div class="report-audit-footer">Dados da API Meta • ${rows.length} conta(s) reconciliada(s) • valores não estimados</div></div>`}
...
function pngMetricIconControl(number,value,field,index,bottom=false){
  const icons=[['people','Pessoas'],['target','Alvo'],['money','Dinheiro'],['trend','Crescimento'],['eye','Olho'],['bank','Banco'],['document','Documento'],['clipboard','Prancheta'],['calendar','Calendário'],['campaign','Campanha']];
...
function pngLowerCardControls(card,index){
  const icons=[['document','Documento'],['clipboard','Prancheta'],['trend','Crescimento'],['eye','Olho'],['people','Pessoas'],['target','Alvo'],['money','Dinheiro'],['calendar','Calendário'],['campaign','Campanha']];
  return `<div class="png-lower-card-editor"><strong>Card ${index+1}</strong><label class="png-edit-control"><span>Título</span><input data-png-lower-card-index="${index}" data-png-lower-card-field="title" value="${escapeHtml(card.title)}"></label><label class="png-edit-control wide"><span>Texto</span><textarea rows="3" data-png-lower-card-index="${index}" data-png-lower-card-field="text">${escapeHtml(card.text)}</textarea></label><label class="png-edit-control"><span>Ícone</span><select data-png-lower-card-index="${index}" data-png-lower-card-field="icon">${icons.map(([key,label])=>`<option value="${key}" ${key===(card.icon||'document')?'selected':''}>${label}</option>`).join('')}</select></label><button type="button" class="png-group-add" data-remove-png-lower-card="${index}">Remover card ${index+1}</button></div>`;
}
...
function renderPngReportFields(accountId=currentPngAccountId){
  ...
  root.innerHTML=`
    ...
    <fieldset><legend>Card “Total confirmado”</legend><div class="png-edit-grid">
      ${pngEditorControl('Título do card','confirmedLabel',edit.confirmedLabel)}
      ${pngEditorControl('Número principal','confirmedValue',edit.confirmedValue)}
      ${pngEditorControl('Unidade abaixo do número','confirmedUnit',edit.confirmedUnit)}
...
document.querySelector('#pngReportFields').addEventListener('click',event=>{
  ...
  if(add){if(edit.lowerCards.length>=4)return;edit.lowerCards.push({title:'Novo card',text:'Escreva aqui a informação que deseja destacar.',icon:'document'});}else edit.lowerCards.splice(Number(remove.dataset.removePngLowerCard),1);
  renderPngReportFields();schedulePngEditorRedraw();
...
const alertAuthCopy={login:['Entrar no Traffic pocket','Entre com seu e-mail para continuar a consulta.'],signup:['Criar sua conta','Use um e-mail válido. Você precisará confirmá-lo antes de entrar.'],recover:['Recuperar senha','Enviaremos um link para o e-mail cadastrado.'],reset:['Definir nova senha','Escolha uma nova senha com pelo menos 8 caracteres.']};
...
const taskApi=async(url,options={})=>{const response=await fetch(url,options),payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||'Não foi possível atualizar as tarefas');if((options.method||'GET').toUpperCase()!=='GET')taskMutationRevision++;return payload};
...
const taskColumnRoleLabel=role=>({standard:'Normal',in_progress:'Em andamento',review:'Revisão',blocked:'Bloqueada',completed:'Concluída'})[role]||'Normal';
...
function openTaskColumnSettings(columnId){
  ...
  document.querySelector('#taskColumnSettingsSummary').innerHTML=`<span><b>${taskCount}</b> ${taskCount===1?'tarefa nesta etapa':'tarefas nesta etapa'}</span><span>Função atual: <b>${taskColumnRoleLabel(taskColumnRole(column))}</b></span>`;
  ...
  document.querySelector('#taskColumnDeleteHint').textContent=nativeTasks.columns.length<=1?'O quadro precisa manter pelo menos uma etapa.':taskCount>0?`Mova ou exclua ${taskCount===1?'a tarefa':'as tarefas'} antes de excluir esta etapa.`:'Esta etapa está vazia e pode ser excluída.';
...
function initializeTaskColumnSettings(){
  ...
  form.onsubmit=async event=>{event.preventDefault();const id=document.querySelector('#taskColumnSettingsId').value,column=nativeTasks.columns.find(item=>item.id===id),title=document.querySelector('#taskColumnSettingsTitle').value.trim(),role=form.querySelector('[name="taskColumnSettingsRole"]:checked')?.value,button=event.submitter;if(!column||!title||!role)return;button.disabled=true;status.textContent='Salvando configurações...';try{await taskApi('/api/task-columns',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,title,position:column.position,role})});closeTaskColumnSettings();await loadNativeTasks()}catch(error){status.textContent=error.message}finally{button.disabled=false}};
  document.querySelector('#deleteTaskColumn').onclick=async()=>{const id=document.querySelector('#taskColumnSettingsId').value,column=nativeTasks.columns.find(item=>item.id===id),taskCount=nativeTasks.tasks.filter(task=>task.column_id===id).length;if(!column||taskCount||nativeTasks.columns.length<=1)return;if(!confirm(`Excluir permanentemente a etapa “${column.title}”?`))return;status.textContent='Excluindo etapa...';try{await taskApi(`/api/task-columns?id=${encodeURIComponent(id)}`,{method:'DELETE'});closeTaskColumnSettings();await loadNativeTasks()}catch(error){status.textContent=error.message}};
}
...
function renderTaskDeadlineAlerts(){
  ...
  target.innerHTML=`<strong>${late?`${late} tarefa${late===1?'':'s'} atrasada${late===1?'':'s'}`:`${soon.length} prazo${soon.length===1?'':'s'} próximo${soon.length===1?'':'s'}`}</strong><span>${soon.slice(0,3).map(task=>escapeHtml(task.title)).join(' • ')}</span><button type="button" id="showDeadlineTasks">Ver tarefas</button>`;
...
function renderTaskAlternativeView(tasks){
  ...
  if(taskView==='list'){
    board.className='tasks-board task-list-view';board.innerHTML=`<div class="task-table"><div class="task-table-head"><span>Tarefa</span><span>Etapa</span><span>Projeto</span><span>Responsável</span><span>Prioridade</span><span>Prazo</span><span>Progresso</span></div>${tasks.map(task=>`<button type="button" class="task-table-row" data-edit-task="${task.id}"><span><b>${escapeHtml(task.title)}</b><small>${escapeHtml((task.labels||[]).join(' • '))}</small></span><span>${escapeHtml(taskPropertyTitle(nativeTasks.columns,task.column_id))}</span><span>${escapeHtml(taskPropertyTitle(nativeTasks.projects,task.project_id))}</span><span>${escapeHtml(task.assignee||'—')}</span><span class="task-priority ${task.priority}">${task.priority==='high'?'Alta':task.priority==='low'?'Baixa':'Média'}</span><span>${task.due_date?taskDateLabel(task.due_date):'—'}</span><span>${taskProgress(task)}%</span></button>`).join('')||'<div class="task-empty">Nenhuma tarefa encontrada.</div>'}</div>`;bindTaskEditButtons(board);return;
  }
  if(taskView==='calendar'){
    ...
    board.className='tasks-board task-calendar-view';board.innerHTML=`<div class="task-calendar-grid">${Object.entries(groups).map(([date,items])=>`<section><header><b>${taskDateLabel(date)}</b><span>${items.length}</span></header>${items.map(task=>`<button type="button" data-edit-task="${task.id}"><i class="${task.priority}"></i><span>${escapeHtml(task.title)}</span><small>${escapeHtml(task.assignee||'Sem responsável')}</small></button>`).join('')}</section>`).join('')||'<div class="task-empty">Defina prazos para visualizar o calendário.</div>'}</div>`;bindTaskEditButtons(board);return;
  }
  const completed=nativeTasks.columns.find(column=>/conclu/i.test(column.title)),done=tasks.filter(task=>task.column_id===completed?.id).length,late=tasks.filter(task=>task.due_date&&task.column_id!==completed?.id&&new Date(`${task.due_date}T23:59:59`)<NOW).length,estimated=tasks.reduce((sum,task)=>sum+(Number(task.estimate_minutes)||0),0),byAssignee=Object.entries(tasks.reduce((all,task)=>{const key=task.assignee||'Sem responsável';all[key]=(all[key]||0)+1;return all},{})).sort((a,b)=>b[1]-a[1]);
  board.className='tasks-board task-analytics-view';board.innerHTML=`<div class="task-kpis"><article><span>Total</span><b>${tasks.length}</b><small>tarefas no recorte</small></article><article><span>Concluídas</span><b>${done}</b><small>${tasks.length?Math.round(done/tasks.length*100):0}% do total</small></article><article><span>Atrasadas</span><b>${late}</b><small>exigem atenção</small></article><article><span>Estimativa</span><b>${Math.round(estimated/60)}h</b><small>de trabalho planejado</small></article></div><div class="task-analytics-panels"><section><h3>Distribuição por etapa</h3>${nativeTasks.columns.map(column=>{const value=tasks.filter(task=>task.column_id===column.id).length,percent=tasks.length?value/tasks.length*100:0;return `<div class="task-progress-row"><span>${escapeHtml(column.title)}</span><i><b style="width:${percent}%"></b></i><strong>${value}</strong></div>`}).join('')}</section><section><h3>Carga por responsável</h3>${byAssignee.slice(0,8).map(([name,value])=>`<div class="task-progress-row"><span>${escapeHtml(name)}</span><i><b style="width:${tasks.length?value/tasks.length*100:0}%"></b></i><strong>${value}</strong></div>`).join('')}</section></div>`;
}
...
async function refreshNativeTasks(){
  ...
  }catch(error){if(!nativeTasks.columns.length)board.innerHTML=`<div class="task-error">${escapeHtml(error.message)}</div>`;else{board.dataset.syncError=error.message;board.setAttribute('aria-label','Quadro salvo. Atualização indisponível: '+error.message)}}
}
...
function openTaskStageCreator(){
  ...
  form.innerHTML='<label>Nome da etapa<input name="title" maxlength="80" placeholder="Ex.: Em revisão" required></label><label>Função<select name="role"><option value="standard">Normal</option><option value="in_progress">Em andamento</option><option value="review">Revisão</option><option value="blocked">Bloqueada</option><option value="completed">Concluída</option></select></label><div class="task-inline-editor-actions"><button class="save" type="submit">Criar etapa</button><button type="button" data-cancel-stage>Cancelar</button></div><span class="task-inline-editor-status" role="status"></span>';
...
function initializeTaskStructures(){
  ...
  modal.querySelectorAll('[data-structure-form]').forEach(form=>form.onsubmit=async event=>{
    ...
    if(type==='cycle'){payload.starts_on=data.get('starts_on')||null;payload.ends_on=data.get('ends_on')||null;if(payload.starts_on&&payload.ends_on&&payload.starts_on>payload.ends_on){status.textContent='O fim do ciclo deve ser igual ou posterior ao início.';return}}
    button.disabled=true;status.textContent='Salvando...';
    try{await taskApi(`/api/task-${type}s`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});form.reset();await loadNativeTasks();refreshTaskStructureProjects();renderTaskStructures();status.textContent=({project:'Projeto criado.',module:'Módulo criado.',cycle:'Ciclo criado.'})[type]}
...
function renderTaskStructures(){
  const target=document.querySelector('#taskStructureExisting'),status=document.querySelector('#taskStructureStatus'),groups=[['Projetos','project',nativeTasks.projects],['Módulos','module',nativeTasks.modules],['Ciclos','cycle',nativeTasks.cycles]];
  ...
  target.querySelectorAll('[data-delete-structure]').forEach(button=>button.onclick=async()=>{if(!confirm('Excluir este item? As tarefas serão preservadas sem este vínculo.'))return;button.disabled=true;try{await taskApi(`/api/task-${button.dataset.deleteStructure}s?id=${button.dataset.id}`,{method:'DELETE'});await loadNativeTasks();refreshTaskStructureProjects();renderTaskStructures();status.textContent='Item excluído.'}catch(error){status.textContent=error.message;button.disabled=false}});
}
...
const compactMetric=value=>{const number=Number(value);if(!Number.isFinite(number)||number<0)return '—';return number>=1e6?`${(number/1e6).toLocaleString('pt-BR',{maximumFractionDigits:1})} mi`:number>=1e3?`${(number/1e3).toLocaleString('pt-BR',{maximumFractionDigits:1})} mil`:number.toLocaleString('pt-BR')};
function renderCreativeLibrary(){
  ...
  status.textContent=`${rows.length} de ${creativeVideos.length} vídeo(s) salvos`;
...
renderCreativeLibrary=function(){
  const root=document.querySelector('#creativeLibrary'),status=document.querySelector('#creativeLibraryStatus');status.textContent=creativeVideos.length?`${creativeVideos.length} vídeo(s) encontrados • resultados não armazenados`:'Nenhum resultado nesta sessão.';
  root.innerHTML=creativeVideos.map((video,index)=>{const platform=video.platform==='instagram'?'instagram':'tiktok',platformName=platform==='instagram'?'Instagram':'TikTok',coverTag=video.cover_text_detected===false?'<span class="cover-clean">Sem texto detectado</span>':video.cover_text_detected===true?'<span class="cover-text">Texto na capa</span>':video.cover_ocr_checked?'<span class="cover-unknown">Capa não legível</span>':'';return `<article class="creative-video-card" data-creative-index="${index}"><label class="creative-select"><input type="checkbox" checked data-select-creative="${index}"><span>Selecionar</span></label><button class="creative-thumb" type="button" data-preview-creative="${index}">${video.thumbnail_url?`<img src="${escapeHtml(video.thumbnail_url)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:'<span>▶</span>'}<b>Assistir preview</b></button><div class="creative-video-body"><div class="creative-video-tags"><span class="platform-${platform}">${platformName}</span><span>${escapeHtml(video.search_term||'Busca relacionada')}</span><span>${video.duration?`${video.duration}s`:'Vídeo'}</span>${coverTag}${video.visual_verified?`<span class="verified">✓ ${video.detected_type==='car'?'Carro confirmado':'Imóvel confirmado'}</span>`:''}</div><h4>${escapeHtml(video.title||`Vídeo do ${platformName}`)}</h4><a href="${escapeHtml(video.creator_url||video.video_url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(video.creator_name||'Criador não identificado')}</a><div class="creative-video-metrics"><span>Visualizações <b>${compactMetric(video.view_count)}</b></span><span>Curtidas <b>${compactMetric(video.like_count)}</b></span><span>Comentários <b>${compactMetric(video.comment_count)}</b></span></div><div class="creative-video-actions"><a href="${escapeHtml(video.video_url)}" target="_blank" rel="noopener noreferrer">Abrir no ${platformName}</a>${video.download_url?`<a class="primary" href="${escapeHtml(video.download_url)}" target="_blank" rel="noopener noreferrer" download="${platform}-${escapeHtml(video.id)}.mp4">Baixar vídeo</a>`:'<span>Download indisponível</span>'}</div></div></article>`}).join('')||'<div class="creative-library-empty"><strong>Nenhum vídeo encontrado</strong><span>Tente termos mais específicos ou escolha outra plataforma.</span></div>';
};
const prepareCreativeCover=async url=>{const response=await fetch(url);if(!response.ok)throw new Error('Capa indisponível');const bitmap=await createImageBitmap(await response.blob()),scale=Math.min(1,720/bitmap.width),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));const context=canvas.getContext('2d',{alpha:false});context.fillStyle='#fff';context.fillRect(0,0,canvas.width,canvas.height);context.drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close();return canvas};
...
initializeCreativeLibrary=function(){
  ...
  const resumeCreativeSearch=async saved=>{
    ...
    try{
      ...
      do{
        ...
        progressDetail.textContent='Preparando os vídeos encontrados no servidor.';
        document.querySelector('#creativeLibraryStatus').textContent='Preparando vídeos encontrados...';
  ...
  const loadCreativePresets=async()=>{try{const response=await fetch('/api/creative-search-settings'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Não foi possível carregar os termos salvos.');creativePresets=Array.isArray(payload.presets)?payload.presets:[];renderCreativePresets()}catch(error){showPresetStatus(error.message,'error')}};
  presetSelect.onchange=()=>{const preset=creativePresets.find(item=>item.id===presetSelect.value);deletePreset.disabled=!preset;savePreset.textContent=preset?'Atualizar termos':'Salvar nova';if(!preset){presetName.value='';return showPresetStatus('Edite os termos acima e salve uma nova predefinição.')}termsField.value=(preset.terms||[]).join('\n');presetName.value=preset.name;showPresetStatus(`Edite, adicione ou remova termos e clique em “Atualizar termos”.`,'success')};
  savePreset.onclick=async()=>{const id=presetSelect.value,name=presetName.value.trim(),terms=[...new Set(termsField.value.split(/\r?\n|,/).map(value=>value.trim()).filter(Boolean))];if(!name)return showPresetStatus('Digite um nome para a predefinição.','error');if(!terms.length)return showPresetStatus('Informe pelo menos um termo para salvar. Para apagar a predefinição, use “Excluir esta”.','error');savePreset.disabled=true;showPresetStatus(id?'Atualizando...':'Salvando...');try{const response=await fetch('/api/creative-search-settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,name,terms})}),preset=await response.json();if(!response.ok)throw new Error(preset.error||'Não foi possível salvar.');creativePresets=creativePresets.filter(item=>item.id!==preset.id&&item.name.toLowerCase()!==preset.name.toLowerCase());creativePresets.push(preset);renderCreativePresets(preset.id);showPresetStatus(id?'Predefinição atualizada com sucesso.':'Termos salvos com sucesso.','success')}catch(error){showPresetStatus(error.message,'error')}finally{savePreset.disabled=false}};
...
function reportAdsForAccount(accountId){const context=currentReportContext,analysis=context?.analysis?.accounts?.[accountId],ads=analysis?.ads||[];if(ads.length)return [...ads].sort((a,b)=>String(a.campaign_name||'').localeCompare(String(b.campaign_name||''),'pt-BR')||Number(b.spend)-Number(a.spend));const campaigns=context?.payload?.accounts?.[accountId]?.campaigns||[];return campaigns.map(campaign=>({account_id:accountId,ad_id:`campaign:${campaign.campaign_id||campaign.campaign_name||campaign.name||''}`,ad_name:campaign.campaign_name||campaign.name||'Campanha sem nome',campaign_id:campaign.campaign_id||'',campaign_name:campaign.campaign_name||campaign.name||'Campanha sem nome',objective:campaign.objective||'',result_type:campaign.objective_label||campaign.result_type||'Sem resultado atribuído',format:'Campanha',status:campaign.status||'UNKNOWN',effective_status:campaign.effective_status||campaign.status||'UNKNOWN',spend:Number(campaign.spend||0),results:Number(campaign.results||0),cost_per_result:campaign.cost_per_result??null,no_delivery_in_period:Boolean(campaign.no_delivery_in_period),report_source:'campaign'})).sort((a,b)=>String(a.campaign_name).localeCompare(String(b.campaign_name),'pt-BR')||Number(b.spend)-Number(a.spend))}
...
function openReportAdsModal(accountId,trigger){
  ...
  document.querySelector('#reportAdsModalMeta').textContent=`${context.from.toLocaleDateString('pt-BR')} a ${context.to.toLocaleDateString('pt-BR')} • ${ads.length} ${ads.length===1?'anúncio':'anúncios'} no período`;
  const campaignFallback=ads.some(ad=>ad.report_source==='campaign');document.querySelector('#reportAdsModalBody').innerHTML=`<section class="report-ads-detail"><div class="report-ads-title"><div><span>${campaignFallback?'DETALHAMENTO POR CAMPANHA':'DETALHAMENTO POR ANÚNCIO'}</span><h4>${campaignFallback?'Campanhas auditadas no período':'Campanhas e peças veiculadas'}</h4><p>${campaignFallback?'A Meta não retornou peças individuais para este período; exibimos as campanhas auditadas, inclusive as sem veiculação.':'Anúncios retornados pela Meta no período, inclusive resultados atribuídos sem gasto. O status indica a situação atual.'}</p></div><b>${ads.length} ${campaignFallback?(ads.length===1?'campanha':'campanhas'):(ads.length===1?'anúncio':'anúncios')}</b></div><div class="report-ad-list">${ads.map((ad,index)=>reportAdCard(accountId,ad,index)).join('')||'<p class="report-ads-empty">A Meta não retornou campanhas nem anúncios para o período selecionado.</p>'}</div></section>`;
...
document.addEventListener('click',event=>{const trigger=event.target.closest('[data-open-ad-preview]');if(!trigger)return;const card=trigger.closest('[data-report-ad]'),accountId=card?.dataset.accountId,ad=reportAdsForAccount(accountId)[Number(card?.dataset.adIndex)],name=ad?.ad_name||'Anúncio';closeReportAdPreview();const lightbox=document.createElement('div');lightbox.className='report-ad-lightbox';lightbox.setAttribute('role','dialog');lightbox.setAttribute('aria-modal','true');lightbox.setAttribute('aria-label',`Prévia ampliada de ${name}`);lightbox.innerHTML=`<div class="report-ad-lightbox-panel"><div class="report-ad-lightbox-head"><div><span>PRÉVIA DO CRIATIVO</span><strong>${escapeHtml(name)}</strong><small>Clique na imagem para visualizar todos os detalhes da capa.</small></div><button type="button" data-close-ad-preview aria-label="Fechar prévia">×</button></div><div class="report-ad-lightbox-stage"><img src="${escapeHtml(trigger.dataset.openAdPreview)}" alt="Capa completa do anúncio ${escapeHtml(name)}" referrerpolicy="no-referrer"></div></div>`;document.body.appendChild(lightbox);document.body.classList.add('report-preview-open');lightbox.querySelector('[data-close-ad-preview]').focus()});
...
async function persistAccountProfiles(){
  ...
  const result=await response.json();if(!response.ok)throw new Error(result.error||'Não foi possível salvar o perfil.');
...
const commentDate=ad=>ad.published_time?'Publicado em '+new Date(ad.published_time).toLocaleDateString('pt-BR'):ad.created_time?'Anúncio criado em '+new Date(ad.created_time).toLocaleDateString('pt-BR'):'Data de publicação indisponível';
function renderComments(){
 ...
 document.querySelector('#commentsResultsSummary').textContent=`${count} comentário(s) em ${rows.length} anúncio(s), conforme os filtros locais.`;
 root.innerHTML=rows.length?rows.map(item=>`<button type="button" class="comments-ad-card comments-ad-open" data-comment-ad="${escapeHtml(commentAdKey(item))}" aria-haspopup="dialog"><span class="comments-ad-head">${item.ad.thumbnail_url?`<img class="comments-ad-thumb" src="${escapeHtml(item.ad.thumbnail_url)}" alt="" referrerpolicy="no-referrer">`:'<span class="comments-ad-thumb"></span>'}<span class="comments-ad-copy"><strong>${escapeHtml(item.ad.name)}</strong><span>${escapeHtml(item.account_name)} · ${escapeHtml(item.ad.campaign_name)}</span><small>${escapeHtml(commentDate(item.ad))}</small></span><span class="comments-ad-meta"><span class="comments-status ${item.ad.effective_status==='ACTIVE'?'active':'inactive'}">${item.ad.effective_status==='ACTIVE'?'Ativo':item.ad.effective_status==='PAUSED'?'Pausado':'Inativo'}</span><span class="comments-count">${item.comments.length} comentário(s)${item.deleted_count?' · '+item.deleted_count+' apagado(s)':''}</span><span class="comment-platform ${item.platform==='instagram'?'comment-platform-instagram':''}">${item.platform==='instagram'?'Instagram':item.platform==='facebook'?'Facebook · Página':'Origem não informada'}</span><span class="comments-open-label">Ver comentários →</span></span></span></button>`).join(''):'<div class="comments-empty"><strong>Nenhum comentário encontrado</strong><span>Selecione uma página ou altere os filtros.</span></div>';
...
function renderCommentsDialog(){
 ...
 document.querySelector('#commentsDialogTitle').textContent=original?.ad.name||'Comentários do anúncio';
 document.querySelector('#commentsDialogMeta').textContent=original?commentDate(original.ad)+' · '+(original.ad.effective_status==='ACTIVE'?'Ativo':'Inativo'):'';
 list.innerHTML=item?item.comments.map(comment=>`<div class="comment-row" data-comment-row="${escapeHtml(comment.id)}"><label class="comment-check"><input type="checkbox" value="${escapeHtml(comment.id)}" data-comment-select aria-label="Selecionar comentário de ${escapeHtml(comment.from?.name||'autor não identificado')}"></label>${commentAvatar(comment)}<div class="comment-copy"><strong>${escapeHtml(comment.from?.name||'Nome não disponibilizado pela Meta')}${comment.is_hidden?'<span class="comment-hidden-label">Oculto</span>':''}</strong><span class="comment-platform ${comment.platform==='instagram'?'comment-platform-instagram':''}">${comment.platform==='instagram'?'Instagram':comment.platform==='facebook'?'Facebook · Página':'Origem não informada'}</span><p>${escapeHtml(comment.message||'Comentário sem texto')}</p><small>${comment.created_time?new Date(comment.created_time).toLocaleString('pt-BR'):'Data indisponível'} • ${num(comment.like_count)} curtidas • ${num(comment.comment_count)} respostas</small></div><div class="comment-actions">${comment.permalink_url?`<a class="comment-action" href="${escapeHtml(comment.permalink_url)}" target="_blank" rel="noopener noreferrer">Abrir</a>`:''}<button class="comment-action" type="button" data-comment-action="${comment.is_hidden?'unhide':'hide'}" data-comment-id="${escapeHtml(comment.id)}">${comment.is_hidden?'Reexibir':'Ocultar'}</button><button class="comment-action danger" type="button" data-comment-action="delete" data-comment-id="${escapeHtml(comment.id)}">Excluir</button></div></div>`).join(''):'<p class="comments-dialog-empty">Nenhum comentário para exibir neste anúncio com os filtros atuais.</p>';
...
async function renderDeletedComments(item){const request=++commentsArchiveRequest,list=document.querySelector('#commentsDialogList');list.innerHTML='<p class="comments-dialog-empty" role="status">Carregando histórico de exclusões...</p>';updateCommentsSelection();if(!item)return;
 try{const response=await fetch('/api/meta-comment-archive?'+new URLSearchParams({account:item.account_id,post:item.ad.post_id})),data=await response.json();if(!response.ok)throw new Error(data.error||'Não foi possível carregar o histórico.');if(request!==commentsArchiveRequest||commentsDialogTab!=='deleted')return;
 list.innerHTML='<p class="comments-archive-note">Exclusões confirmadas pelo Traffic pocket a partir desta atualização. Exclusões anteriores ou feitas fora do app não têm conteúdo recuperável aqui. Histórico limitado aos 2.000 registros mais recentes do seu espaço.</p>'+(data.comments.length?data.comments.map(record=>{const comment=record.comment;return `<article class="comment-row comment-deleted">${commentAvatar(comment)}<div class="comment-copy"><strong>${escapeHtml(comment.from?.name||'Nome não disponibilizado pela Meta')}</strong><span class="comment-platform ${comment.platform==='instagram'?'comment-platform-instagram':''}">${comment.platform==='instagram'?'Instagram':'Facebook · Página'}</span><p>${escapeHtml(comment.message||'Comentário sem texto')}</p><small>Excluído em ${new Date(record.deleted_at).toLocaleString('pt-BR')} · Exclusão confirmada pela Meta</small></div><span class="comment-deleted-label">Apagado</span></article>`}).join(''):'<p class="comments-dialog-empty">Nenhuma exclusão registrada para esta publicação.</p>');bindCommentAvatarFallback();
...
async function loadCommentsInstagram(){const page=commentsPage,request=++commentsInstagramRequest;showCommentsInstagram({state:'loading',message:page?'Verificando Instagram vinculado...':'Selecione uma página para verificar o Instagram.'});if(!page)return;try{const response=await fetch('/api/meta-comment-instagram?'+new URLSearchParams({page:page.id})),data=await response.json();if(!response.ok)throw new Error(data.error||'Falha ao verificar Instagram.');if(request===commentsInstagramRequest&&commentsPage?.id===page.id)showCommentsInstagram(data)}catch{if(request===commentsInstagramRequest)showCommentsInstagram({state:'error',message:'Não foi possível verificar o Instagram. Atualize a consulta para tentar novamente.'})}}
let commentsPage=null,commentsPages=[],commentsPageRequest=null,commentsPagesLoadedAt=0;
function renderCommentsPageList(){const list=document.querySelector('#commentsPageList'),query=document.querySelector('#commentsPageSearch').value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase(),pages=commentsPages.filter(page=>page.name.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().includes(query));list.innerHTML=pages.length?pages.map(page=>`<button type="button" class="comments-page-option ${commentsPage?.id===page.id?'selected':''}" data-comments-page="${escapeHtml(page.id)}" role="option" aria-selected="${commentsPage?.id===page.id}"><span class="comments-page-initial">${escapeHtml(page.name.trim().slice(0,1).toUpperCase())}</span><strong>${escapeHtml(page.name)}</strong>${commentsPage?.id===page.id?'<b aria-hidden="true">✓</b>':''}</button>`).join(''):'<p class="comments-page-empty">Nenhuma página encontrada.</p>'}
function renderCommentsPages(){const select=document.querySelector('#commentsPageSelect'),trigger=document.querySelector('#commentsPageTrigger');select.innerHTML='<option value="">Selecione uma página</option>'+commentsPages.map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.name)}</option>`).join('');if(commentsPage&&commentsPages.some(p=>p.id===commentsPage.id))select.value=commentsPage.id;else{commentsPage=null;commentsData=[];renderComments()}select.disabled=!commentsPages.length;trigger.disabled=!commentsPages.length;trigger.querySelector('span').textContent=commentsPage?.name||'Selecione uma página';renderCommentsPageList()}
function fetchCommentsPages(){if(commentsPageRequest)return commentsPageRequest;const select=document.querySelector('#commentsPageSelect'),status=document.querySelector('#commentsPagesStatus');select.disabled=true;status.textContent='Atualizando suas páginas autorizadas...';commentsPageRequest=(async()=>{try{const response=await fetch('/api/meta-comment-pages'),data=await response.json();if(!response.ok)throw new Error(data.error||'Não foi possível listar as páginas.');commentsPages=data.pages||[];const previous=commentsPage?.id,saved=userStorage.getItem('hurtz-comments-page');if(!commentsPage&&saved)commentsPage=commentsPages.find(p=>p.id===saved)||null;renderCommentsPages();commentsPagesLoadedAt=Date.now();void loadCommentsInstagram();const labels={pages_show_list:'listar páginas',pages_read_engagement:'ler publicações',pages_read_user_content:'ler comentários',pages_manage_engagement:'moderar comentários'},missing=data.missingPermissions||[];status.textContent=missing.length?'Falta autorizar: '+missing.map(p=>labels[p]||p).join(', ')+'. Se você já confirmou no Facebook, essa conexão não recebeu as permissões. O administrador do Tryv CRM deve verificar o acesso delas para usuários em produção.':data.warnings?.length?'Algumas páginas não puderam ser consultadas. Clique em Atualizar páginas para tentar novamente.':commentsPages.length?commentsPages.length+' páginas disponíveis. A lista é atualizada automaticamente.':'Nenhuma página disponível. Clique em Autorizar páginas e revise as páginas e empresas liberadas para o Tryv CRM.';document.querySelector('#commentsPagesAuthorize').textContent=missing.length?'Autorizar páginas':'Revisar autorização';if(commentsPage&&!previous&&!missing.includes('pages_read_user_content')&&!missing.includes('pages_read_engagement'))void loadAdComments();return data}catch(e){status.textContent=e.message;select.disabled=!commentsPages.length}finally{commentsPageRequest=null}})();return commentsPageRequest}
...
async function loadCommentsHistory(){const root=document.querySelector('#commentsHistoryList');try{const response=await fetch('/api/meta-comment-history'),payload=await response.json();if(!response.ok)throw new Error();const labels={hide:'Ocultou',unhide:'Reexibiu',delete:'Excluiu'},events=payload.events||[];root.innerHTML=events.length?events.slice(0,50).map(event=>`<div class="comments-history-row"><strong>${new Date(event.created_at).toLocaleString('pt-BR')}</strong><span>${labels[event.action]||event.action} ${event.comment_ids?.length||0} comentário(s)</span><span>${event.success||0} concluído(s)</span><span class="${event.failed?'failed':''}">${event.failed||0} falha(s)</span></div>`).join(''):'<div class="comments-history-empty">Nenhuma ação registrada.</div>'}catch{root.innerHTML='<div class="comments-history-empty">Histórico indisponível neste momento.</div>'}}
async function loadAdComments(){const page=commentsPage,button=document.querySelector('#commentsFilterForm [type="submit"]'),status=document.querySelector('#commentsRequestStatus'),view=document.querySelector('#comments');if(!page){status.textContent='Selecione uma página para buscar os comentários.';document.querySelector('#commentsPageSelect').focus();return}button.disabled=true;view.classList.add('is-loading');status.classList.remove('error');status.textContent='Consultando anúncios, publicações e comentários na Meta...';try{const query=new URLSearchParams({page:page.id,status:document.querySelector('#commentsAdStatus').value,days:document.querySelector('#commentsPeriod').value}),response=await fetch(`/api/meta-comments?${query}`),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao consultar comentários.');if(commentsPage?.id!==page.id)return;commentsData=payload.ads||[];if(payload.instagram){commentsInstagramRequest++;showCommentsInstagram(payload.instagram)}renderComments();status.textContent=`Consulta concluída: ${payload.comments||0} comentário(s) em ${commentsData.length} anúncio(s).${payload.warnings?.length?` ${payload.warnings.length} consulta(s) de publicações não concluída(s).`:''}`}catch(error){status.textContent=error.message;status.classList.add('error');commentsData=[];renderComments()}finally{button.disabled=false;view.classList.remove('is-loading')}}
async function moderateComments(action,ids){
 ...
 announce(`${action==='delete'?'Excluindo':action==='hide'?'Ocultando':'Reexibindo'} ${ids.length} comentário(s)...`);
 document.querySelectorAll('[data-comment-action],[data-comments-action]').forEach(button=>button.disabled=true);
 try{const response=await fetch('/api/meta-comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,comment_ids:ids})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'A moderação não foi concluída.');
 ...
 const successMessage=succeeded.size?`${succeeded.size} comentário(s) ${action==='delete'?'excluído(s). Exclusão confirmada pela Meta; removido(s) desta lista':action==='hide'?'ocultado(s) com confirmação da Meta':'reexibido(s) com confirmação da Meta'}.` : '';
 announce(successMessage+(failed?` ${failed} comentário(s) não alterado(s). `+((payload.results||[]).find(row=>!row.ok)?.error||'A Meta não confirmou a operação. Tente novamente.') : ''),failed>0);
...
function requestCommentsDelete(ids){if(!ids.length)return;commentsPendingDelete=ids;const modal=document.querySelector('#commentsConfirmModal');document.querySelector('#commentsConfirmDescription').textContent=`Você está prestes a excluir permanentemente ${ids.length} comentário(s). Esta ação não poderá ser desfeita pelo dashboard.`;modal.hidden=false;document.querySelector('#commentsConfirmInput').focus()}
...
function requestFacebookPermissions(instagram=false){if(facebookLoginBusy)return;const status=document.querySelector('#commentsPagesStatus');if(!window.FB||!facebookNonce){status.textContent='O login ainda está carregando. Tente novamente em alguns instantes.';return}reconnectingComments=true;reconnectingInstagram=instagram;status.textContent=instagram?'Autorize o Instagram na janela do Facebook. Se a Meta recusar a permissão, sua conexão atual será mantida.':'Revise as páginas autorizadas no Facebook. Sua conexão atual será mantida se você cancelar.';facebookConnect.click()}
...
function openFacebookLogin(){
  ...
  const loginScopes=requestedAds?'ads_read,ads_management,business_management,whatsapp_business_management,pages_show_list,pages_read_engagement,pages_manage_ads,pages_manage_metadata,leads_retrieval':requestedInstagram?'ads_read,business_management,pages_show_list,pages_read_engagement,pages_read_user_content,pages_manage_engagement,instagram_basic,instagram_manage_comments':requestedComments?'ads_read,business_management,pages_show_list,pages_read_engagement,pages_read_user_content,pages_manage_engagement':'ads_read,ads_management,business_management,whatsapp_business_management,pages_show_list,pages_read_engagement,pages_manage_ads,pages_manage_metadata,leads_retrieval';
...
const backAccount=document.querySelector('#closeModal');backAccount.textContent='← Voltar às contas';backAccount.setAttribute('aria-label','Voltar às contas');backAccount.className='account-back';document.querySelector('#modalTitle').tabIndex=-1;
