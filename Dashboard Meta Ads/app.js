...
let reconnectingComments=false;
window.addEventListener('hurtz-connect-comments',()=>{reconnectingComments=true;const status=document.querySelector('#commentsPagesStatus');status.textContent='No Facebook, revise as configurações do Tryv CRM e selecione todas as páginas e empresas desejadas. Se houver a opção, inclua também as futuras. Depois da confirmação, a lista será atualizada automaticamente.';if(facebookConnect.disabled||!window.FB||!facebookNonce){status.textContent+=' O login ainda está carregando; tente novamente em alguns instantes.';return}facebookConnect.click()});
...
if(personalIdentity?.personal){
  ...
  void (async()=>{try{
    ...
    if(facebookSettings.connected){const found=await findMetaAccounts(false);if(found){await loadAccountProfiles();loadPlans();await hydrateAlertPlans();if(selectedAccountIds.size){loadAuditedPeriod(globalPeriod.from,globalPeriod.to);loadLastThreeDays();loadRealControlData(globalPeriod.to)}}}
