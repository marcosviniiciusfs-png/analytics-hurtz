...
async function initializeReports(){if(reportInitialized)return;reportInitialized=true;initializeReportAnimation();const yesterday=new Date(NOW);yesterday.setDate(yesterday.getDate()-1);document.querySelector('#reportDateFrom').value=iso(yesterday);document.querySelector('#reportDateTo').value=iso(yesterday);try{const response=await fetch('/api/meta-monitor-config'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Falha ao carregar contas');if(!accounts.length&&payload.accounts?.length)setReportMemberAccounts(payload.accounts)}catch(error){reportInitialized=false;document.querySelector('#reportStatus').textContent=error.message||'Não foi possível carregar as contas monitoradas.'}}
...
let nativeTasks={columns:[],tasks:[],projects:[],modules:[],cycles:[],subtasks:[],comments:[],attachments:[],activities:[],notifications:[]},tasksInitialized=false,draggedTaskId=null,taskView='board',activeTaskDetail='checklist',inlineEditingTaskId=null,inlineNewColumnId=null,inlineNewStage=false,openTaskCardMenuId=null;
const TASK_LOCAL_CACHE_KEY='hurtz-native-tasks-cache-v1';
let taskLoadPromise=null,taskLoadedAt=0,taskMutationRevision=0,taskContext={profile:null,workspaces:[],invites:[]},taskMembers=[],activeTaskWorkspaceId='';
const taskDataPath=url=>/^\/api\/(tasks|task-(columns|order|notifications|subtasks|comments|attachments))(\?|$)/.test(url);
const taskScopedUrl=url=>{if(!activeTaskWorkspaceId||!taskDataPath(url))return url;const separator=url.includes('?')?'&':'?';return `${url}${separator}workspace=${encodeURIComponent(activeTaskWorkspaceId)}`};
const taskApi=async(url,options={})=>{const response=await fetch(taskScopedUrl(url),options),payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||'Não foi possível atualizar as tarefas');if((options.method||'GET').toUpperCase()!=='GET')taskMutationRevision++;return payload};
const taskRoleLabels={owner:'Proprietário',admin:'Administrador',member:'Membro',viewer:'Visualizador'};
const activeTaskWorkspace=()=>taskContext.workspaces.find(item=>item.id===activeTaskWorkspaceId)||taskContext.workspaces[0];
const activeTaskRole=()=>activeTaskWorkspace()?.role||'owner';
const taskCanManage=()=>['owner','admin'].includes(activeTaskRole());
const taskCanEdit=()=>activeTaskRole()!=='viewer';
const taskCacheKey=()=>`${TASK_LOCAL_CACHE_KEY}:${activeTaskWorkspaceId||'local'}`;
...
const taskPriorityLabel=value=>({high:'Alta',medium:'Média',low:'Baixa'})[value]||'Média';
const taskInitials=value=>(value||'Sem responsável').trim().split(/\s+/).slice(0,2).map(part=>part[0]).join('').toUpperCase();
...
function taskCardMarkup(task,completedColumn){
  ...
  return `<article class="task-card task-priority-${task.priority||'medium'}" draggable="true" data-task-id="${task.id}"><div class="task-card-heading"><span class="task-drag-handle" title="Arraste para mover" aria-hidden="true">⠿</span><button class="task-card-title" type="button" data-edit-task="${task.id}">${escapeHtml(task.title)}</button><button class="task-card-settings ${menuOpen?'active':''}" type="button" data-task-card-menu="${task.id}" aria-expanded="${menuOpen}" aria-label="Opções de ${escapeHtml(task.title)}">•••</button></div><span class="task-priority-line"><i></i>${taskPriorityLabel(task.priority)}</span>${task.description?`<p>${escapeHtml(task.description)}</p>`:''}${task.labels?.length?`<div class="task-labels">${task.labels.slice(0,3).map(label=>`<span class="task-label">${escapeHtml(label)}</span>`).join('')}${task.labels.length>3?`<span class="task-label">+${task.labels.length-3}</span>`:''}</div>`:''}${subtasks.length?`<div class="task-card-progress"><span><i style="width:${progress}%"></i></span><b>${subtasks.filter(item=>item.is_done).length}/${subtasks.length}</b></div>`:''}<footer class="task-card-footer"><div class="task-card-dates">${task.due_date?`<span class="${isOverdue?'task-overdue':''}">▣ ${taskDateLabel(task.due_date)}</span>`:''}${task.estimate_minutes?`<span>◷ ${taskEstimateLabel(task.estimate_minutes)}</span>`:''}${task.expires_at?`<span class="task-expiry">⌛ ${taskExpiryLabel(task.expires_at)}</span>`:''}</div>${task.assignee?`<span class="task-avatar" title="${escapeHtml(task.assignee)}">${escapeHtml(taskInitials(task.assignee))}</span>`:'<span class="task-avatar task-avatar-empty" title="Sem responsável">+</span>'}</footer>${menuOpen?`<div class="task-card-popover"><strong>Opções da tarefa</strong><button type="button" data-inline-edit-task="${task.id}">✎ Edição rápida</button><button type="button" data-edit-task="${task.id}">▣ Abrir detalhes</button><button class="danger" type="button" data-delete-card="${task.id}">× Excluir tarefa</button></div>`:''}</article>`;
}
function renderNativeTasks(){
  const board=document.querySelector('#tasksBoard'),count=document.querySelector('#taskCount');
  renderTaskIdentity();
  const search=(document.querySelector('#taskSearch')?.value||'').trim().toLocaleLowerCase('pt-BR'),priority=document.querySelector('#taskPriorityFilter')?.value||'',assignee=document.querySelector('#taskAssigneeFilter')?.value||'';
  const visibleTasks=nativeTasks.tasks.filter(task=>(!search||`${task.title} ${task.description||''} ${task.assignee||''} ${(task.labels||[]).join(' ')}`.toLocaleLowerCase('pt-BR').includes(search))&&(!priority||task.priority===priority)&&(!assignee||task.assignee===assignee));
  ...
  board.innerHTML=nativeTasks.columns.map(column=>{
    ...
    return `<section class="task-column task-column-${role}" data-task-column="${column.id}" data-column-role="${role}"><header class="task-column-header"><div class="task-column-title"><i aria-hidden="true"></i><div><span class="task-column-name">${escapeHtml(column.title)}</span><small>${taskColumnRoleLabel(role)}</small></div><em title="${total} tarefas nesta etapa">${total}</em></div><button class="task-column-settings-button" type="button" data-configure-column="${column.id}" title="Configurar etapa" aria-label="Configurar etapa ${escapeHtml(column.title)}">•••</button></header><div class="task-list">${items.map(task=>taskCardMarkup(task,completedColumn)).join('')}${inlineNewColumnId===column.id?`<article class="task-card task-card-editing task-new-card">${taskInlineForm(null,column.id)}</article>`:''}${!items.length&&inlineNewColumnId!==column.id?`<div class="task-column-empty"><span>◇</span><b>Nenhuma tarefa aqui</b><small>Arraste um cartão ou crie uma nova tarefa.</small></div>`:''}</div><button class="task-add-card" type="button" data-quick-task="${column.id}" ${inlineNewColumnId===column.id?'disabled':''}><span>＋</span> Nova tarefa</button></section>`;
  }).join('')+`<section class="task-column task-add-column">${inlineNewStage?`<form class="task-stage-inline-form" id="taskStageInlineForm"><div><b>Nova etapa</b><button type="button" id="cancelTaskStage" aria-label="Cancelar">×</button></div><label>Nome<input name="title" maxlength="80" placeholder="Ex.: Em revisão" required></label><label>Função<select name="role"><option value="standard">Etapa normal</option><option value="in_progress">Em andamento</option><option value="review">Revisão</option><option value="blocked">Bloqueada</option><option value="completed">Tarefa concluída</option></select></label><button type="submit">Adicionar ao quadro</button><small class="task-inline-editor-status"></small></form>`:`<button type="button" id="taskAddColumn"><span>+</span> Adicionar etapa</button>`}</section>`;
  ...
  const addColumn=document.querySelector('#taskAddColumn');if(addColumn)addColumn.onclick=()=>{inlineNewStage=true;renderNativeTasks();document.querySelector('#taskStageInlineForm input')?.focus()};
  const stageForm=document.querySelector('#taskStageInlineForm');if(stageForm){document.querySelector('#cancelTaskStage').onclick=()=>{inlineNewStage=false;renderNativeTasks()};stageForm.onsubmit=async event=>{event.preventDefault();const data=new FormData(stageForm),title=String(data.get('title')||'').trim(),role=String(data.get('role')||'standard'),status=stageForm.querySelector('.task-inline-editor-status'),button=event.submitter;if(!title)return;button.disabled=true;status.textContent='Adicionando...';try{await taskApi('/api/task-columns',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,role,position:nativeTasks.columns.length})});inlineNewStage=false;await loadNativeTasks()}catch(error){status.textContent=error.message;button.disabled=false}}}
...
function renderTaskAlternativeView(tasks){
  ...
  if(taskView==='list'){
    board.className='tasks-board task-list-view';board.innerHTML=`<div class="task-table"><div class="task-table-head"><span>Tarefa</span><span>Etapa</span><span>Responsável</span><span>Prioridade</span><span>Prazo</span><span>Progresso</span></div>${tasks.map(task=>`<button type="button" class="task-table-row" data-edit-task="${task.id}"><span><b>${escapeHtml(task.title)}</b><small>${escapeHtml((task.labels||[]).join(' • '))}</small></span><span>${escapeHtml(taskPropertyTitle(nativeTasks.columns,task.column_id))}</span><span>${escapeHtml(task.assignee||'—')}</span><span class="task-priority ${task.priority}">${task.priority==='high'?'Alta':task.priority==='low'?'Baixa':'Média'}</span><span>${task.due_date?taskDateLabel(task.due_date):'—'}</span><span>${taskProgress(task)}%</span></button>`).join('')||'<div class="task-empty">Nenhuma tarefa encontrada.</div>'}</div>`;bindTaskEditButtons(board);return;
  }
...
function openTaskModal(task=null,columnId=''){
  ...
  document.querySelector('#taskEstimate').value=task?.estimate_minutes||'';
...
function taskMentionNames(){return [...new Set(taskMembers.map(member=>member.display_name).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR'))}
...
function renderTaskDetailPanel(taskId){
  ...
  if(activeTaskDetail==='checklist'){const items=nativeTasks.subtasks.filter(item=>item.task_id===taskId);panel.innerHTML=`<div class="task-inline-form" id="taskSubtaskForm"><input maxlength="180" placeholder="Adicionar item ao checklist..."><button type="button">Adicionar</button></div><div class="task-checklist">${items.map(item=>`<label><input type="checkbox" data-toggle-subtask="${item.id}" ${item.is_done?'checked':''}><span>${escapeHtml(item.title)}</span><button type="button" data-delete-subtask="${item.id}">×</button></label>`).join('')||'<div class="task-detail-empty">Nenhum item no checklist.</div>'}</div>`;const add=async()=>{const input=panel.querySelector('#taskSubtaskForm input'),title=input.value.trim();if(!title)return input.focus();await taskApi('/api/task-subtasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task_id:taskId,title,position:items.length})});await loadNativeTasks();renderTaskDetailPanel(taskId)};panel.querySelector('#taskSubtaskForm button').onclick=add;panel.querySelector('#taskSubtaskForm input').onkeydown=event=>{if(event.key==='Enter'){event.preventDefault();void add()}};panel.querySelectorAll('[data-toggle-subtask]').forEach(input=>input.onchange=async()=>{await taskApi('/api/task-subtasks',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:input.dataset.toggleSubtask,task_id:taskId,is_done:input.checked})});await loadNativeTasks();renderTaskDetailPanel(taskId)});panel.querySelectorAll('[data-delete-subtask]').forEach(button=>button.onclick=async()=>{await taskApi(`/api/task-subtasks?id=${button.dataset.deleteSubtask}&task_id=${taskId}`,{method:'DELETE'});await loadNativeTasks();renderTaskDetailPanel(taskId)});return}
  if(activeTaskDetail==='comments'){const items=nativeTasks.comments.filter(item=>item.task_id===taskId);panel.innerHTML=`<div class="task-comments">${items.map(item=>`<article><b>${escapeHtml(item.author)}</b><time>${new Date(item.created_at).toLocaleString('pt-BR')}</time><p>${escapeHtml(item.body)}</p></article>`).join('')||'<div class="task-detail-empty">Nenhum comentário.</div>'}</div><div class="task-comment-form" id="taskCommentForm"><input id="taskCommentAuthor" maxlength="100" placeholder="Seu nome" value="Equipe Hurtz"><textarea maxlength="3000" placeholder="Escreva um comentário..."></textarea><button type="button">Comentar</button></div>`;panel.querySelector('#taskCommentForm button').onclick=async()=>{const form=panel.querySelector('#taskCommentForm'),body=form.querySelector('textarea').value.trim();if(!body)return form.querySelector('textarea').focus();await taskApi('/api/task-comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task_id:taskId,author:form.querySelector('input').value,body})});await loadNativeTasks();renderTaskDetailPanel(taskId)};return}
...
function updateTaskFilterState(){
  const selectors=['#taskPriorityFilter','#taskAssigneeFilter'],active=selectors.filter(selector=>document.querySelector(selector)?.value).length,target=document.querySelector('#taskActiveFilterCount'),clear=document.querySelector('#taskClearFilters');
  if(target){target.textContent=String(active);target.hidden=!active}if(clear)clear.disabled=!active;
}
...
function renderTaskIdentity(){
  const profile=taskContext.profile,identity=document.querySelector('#taskUserIdentity'),workspace=activeTaskWorkspace(),role=activeTaskRole();if(profile?.public_id){identity.hidden=false;document.querySelector('#taskPublicId').textContent=profile.public_id}
  document.querySelector('#taskWorkspaceTitle').textContent=workspace?.name||'Meu projeto';const badge=document.querySelector('#taskWorkspaceRole');badge.textContent=taskRoleLabels[role]||role;badge.className=`task-role-badge ${role}`;
  const tasks=document.querySelector('#tasks');tasks.classList.toggle('task-readonly',role==='viewer');tasks.classList.toggle('task-member-restricted',role==='member');
  const inviteCount=(taskContext.invites||[]).length;['#taskInviteCount','#taskProjectInviteTabCount'].forEach(selector=>{const item=document.querySelector(selector);if(item){item.textContent=String(inviteCount);item.hidden=!inviteCount}});
}
async function loadTaskContext(){
  try{taskContext=await taskApi('/api/task-context');const saved=userStorage.getItem('hurtz-active-task-workspace');activeTaskWorkspaceId=taskContext.workspaces.some(item=>item.id===saved)?saved:taskContext.workspaces[0]?.id||'';renderTaskIdentity();await loadTaskMembers()}catch(error){console.warn('Contexto colaborativo indisponível:',error.message)}
}
async function loadTaskMembers(){if(!activeTaskWorkspaceId)return taskMembers=[];try{const payload=await taskApi(`/api/task-members?workspace=${encodeURIComponent(activeTaskWorkspaceId)}`);taskMembers=payload.members||[]}catch{taskMembers=[]}return taskMembers}
function renderTaskProjects(){
  const workspace=activeTaskWorkspace(),list=document.querySelector('#taskWorkspaceList'),canManage=taskCanManage(),invites=taskContext.invites||[];
  list.innerHTML=taskContext.workspaces.map(item=>`<button type="button" class="${item.id===activeTaskWorkspaceId?'active':''}" data-switch-task-workspace="${item.id}"><span><b>${escapeHtml(item.name)}</b><small>${escapeHtml(taskRoleLabels[item.role]||item.role)}</small></span>${item.id===activeTaskWorkspaceId?'<i>ATUAL</i>':''}</button>`).join('')||'<div class="task-project-empty">Nenhum projeto disponível.</div>';
  document.querySelector('#taskProjectCurrentName').textContent=workspace?.name||'—';document.querySelector('#taskProjectCurrentDescription').textContent=workspace?.description||'Quadro colaborativo de tarefas.';document.querySelector('#taskProjectCurrentRole').textContent=taskRoleLabels[workspace?.role]||'—';document.querySelector('#taskInviteForm').hidden=!canManage;document.querySelector('#renameTaskWorkspace').hidden=!canManage;
  document.querySelector('#taskMemberList').innerHTML=taskMembers.map(member=>{const owner=member.role==='owner',canChange=canManage&&!owner&&!(activeTaskRole()==='admin'&&member.role==='admin');return `<article class="task-member-row"><div><b>${escapeHtml(member.display_name||'Usuário')}</b><small>${escapeHtml(member.public_id||'')} ${member.user_id===taskContext.profile?.user_id?'• Você':''}</small></div>${canChange?`<select data-member-role="${member.user_id}" aria-label="Permissão de ${escapeHtml(member.display_name||'usuário')}">${['admin','member','viewer'].map(role=>`<option value="${role}" ${role===member.role?'selected':''}>${taskRoleLabels[role]}</option>`).join('')}</select><button class="danger" type="button" data-remove-member="${member.user_id}">Remover</button>`:`<span>${escapeHtml(taskRoleLabels[member.role]||member.role)}</span>`}</article>`}).join('')||'<div class="task-project-empty">Carregando membros...</div>';
  document.querySelector('#taskInviteList').innerHTML=invites.map(invite=>`<article class="task-invite-row"><div><b>${escapeHtml(invite.workspace_name)}</b><small>Convite para participar como ${escapeHtml((taskRoleLabels[invite.role]||invite.role).toLowerCase())}</small></div><div class="task-invite-actions"><button type="button" data-invite-answer="accept" data-invite-id="${invite.id}">Aceitar</button><button type="button" data-invite-answer="decline" data-invite-id="${invite.id}">Recusar</button></div></article>`).join('')||'<div class="task-project-empty">Você não tem convites pendentes.</div>';
  list.querySelectorAll('[data-switch-task-workspace]').forEach(button=>button.onclick=()=>switchTaskWorkspace(button.dataset.switchTaskWorkspace));
  document.querySelectorAll('[data-member-role]').forEach(select=>select.onchange=async()=>{try{await taskApi('/api/task-members',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({workspace_id:activeTaskWorkspaceId,user_id:select.dataset.memberRole,role:select.value})});await loadTaskMembers();renderTaskProjects()}catch(error){alert(error.message)}});
  document.querySelectorAll('[data-remove-member]').forEach(button=>button.onclick=async()=>{if(!confirm('Remover este membro do projeto?'))return;try{await taskApi(`/api/task-members?workspace=${activeTaskWorkspaceId}&user=${button.dataset.removeMember}`,{method:'DELETE'});await loadTaskMembers();renderTaskProjects()}catch(error){alert(error.message)}});
  document.querySelectorAll('[data-invite-answer]').forEach(button=>button.onclick=async()=>{try{const answer=await taskApi('/api/task-invites',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:button.dataset.inviteId,action:button.dataset.inviteAnswer})});await loadTaskContext();if(button.dataset.inviteAnswer==='accept')await switchTaskWorkspace(answer.workspace_id);else renderTaskProjects()}catch(error){alert(error.message)}});
}
async function switchTaskWorkspace(id){if(id===activeTaskWorkspaceId)return renderTaskProjects();activeTaskWorkspaceId=id;userStorage.setItem('hurtz-active-task-workspace',id);nativeTasks={columns:[],tasks:[],projects:[],modules:[],cycles:[],subtasks:[],comments:[],attachments:[],activities:[],notifications:[]};taskLoadedAt=0;taskLoadPromise=null;await loadTaskMembers();renderTaskIdentity();renderTaskProjects();await loadNativeTasks()}
function initializeTaskProjects(){
  const modal=document.querySelector('#taskProjectsModal'),createForm=document.querySelector('#taskWorkspaceCreateForm'),management=document.querySelector('#taskWorkspaceManagement');document.querySelector('#taskProjectsButton').onclick=async()=>{await loadTaskMembers();renderTaskProjects();createForm.hidden=true;management.hidden=false;modal.hidden=false};document.querySelector('#closeTaskProjects').onclick=()=>modal.hidden=true;modal.onclick=event=>{if(event.target===modal)modal.hidden=true};
  document.querySelector('#copyTaskPublicId').onclick=async event=>{await navigator.clipboard.writeText(taskContext.profile?.public_id||'');event.currentTarget.textContent='Copiado';setTimeout(()=>event.currentTarget.textContent='Copiar',1400)};
  document.querySelector('#showCreateTaskWorkspace').onclick=()=>{management.hidden=true;createForm.hidden=false;document.querySelector('#taskWorkspaceName').focus()};document.querySelector('#cancelTaskWorkspace').onclick=()=>{createForm.hidden=true;management.hidden=false};
  document.querySelector('#renameTaskWorkspace').onclick=async()=>{const workspace=activeTaskWorkspace(),name=prompt('Nome do projeto:',workspace?.name||'');if(!name?.trim())return;const description=prompt('Descrição do projeto:',workspace?.description||'');if(description===null)return;try{const updated=await taskApi('/api/task-workspaces',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:workspace.id,name:name.trim(),description})});Object.assign(workspace,updated);renderTaskIdentity();renderTaskProjects()}catch(error){alert(error.message)}};
  createForm.onsubmit=async event=>{event.preventDefault();const status=document.querySelector('#taskWorkspaceCreateStatus'),button=event.submitter;button.disabled=true;status.textContent='Criando projeto...';try{const created=await taskApi('/api/task-workspaces',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:document.querySelector('#taskWorkspaceName').value,description:document.querySelector('#taskWorkspaceDescription').value})});taskContext.workspaces.push(created);createForm.reset();createForm.hidden=true;management.hidden=false;await switchTaskWorkspace(created.id)}catch(error){status.textContent=error.message}finally{button.disabled=false}};
  document.querySelector('#taskInviteForm').onsubmit=async event=>{event.preventDefault();const status=document.querySelector('#taskInviteStatus'),button=event.submitter;button.disabled=true;status.textContent='Enviando convite...';try{await taskApi('/api/task-invites',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({workspace_id:activeTaskWorkspaceId,public_id:document.querySelector('#taskInvitePublicId').value,role:document.querySelector('#taskInviteRole').value})});event.currentTarget.reset();status.textContent='Convite enviado. O acesso será liberado após o aceite.'}catch(error){status.textContent=error.message}finally{button.disabled=false}};
  document.querySelectorAll('[data-task-project-tab]').forEach(button=>button.onclick=()=>{document.querySelectorAll('[data-task-project-tab]').forEach(item=>item.classList.toggle('active',item===button));document.querySelectorAll('[data-task-project-panel]').forEach(panel=>panel.hidden=panel.dataset.taskProjectPanel!==button.dataset.taskProjectTab)});
}
async function initializeNativeTasks(){
  ...
  await loadTaskContext();try{const cached=JSON.parse(userStorage.getItem(taskCacheKey())||'null');if(cached?.data&&Date.now()-cached.savedAt<7*86400000){nativeTasks=cached.data;taskLoadedAt=cached.savedAt;nativeTasks.tasks=(nativeTasks.tasks||[]).filter(task=>!task.expires_at||new Date(task.expires_at)>new Date());renderNativeTasks()}}catch{}
  initializeTaskProjects();
  ...
  ['#taskSearch','#taskPriorityFilter','#taskAssigneeFilter'].forEach(selector=>{const element=document.querySelector(selector);element[element.tagName==='INPUT'?'oninput':'onchange']=renderNativeTasks});
  const filterToggle=document.querySelector('#taskFilterToggle'),filterPanel=document.querySelector('#taskFilterPanel');filterToggle.onclick=()=>{const open=filterPanel.hidden;filterPanel.hidden=!open;filterToggle.setAttribute('aria-expanded',String(open))};
  document.querySelector('#taskClearFilters').onclick=()=>{['#taskPriorityFilter','#taskAssigneeFilter'].forEach(selector=>document.querySelector(selector).value='');renderNativeTasks()};
  document.querySelector('#taskRetentionDetails').onclick=()=>alert('Cada tarefa e seus arquivos são removidos permanentemente 72 horas após a criação. Antes desse prazo, você pode editar, mover ou baixar os anexos normalmente.');
  ...
  document.querySelector('#taskForm').onsubmit=async event=>{
    ...
    const existing=nativeTasks.tasks.find(task=>task.id===id),payload={id,title:document.querySelector('#taskTitle').value,description:document.querySelector('#taskDescription').value,assignee:document.querySelector('#taskAssignee').value,priority:document.querySelector('#taskPriority').value,due_date:document.querySelector('#taskDueDate').value||null,labels:document.querySelector('#taskLabels').value.split(',').map(item=>item.trim()).filter(Boolean),column_id:columnId,project_id:existing?.project_id||null,module_id:existing?.module_id||null,cycle_id:existing?.cycle_id||null,estimate_minutes:Number(document.querySelector('#taskEstimate').value)||null,completed_at:taskColumnRole(column)==='completed'?new Date().toISOString():null,position:nativeTasks.tasks.filter(task=>task.column_id===columnId).length};
...
function initializeTaskStructures(){
  ...
  const selectTab=name=>{document.querySelectorAll('[data-structure-tab]').forEach(button=>{const active=button.dataset.structureTab===name;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active))});document.querySelectorAll('[data-structure-panel]').forEach(panel=>panel.hidden=panel.dataset.structurePanel!==name)};
  const open=()=>{renderTaskStructures();selectTab('stages');status.textContent='';modal.hidden=false};
  ...
  document.querySelectorAll('[data-structure-tab]').forEach(button=>button.onclick=()=>selectTab(button.dataset.structureTab));
  document.querySelectorAll('[data-create-structure]').forEach(button=>button.onclick=async()=>{const type=button.dataset.createStructure;let url,payload,required=[];if(type==='column'){url='/api/task-columns';payload={title:document.querySelector('#newTaskColumnTitle').value.trim(),role:document.querySelector('#newTaskColumnRole').value,position:nativeTasks.columns.length};required=[payload.title]}else if(type==='project'){url='/api/task-projects';payload={title:document.querySelector('#newTaskProjectTitle').value.trim()};required=[payload.title]}else if(type==='module'){url='/api/task-modules';payload={project_id:document.querySelector('#newTaskModuleProject').value,title:document.querySelector('#newTaskModuleTitle').value.trim()};required=[payload.project_id,payload.title]}else{url='/api/task-cycles';payload={project_id:document.querySelector('#newTaskCycleProject').value,title:document.querySelector('#newTaskCycleTitle').value.trim(),starts_on:document.querySelector('#newTaskCycleStart').value||null,ends_on:document.querySelector('#newTaskCycleEnd').value||null};required=[payload.project_id,payload.title]};if(required.some(value=>!value))return status.textContent=type==='module'||type==='cycle'?'Selecione um projeto e informe um nome.':'Informe um nome antes de continuar.';button.disabled=true;status.textContent='Salvando...';try{await taskApi(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});status.textContent=type==='column'?'Etapa adicionada ao quadro.':'Item criado.';await loadNativeTasks();renderTaskStructures();if(type!=='column')selectTab('advanced')}catch(error){status.textContent=error.message}finally{button.disabled=false}});
}
function renderTaskStructures(){
  const target=document.querySelector('#taskStructureExisting'),advanced=document.querySelector('#taskAdvancedStructures'),status=document.querySelector('#taskStructureStatus'),projects=nativeTasks.projects||[],options=projects.map(item=>`<option value="${item.id}">${escapeHtml(item.title)}</option>`).join(''),hasProjects=projects.length>0;
  ['#newTaskModuleProject','#newTaskCycleProject'].forEach(selector=>{const select=document.querySelector(selector);select.innerHTML=hasProjects?`<option value="">Selecione um projeto</option>${options}`:'<option value="">Nenhum projeto criado</option>';select.disabled=!hasProjects});
  document.querySelectorAll('[data-create-structure="module"],[data-create-structure="cycle"]').forEach(button=>button.disabled=!hasProjects);document.querySelectorAll('.task-project-required').forEach(help=>help.hidden=hasProjects);
  target.innerHTML=`<section class="task-structure-columns">${nativeTasks.columns.map((item,index)=>`<div><span class="task-stage-position">${index+1}</span><span class="task-stage-copy"><b>${escapeHtml(item.title)}</b><small>${taskColumnRoleLabel(taskColumnRole(item))} · ${nativeTasks.tasks.filter(task=>task.column_id===item.id).length} tarefa(s)</small></span><span class="task-column-order"><button type="button" data-move-column="up" data-id="${item.id}" ${index===0?'disabled':''} aria-label="Mover ${escapeHtml(item.title)} para a esquerda">←</button><button type="button" data-move-column="down" data-id="${item.id}" ${index===nativeTasks.columns.length-1?'disabled':''} aria-label="Mover ${escapeHtml(item.title)} para a direita">→</button></span><button class="task-stage-configure" type="button" data-configure-structure-column="${item.id}">Configurar</button></div>`).join('')}</section>`;
  const groups=[['Projetos','project',projects],['Módulos','module',nativeTasks.modules||[]],['Ciclos','cycle',nativeTasks.cycles||[]]];advanced.innerHTML=groups.map(([title,type,items])=>`<section><h4>${title}</h4>${items.map(item=>`<div><span title="${escapeHtml(item.title)}">${escapeHtml(item.title)}</span><button type="button" data-edit-structure="${type}" data-id="${item.id}" data-title="${escapeHtml(item.title)}">Renomear</button><button type="button" data-delete-structure="${type}" data-id="${item.id}">Excluir</button></div>`).join('')||'<small>Nenhum item criado.</small>'}</section>`).join('');
  target.querySelectorAll('[data-configure-structure-column]').forEach(button=>button.onclick=()=>{document.querySelector('#taskStructureModal').hidden=true;openTaskColumnSettings(button.dataset.configureStructureColumn)});
  ...
  advanced.querySelectorAll('[data-edit-structure]').forEach(button=>button.onclick=async()=>{const title=prompt('Novo nome:',button.dataset.title);if(!title?.trim())return;try{await taskApi(`/api/task-${button.dataset.editStructure}s`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:button.dataset.id,title:title.trim()})});await loadNativeTasks();renderTaskStructures()}catch(error){status.textContent=error.message}});
  advanced.querySelectorAll('[data-delete-structure]').forEach(button=>button.onclick=async()=>{if(!confirm('Excluir este item? As tarefas serão preservadas sem este vínculo.'))return;try{await taskApi(`/api/task-${button.dataset.deleteStructure}s?id=${button.dataset.id}`,{method:'DELETE'});await loadNativeTasks();renderTaskStructures()}catch(error){status.textContent=error.message}});
}
...
if(requestedView!=='tasks')void loadTaskContext();
...
function renderCommentsPages(){const select=document.querySelector('#commentsPageSelect');select.innerHTML='<option value="">Selecione uma página</option>'+commentsPages.map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.name)}</option>`).join('');if(commentsPage&&commentsPages.some(p=>p.id===commentsPage.id))select.value=commentsPage.id;else{commentsPage=null;commentsData=[];renderComments()}select.disabled=!commentsPages.length}
...
const campaignModule=await import('./campaign-manager-ui.js?v=20260910-account-workspace');
...
document.querySelector('.sidebar nav').addEventListener('click',event=>{if(event.target.closest('a')){if(campaignManagerUI.busy()){event.preventDefault();event.stopImmediatePropagation();return}hideAccountPage()}},true);
routeAccount();
