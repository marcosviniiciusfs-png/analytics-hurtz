...
window.fetch=(input,options={})=>{const url=typeof input==='string'?input:input?.url||'',isMonitorApi=url.startsWith('/api/');return isMonitorApi?(personalIdentity?.personal&&/^\/api\/meta-(spend|analysis)\?/.test(url)?personalReportFetch(url,options):monitorApiFetch(input,options)):browserFetch(input,options)};
...
function hydrateAnalysisLoaders(){clearAnalysisLoaders();if(!window.lottie)return;document.querySelectorAll('#analysis .analysis-lottie-animation').forEach(container=>analysisLoaderAnimations.push(window.lottie.loadAnimation({container,renderer:'svg',loop:true,autoplay:true,path:'toaster.json?v=20260723-02'})))}
