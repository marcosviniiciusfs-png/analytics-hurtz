...
function renderNativeTasks(){
  ...
  board.innerHTML=nativeTasks.columns.map(column=>{
    ...
    return `<section class="task-column task-column-${role}" data-task-column="${column.id}" data-column-role="${role}"><header class="task-column-header"><div class="task-column-title"><button class="task-column-name" type="button" data-rename-column="${column.id}" title="Editar nome e função da etapa">${escapeHtml(column.title)}</button><small>${taskColumnRoleLabel(role)}</small><span title="${total} tarefas nesta etapa">${total}</span></div><button class="task-column-menu" type="button" data-delete-column="${column.id}" title="Excluir etapa vazia">×</button></header><div class="task-list">${items.map(task=>taskCardMarkup(task,completedColumn)).join('')}${inlineNewColumnId===column.id?`<article class="task-card task-card-editing task-new-card">${taskInlineForm(null,column.id)}</article>`:''}</div><button class="task-add-card" type="button" data-quick-task="${column.id}" ${inlineNewColumnId===column.id?'disabled':''}><span>+</span> Adicionar Cartão</button></section>`;
  ...
  board.querySelectorAll('[data-rename-column]').forEach(button=>button.onclick=async()=>{const column=nativeTasks.columns.find(item=>item.id===button.dataset.renameColumn),title=prompt('Nome da etapa:',column?.title||'');if(!title?.trim())return;const choices='1 Normal\n2 Em andamento\n3 Revisão\n4 Bloqueada\n5 Concluída',current={standard:1,in_progress:2,review:3,blocked:4,completed:5}[taskColumnRole(column)],choice=prompt('Função da etapa:\n'+choices,String(current));if(choice===null)return;const role=({1:'standard',2:'in_progress',3:'review',4:'blocked',5:'completed'})[choice]||taskColumnRole(column);await taskApi('/api/task-columns',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:column.id,title:title.trim(),position:column.position,role})});await loadNativeTasks()});
  board.querySelectorAll('[data-delete-column]').forEach(button=>button.onclick=async()=>{const column=nativeTasks.columns.find(item=>item.id===button.dataset.deleteColumn);if(!column||nativeTasks.columns.length<=1)return alert('O quadro precisa manter pelo menos uma etapa.');if(!confirm(`Excluir a etapa “${column.title}”? Ela precisa estar vazia.`))return;try{await taskApi(`/api/task-columns?id=${column.id}`,{method:'DELETE'});await loadNativeTasks()}catch(error){alert(error.message)}});
