...
function renderComments(){
 ...
 root.innerHTML=rows.length?rows.map(item=>`<button type="button" class="comments-ad-card comments-ad-open" data-comment-ad="${escapeHtml(commentAdKey(item))}" aria-haspopup="dialog"><span class="comments-ad-head">${item.ad.thumbnail_url?`<img class="comments-ad-thumb" src="${escapeHtml(item.ad.thumbnail_url)}" alt="" referrerpolicy="no-referrer">`:'<span class="comments-ad-thumb"></span>'}<span class="comments-ad-copy"><strong>${escapeHtml(item.ad.name)}</strong><span>${escapeHtml(item.account_name)} · ${escapeHtml(item.ad.campaign_name)}</span><small>${escapeHtml(commentDate(item.ad))}</small></span><span class="comments-ad-meta"><span class="comments-status ${item.ad.effective_status==='ACTIVE'?'active':'inactive'}">${item.ad.effective_status==='ACTIVE'?'Ativo':item.ad.effective_status==='PAUSED'?'Pausado':'Inativo'}</span><span class="comments-count">${item.comments.length} comentário(s)${item.deleted_count?' · '+item.deleted_count+' apagado(s)':''}</span><span class="comment-platform">${item.platform==='instagram'?'Instagram':item.platform==='facebook'?'Facebook · Página':'Origem não informada'}</span><span class="comments-open-label">Ver comentários →</span></span></span></button>`).join(''):'<div class="comments-empty"><strong>Nenhum comentário encontrado</strong><span>Selecione uma página ou altere os filtros.</span></div>';
...
async function renderDeletedComments(item){const request=++commentsArchiveRequest,list=document.querySelector('#commentsDialogList');list.innerHTML='<p class="comments-dialog-empty" role="status">Carregando histórico de exclusões...</p>';updateCommentsSelection();if(!item)return;
 ...
 list.innerHTML='<p class="comments-archive-note">Exclusões confirmadas pelo Analytics a partir desta atualização. Exclusões anteriores ou feitas fora do app não têm conteúdo recuperável aqui. Histórico limitado aos 2.000 registros mais recentes do seu espaço.</p>'+(data.comments.length?data.comments.map(record=>{const comment=record.comment;return `<article class="comment-row comment-deleted">${commentAvatar(comment)}<div class="comment-copy"><strong>${escapeHtml(comment.from?.name||'Nome não disponibilizado pela Meta')}</strong><span class="comment-platform">${comment.platform==='instagram'?'Instagram':'Facebook · Página'}</span><p>${escapeHtml(comment.message||'Comentário sem texto')}</p><small>Excluído em ${new Date(record.deleted_at).toLocaleString('pt-BR')} · Exclusão confirmada pela Meta</small></div><span class="comment-deleted-label">Apagado</span></article>`}).join(''):'<p class="comments-dialog-empty">Nenhuma exclusão registrada para esta publicação.</p>');bindCommentAvatarFallback();
