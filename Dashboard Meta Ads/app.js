...
let commentsPage=null,commentsPages=[],commentsPageRequest=null;
async function chooseCommentsPage(){
 let modal=document.querySelector('#commentsPageModal');
 if(!modal){modal=document.createElement('dialog');modal.id='commentsPageModal';modal.setAttribute('aria-labelledby','commentsPageTitle');modal.style.cssText='width:min(560px,90vw);max-height:80vh;border:1px solid #e7ded5;border-radius:18px;padding:24px;color:#201b16';modal.innerHTML='<h3 id="commentsPageTitle">Selecione uma página</h3><p>Escolha a página para consultar os comentários dos seus anúncios.</p><input id="commentsPageSearch" type="search" placeholder="Buscar página por nome" aria-label="Buscar página" style="width:100%;padding:12px;margin:12px 0"><div id="commentsPageList" role="status"></div><button type="button" id="commentsPagesRetry">Atualizar páginas</button><button type="button" id="commentsPagesAuthorize">Autorizar páginas no Facebook</button><button type="button" id="commentsPageClose">Fechar</button>';document.body.append(modal);modal.addEventListener('keydown',e=>{if(e.key==='Escape'){e.preventDefault();modal.close()}});modal.querySelector('#commentsPageClose').onclick=()=>modal.close();modal.querySelector('#commentsPageSearch').oninput=renderCommentsPages;modal.querySelector('#commentsPagesRetry').onclick=fetchCommentsPages;modal.querySelector('#commentsPagesAuthorize').onclick=()=>{modal.close();window.dispatchEvent(new Event('hurtz-connect-comments'))};modal.addEventListener('click',e=>{if(e.target===modal){const r=modal.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)modal.close()}});modal.addEventListener('close',()=>document.querySelector('#commentsChoosePage').focus())}
 if(!modal.open)modal.showModal();if(!commentsPages.length)await fetchCommentsPages();else renderCommentsPages();
}
function renderCommentsPages(){const list=document.querySelector('#commentsPageList'),q=document.querySelector('#commentsPageSearch').value.trim().toLocaleLowerCase('pt-BR'),rows=commentsPages.filter(p=>p.name.toLocaleLowerCase('pt-BR').includes(q));list.innerHTML=rows.map(p=>`<button type="button" data-comments-page="${escapeHtml(p.id)}" style="display:block;width:100%;text-align:left;padding:14px;margin:8px 0;background:#fff8f1;border:1px solid #eaded3;border-radius:10px"><strong>${escapeHtml(p.name)}</strong><small style="display:block">${escapeHtml(p.id)}</small></button>`).join('')||'<p>Nenhuma página disponível. Confira a autorização do Facebook ou atualize a lista.</p>';list.querySelectorAll('[data-comments-page]').forEach(button=>button.onclick=()=>{commentsPage=commentsPages.find(p=>p.id===button.dataset.commentsPage);document.querySelector('#commentsPageSummary').textContent=commentsPage.name;document.querySelector('#commentsChoosePage').textContent='Trocar página';commentsData=[];renderComments();document.querySelector('#commentsPageModal').close();void loadAdComments()})}
function fetchCommentsPages(){if(commentsPageRequest)return commentsPageRequest;const list=document.querySelector('#commentsPageList');list.textContent='Carregando suas páginas...';commentsPageRequest=(async()=>{try{const response=await fetch('/api/meta-comment-pages'),data=await response.json();if(!response.ok)throw new Error(data.error||'Não foi possível listar as páginas.');commentsPages=data.pages||[];renderCommentsPages()}catch(e){list.textContent=e.message}finally{commentsPageRequest=null}})();return commentsPageRequest}
...
let requestCommentPermissions=false;
window.addEventListener('hurtz-connect-comments',()=>{requestCommentPermissions=true;facebookConnect.click()});
