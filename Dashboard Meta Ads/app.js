...
function renderComments(){
  const rows=visibleCommentsData(),root=document.querySelector('#commentsResults'),count=rows.reduce((sum,item)=>sum+item.comments.length,0);document.querySelector('#commentsResultsSummary').textContent=commentsData.length?`${count} comentário(s) em ${rows.length} anúncio(s), conforme os filtros locais.`:'Faça uma busca para carregar os resultados.';
  root.innerHTML=rows.length?rows.map(item=>`<article class="comments-ad-card"><div class="comments-ad-head">${item.ad.thumbnail_url?`<img class="comments-ad-thumb" src="${escapeHtml(item.ad.thumbnail_url)}" alt="Prévia do anúncio" referrerpolicy="no-referrer">`:'<span class="comments-ad-thumb"></span>'}<div class="comments-ad-copy"><strong>${escapeHtml(item.ad.name)}</strong><span>${escapeHtml(item.account_name)} • ${escapeHtml(item.ad.campaign_name)}</span><small>${escapeHtml(item.ad.adset_name)} • ID ${escapeHtml(item.ad.id)}</small></div><div class="comments-ad-meta"><span class="comments-status ${item.ad.effective_status==='ACTIVE'?'active':'inactive'}">${item.ad.effective_status==='ACTIVE'?'Ativo':'Inativo'}</span><span class="comments-count">${item.comments.length} comentário(s)</span></div></div><div class="comments-list">${item.comments.map(comment=>`<div class="comment-row" data-comment-row="${escapeHtml(comment.id)}"><label class="comment-check"><input type="checkbox" value="${escapeHtml(comment.id)}" data-comment-select aria-label="Selecionar comentário de ${escapeHtml(comment.from?.name||'autor não identificado')}"></label><span class="comment-avatar">${escapeHtml(commentsInitials(comment.from?.name))}</span><div class="comment-copy"><strong>${escapeHtml(comment.from?.name||'Autor não identificado')}${comment.is_hidden?'<span class="comment-hidden-label">Oculto</span>':''}</strong><p>${escapeHtml(comment.message||'Comentário sem texto')}</p><small>${comment.created_time?new Date(comment.created_time).toLocaleString('pt-BR'):'Data indisponível'} • ${num(comment.like_count)} curtidas • ${num(comment.comment_count)} respostas</small></div><div class="comment-actions">${comment.permalink_url?`<a class="comment-action" href="${escapeHtml(comment.permalink_url)}" target="_blank" rel="noopener noreferrer">Abrir</a>`:''}<button class="comment-action" type="button" data-comment-action="${comment.is_hidden?'unhide':'hide'}" data-comment-id="${escapeHtml(comment.id)}">${comment.is_hidden?'Reexibir':'Ocultar'}</button><button class="comment-action danger" type="button" data-comment-action="delete" data-comment-id="${escapeHtml(comment.id)}">Excluir</button></div></div>`).join('')}</div></article>`).join(''):`<div class="comments-empty"><strong>Nenhum comentário encontrado</strong><span>${commentsData.length?'Altere a busca ou os filtros locais.':'Selecione as contas e clique em buscar comentários.'}</span></div>`;updateCommentsSelection();
}
...
async function moderateComments(action,ids){if(!ids.length)return;const status=document.querySelector('#commentsRequestStatus');status.classList.remove('error');status.textContent=`${action==='delete'?'Excluindo':action==='hide'?'Ocultando':'Reexibindo'} ${ids.length} comentário(s)...`;document.querySelectorAll('[data-comment-action],[data-comments-action]').forEach(button=>button.disabled=true);try{const response=await fetch('/api/meta-comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,comment_ids:ids})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||'A moderação não foi concluída.');const succeeded=new Set((payload.results||[]).filter(row=>row.ok).map(row=>row.id));commentsData=commentsData.map(item=>({...item,comments:item.comments.filter(comment=>action!=='delete'||!succeeded.has(comment.id)).map(comment=>succeeded.has(comment.id)&&action!=='delete'?{...comment,is_hidden:action==='hide'}:comment)})).filter(item=>item.comments.length);renderComments();loadCommentsHistory();status.textContent=`${payload.success||0} concluído(s)${payload.failed?` • ${payload.failed} falha(s)`:''}.`;if(payload.failed)status.classList.add('error')}catch(error){status.textContent=error.message;status.classList.add('error')}finally{document.querySelectorAll('[data-comment-action],[data-comments-action]').forEach(button=>button.disabled=false)}}
...
function initializeComments(){if(commentsInitialized){refreshCommentsPagesIfNeeded();return}commentsInitialized=true;document.querySelector('#commentsPageSelect').onchange=event=>{commentsPage=commentsPages.find(p=>p.id===event.target.value)||null;if(commentsPage)userStorage.setItem('hurtz-comments-page',commentsPage.id);else userStorage.removeItem('hurtz-comments-page');commentsData=[];renderComments();if(commentsPage)void loadAdComments()};document.querySelector('#commentsPagesRefresh').onclick=fetchCommentsPages;document.querySelector('#commentsPagesAuthorize').onclick=()=>window.dispatchEvent(new Event('hurtz-connect-comments'));void fetchCommentsPages();document.querySelector('#commentsFilterForm').onsubmit=event=>{event.preventDefault();loadAdComments()};document.querySelector('#commentsVisibility').onchange=renderComments;document.querySelector('#commentsSearch').oninput=renderComments;document.querySelector('#commentsResults').onchange=event=>{if(event.target.matches('[data-comment-select]'))updateCommentsSelection()};document.querySelector('#commentsResults').onclick=event=>{const button=event.target.closest('[data-comment-action]');if(!button)return;button.dataset.commentAction==='delete'?requestCommentsDelete([button.dataset.commentId]):moderateComments(button.dataset.commentAction,[button.dataset.commentId])};document.querySelector('#commentsSelectAll').onchange=event=>{document.querySelectorAll('[data-comment-select]').forEach(input=>input.checked=event.target.checked);updateCommentsSelection()};document.querySelector('#commentsBulkBar').onclick=event=>{const button=event.target.closest('[data-comments-action]');if(!button)return;const ids=commentsSelectedIds();button.dataset.commentsAction==='delete'?requestCommentsDelete(ids):moderateComments(button.dataset.commentsAction,ids)};document.querySelector('#commentsConfirmInput').oninput=event=>document.querySelector('#commentsConfirmDelete').disabled=event.target.value.trim().toUpperCase()!=='EXCLUIR';document.querySelector('#commentsConfirmDelete').onclick=async()=>{const ids=[...commentsPendingDelete];closeCommentsConfirm();await moderateComments('delete',ids)};document.querySelector('#commentsConfirmClose').onclick=closeCommentsConfirm;document.querySelector('#commentsConfirmCancel').onclick=closeCommentsConfirm;document.querySelector('#commentsConfirmModal').onclick=event=>{if(event.target===event.currentTarget)closeCommentsConfirm()};document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!document.querySelector('#commentsConfirmModal').hidden)closeCommentsConfirm()})}
