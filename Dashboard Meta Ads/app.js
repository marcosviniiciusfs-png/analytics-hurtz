...
function reportMemberAccounts(){return [...document.querySelectorAll('#reportAccountOptions input')].map(input=>({id:input.value,name:input.nextElementSibling.textContent,selected:input.checked}))}
...
function renderReportMemberList(){const query=document.querySelector('#reportMemberSearch').value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase(),rows=reportMemberAccounts().filter(a=>(a.name+' '+a.id).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().includes(query)).sort((a,b)=>Number(b.selected)-Number(a.selected));document.querySelector('#reportMemberList').innerHTML=rows.length?rows.map(a=>`<button type="button" class="account-member-option ${a.selected?'selected':''}" data-report-member="${escapeHtml(a.id)}" aria-pressed="${a.selected}"><span class="account-member-small" aria-hidden="true">${escapeHtml(reportMemberInitials(a.name))}</span><span class="account-member-copy"><strong>${escapeHtml(a.name)}</strong><small>${escapeHtml(a.id)}</small></span><span class="account-member-check" aria-hidden="true">${a.selected?'✓':''}</span></button>`).join(''):'<p class="account-member-empty">Nenhuma conta encontrada.</p>'}
