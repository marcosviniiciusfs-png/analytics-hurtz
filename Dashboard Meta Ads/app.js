...
function openReportAdsModal(accountId,trigger){
  ...
  document.querySelector('#reportAdsModalBody').innerHTML=`<section class="report-ads-detail"><div class="report-ads-title"><div><span>DETALHAMENTO POR ANÚNCIO</span><h4>Campanhas e peças veiculadas</h4><p>Anúncios retornados pela Meta no período, inclusive resultados atribuídos sem gasto. O status indica a situação atual.</p></div><b>${ads.length} ${ads.length===1?'anúncio':'anúncios'}</b></div><div class="report-ad-list">${ads.map((ad,index)=>reportAdCard(accountId,ad,index)).join('')||'<p class="report-ads-empty">Nenhum anúncio teve veiculação no período selecionado.</p>'}</div></section>`;
