...
function configuration(){
  ...
  const groq={key,model:openrouterKey?'openrouter/free':(process.env.CAMPAIGN_AI_MODEL||values.CAMPAIGN_AI_MODEL||'openai/gpt-oss-20b'),url:openrouterKey?'https://openrouter.ai/api/v1/chat/completions':'https://api.groq.com/openai/v1/chat/completions'};
...
function normalize(result){
  ...
  for(const key of ['name','headline','message','rationale','locationQuery'])if(typeof value[key]==='string')value[key]=value[key].replace(/\*\*/g,'').trim();
...
const fields=['name','headline','message','dailyBudget','category','rationale','destination','locationQuery','ageMin','ageMax','interestQueries','placements'];
const ollamaSchema={type:'object',additionalProperties:false,required:fields,properties:{name:{type:'string'},headline:{type:'string'},message:{type:'string'},dailyBudget:{type:'number',minimum:1,maximum:1000},category:{type:'string',enum:categories},rationale:{type:'string'},destination:{type:'string',enum:['site','whatsapp','form']},locationQuery:{type:'string'},ageMin:{type:'integer',minimum:18,maximum:65},ageMax:{type:'integer',minimum:18,maximum:65},interestQueries:{type:'array',items:{type:'string'},maxItems:5},placements:{type:'string',enum:['automatic','feeds']}}};
const correction='Refaça o rascunho como JSON válido. Use todos os campos exigidos, números para dailyBudget, ageMin e ageMax, uma lista de textos em interestQueries e uma única string automatic ou feeds em placements.';
function localOllama(url){try{const target=new URL(url);return target.protocol==='http:'&&['127.0.0.1','localhost','::1','analytics_ollama'].includes(target.hostname.replace(/^\[|\]$/g,''));}catch{return false;}}
function providers(config){return {groq:config.groq||config.key?{key:config.groq?.key??config.key,model:config.groq?.model??config.model,url:config.groq?.url??config.url}:null,ollama:config.ollama&&localOllama(config.ollama.url)&&config.ollama.model?config.ollama:null};}
function providerFailure(provider,status){const error=fail(status||502,provider==='groq'?'Groq indisponível.':'Ollama local indisponível.');error.provider=provider;return error;}
async function requestGroq(messages,config,fetchImpl){let response,data;try{response=await fetchImpl(config.url||'https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{Authorization:'Bearer '+config.key,'Content-Type':'application/json'},signal:AbortSignal.timeout(45000),body:JSON.stringify({model:config.model,temperature:0.1,response_format:{type:'json_object'},messages})});data=await response.json();}catch{throw providerFailure('groq');}if(!response.ok)throw providerFailure('groq',response.status);return data.choices?.[0]?.message?.content||'';}
async function requestOllama(messages,config,fetchImpl){let response,data;try{response=await fetchImpl(config.url,{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(90000),body:JSON.stringify({model:config.model,messages,format:ollamaSchema,stream:false,keep_alive:'10m',options:{temperature:0.1,num_predict:700}})});data=await response.json();}catch{throw providerFailure('ollama');}if(!response.ok)throw providerFailure('ollama',response.status);return data.message?.content||'';}
async function draftFrom(provider,context,config,fetchImpl){const messages=[{role:'system',content:systemPrompt},{role:'user',content:JSON.stringify(context)}];for(let attempt=0;attempt<2;attempt++){const content=await (provider==='groq'?requestGroq(messages,config,fetchImpl):requestOllama(messages,config,fetchImpl));let result;try{result=normalize(JSON.parse(content));}catch{result=null;}if(valid(result))return Object.fromEntries(fields.map(key=>[key,result[key]]));messages.push({role:'assistant',content},{role:'user',content:correction});}throw providerFailure(provider);}
async function plan(context,{fetchImpl=fetch,config=configuration()}={}){const available=providers(config);for(const [provider,settings] of [['groq',available.groq],['ollama',available.ollama]]){if(!settings||(provider==='groq'&&!settings.key))continue;try{return await draftFrom(provider,context,settings,fetchImpl);}catch(error){console.warn('Campaign planner provider failed:',provider,'status',error.status||502);}}if(!available.groq&&!available.ollama)throw fail(503,'A criação inteligente não está configurada. Use a criação manual ou contate o administrador.');throw fail(503,'A criação inteligente está temporariamente indisponível. Tente novamente ou use a criação manual.');}
module.exports={plan,configuration,valid,normalize,missingRequiredDetails,localOllama};
