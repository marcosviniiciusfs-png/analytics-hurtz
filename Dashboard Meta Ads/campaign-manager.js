...
function createCampaignManager({graph,rows,authorizeAccounts,connection,read,write,fetchImpl,planCampaign=require("./campaign-planner").plan}){
  ...
  async function handle(req,user,url){const action=url.pathname.slice('/api/ads-manager/'.length),account=url.searchParams.get('account');const {conn,account:accountInfo}=await access(user,account,req.method!=='GET');
    ...
    try{
      ...
      if(p.location&&!existing){const geo=(read('ads-locations',user.id)||[]).find(x=>x.key===p.location.key&&x.type===p.location.type);if(!geo)throw fail(400,'A localização não está mais disponível. Busque novamente.');if(p.countries.length!==1||p.countries[0]!==geo.country)throw fail(400,'Confira o país da localização.');targeting.geo_locations=geo.type==='country'?{countries:[geo.country]}:geo.type==='region'?{regions:[{key:geo.key}]}:{cities:[{key:geo.key}]};}
