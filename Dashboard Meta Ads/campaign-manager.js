...
function draftFromVideoAnalysis(analysis={}){const pick=(...values)=>values.map(value=>copyFact(value)).find(Boolean)||'';const offer=pick(analysis.offer,analysis.visibleText?.[0])||'Conheça nossa oferta',transcript=pick(analysis.transcript,...(analysis.visibleText||[])),headline=offer.slice(0,100),name=('Campanha — '+headline).slice(0,200),benefits=[...(analysis.benefits||[]),...(analysis.visibleText||[]),transcript].map(copyFact).filter(Boolean).filter((value,index,items)=>items.indexOf(value)===index&&value!==offer).slice(0,3);while(benefits.length<3)benefits.push(['Informações claras para você','Atendimento para tirar suas dúvidas','Orientação para o próximo passo'][benefits.length]);const cta=pick(analysis.cta)||'Fale com nossa equipe.',message=['🏠 '+offer,transcript||'Conheça os detalhes desta oportunidade.','✅ '+benefits[0],'✅ '+benefits[1],'✅ '+benefits[2],'Saiba mais sobre esta oportunidade.','📲 '+cta].join('\n').slice(0,2200);return {name,headline,message,dailyBudget:1,category:'',rationale:'Rascunho preparado a partir das informações encontradas no vídeo. Revise os campos antes de publicar.',destination:['whatsapp','site','form'].includes(analysis.destination)?analysis.destination:'site',locationQuery:pick(analysis.location,'Brasil'),ageMin:Number.isInteger(analysis.audience?.ageMin)?analysis.audience.ageMin:18,ageMax:Number.isInteger(analysis.audience?.ageMax)?analysis.audience.ageMax:65,interestQueries:[],placements:'automatic'};}
...
function createCampaignManager({graph,rows,authorizeAccounts,connection,read,write,fetchImpl,planCampaign=require("./campaign-planner").plan,requiredDetails=require("./campaign-planner").missingRequiredDetails,analyzeVideo=require("./video-campaign-analysis").analyzeCampaignVideo,videoDescription=require("./video-campaign-analysis").analysisDescription,plannerConfiguration=require("./campaign-planner").configuration}){
  ...
  async function handle(req,user,url){const action=url.pathname.slice('/api/ads-manager/'.length),account=url.searchParams.get('account');const {conn,account:accountInfo}=await access(user,account,req.method!=='GET');
    if(req.method==='GET'){
      ...
      if(action==='assets'){const p=await pages(conn,account),instagram=await instagramProfiles(conn,account,p),whatsapp={state:'empty',items:[],message:'Selecione uma Página para consultar os números do WhatsApp.',retryable:false};return {pages:p,instagram,whatsapps:[],whatsapp,warning:''}}
    ...
    try{
      ...
      // A Meta valida o vínculo Página/BM/número quando o conjunto é criado. Não
      // bloqueamos a publicação tentando enumerar WABAs antes: essa enumeração
      // depende de permissões que muitos anunciantes não concedem ao app.
      if(destination==='whatsapp'){const phone=String(p.phone||'').replace(/\D/g,'');if(!/^\d{10,15}$/.test(phone))throw fail(400,'Informe um WhatsApp com DDI e DDD.');p.phone=phone;target='https://wa.me/'+phone;cta={type:'WHATSAPP_MESSAGE',value:{link:target}}}
