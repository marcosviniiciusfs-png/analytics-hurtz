...
const input = path.join(dir, 'judo_edit_epico_local.mp4');
const output = path.join(dir, 'judo_edit_epico_upscaled.mp4');
if (!fs.existsSync(input)) throw new Error('Edit local nao encontrado');

const args = [
  '-y',
  '-hide_banner',
  '-i', input,
  '-vf', 'scale=1440:2560:flags=lanczos,unsharp=5:5:0.45:3:3:0.18,eq=contrast=1.04:saturation=1.08',
  '-c:v', 'libx264',
  '-preset', 'slow',
  '-crf', '17',
  '-pix_fmt', 'yuv420p',
